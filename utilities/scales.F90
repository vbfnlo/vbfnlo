...
!     and printout for scales. Initialization of PDFs.
!***************************************************************************
...
! dynamic scale = momentum transfer of W/Z
      CASE(1)
         ...
         CASE(Hjj, Hjj_AA, Hjj_mu, Hjj_tau, Hjj_bbar, Hjj_WW, Hjj_ZZ_ll, &
              ...
              WPAjj, WMAjj, ZAjj, ZAjj_n, EWAAjj)
...
! dynamic scale = min(pt_j)
      CASE(2)
         ...
         CASE(Hjj, Hjj_AA, Hjj_mu, Hjj_tau, Hjj_bbar, Hjj_WW, Hjj_ZZ_ll, &
              ...
              WPhadWMjj, WPWMhadjj, WPhadZjj, WPZhadjj, WMhadZjj, WMZhadjj, ZZhadjj, &
              Hjj_WPhadWM, Hjj_WPWMhad, Hjj_ZZhad, WPhadWPjj, WMhadWMjj,  &
              Sp2jj_WW, Sp2jj_ZZ_ll, Sp2jj_ZZ_lnu, WPAjj, WMAjj, ZAjj, ZAjj_n, EWAAjj)
...
! dynamical scale sum(pT_j*exp(|y_j-y12|)) + sum( ET V ) ( see arXiv 1311.6738 )
      CASE(9)
         SELECT CASE(procID)
         CASE(QCDWMWMjj,QCDWPWPjj,QCDWPjj,QCDWMjj,QCDWPZjj,QCDWMZjj,QCDAAjj,QCDAjj, &
...
! dynamical scale HT/2,  HT = sum(pT partons) + sum( ET V )
      CASE(10)
         ...
         CASE DEFAULT
            ...
            STOP
         END SELECT
! dynamical scale ET_V (V = photon/lepton system), ET_V = sum( ET V )
      CASE(11)
         SELECT CASE(procID)
         CASE(QCDWMWMjj,QCDWPWPjj,QCDWPjj,QCDWMjj,QCDWPZjj,QCDWMZjj,QCDAAjj,QCDAjj, &
              QCDZjj_l, QCDZjj_nu,QCDWPAjj,QCDWMAjj,QCDZZjj_ll,QCDZZjj_lnu,QCDZAjj_l,QCDZAjj_n, &
              ...
              Sp2jj_WW, Sp2jj_ZZ_ll, Sp2jj_ZZ_lnu, WPAjj, WMAjj, ZAjj, ZAjj_n, EWAAjj)
            if (lglobalprint) print*,"Dynamical factorization scale chosen:"
            if (lglobalprint) print*,"      mu_F = ET(VV)"
         CASE DEFAULT
            print *,"unreasonable scale choice for this process:  &
                  id_muf = ",id_muf
            STOP
...
! dynamical scale sqrt( pt_j1 * pt_j2 )
      CASE(12)
         SELECT CASE(procID)
         CASE(QCDWMWMjj,QCDWPWPjj,QCDWPjj,QCDWMjj,QCDWPZjj,QCDWMZjj,QCDAAjj,QCDAjj, &
              QCDZjj_l, QCDZjj_nu,QCDWPAjj,QCDWMAjj,QCDZZjj_ll,QCDZZjj_lnu,QCDZAjj_l,QCDZAjj_n, &
         ...
         CASE DEFAULT
            print *,"unreasonable scale choice for this process:  &
                 id_muf = ",id_muf
         END SELECT
!... dynamic scale = (pt_j1*pt_j2*pt_j3)^(1/3)
!... for H+3jet process in gluon fusion
      CASE(13)                  ! before merged 8
         SELECT CASE(procID)
         CASE(GFHjjj)
            if (lglobalprint) print*,"Dynamical factorization scale chosen:"
            if (lglobalprint) print*,"     mu_F = (pt_j1*pt_j2*pt_j3)^(1/3)"
         CASE DEFAULT
            print *,"unreasonable scale choice for this process:  &
                 id_muf = ",id_muf
            STOP
...
!... dynamic scale = 1/3*[ pT(H) + sum(pT_ji) ]
!... for H+3jet process in gluon fusion
      CASE(14)                  ! before merged 9
         SELECT CASE(procID)
         CASE(GFHjjj,GFHjj)
            if (lglobalprint) print*,"Dynamical factorization scale chosen:"
            if (lglobalprint) print*,"     mu_F = 1/3*[pT(H)+sum(pT_j)]"
         CASE DEFAULT
            print *,"unreasonable scale choice for this process: &
                 id_muf = ",id_muf
            STOP
...
!... dynamic scale = 1/3*{ sqrt[ mH^2 + spT_H^2) ]^(1/2) + sum(pT_ji)}
!... for H+3jet process in gluon fusion
      CASE(15)                  ! before merged 10 
         SELECT CASE(procID)
         CASE(GFHjjj,GFHjj)
            if (lglobalprint) print*,"Dynamical factorization scale chosen:"
            if (lglobalprint) print*,"  mu_F = 1/3*[ET(H) + sum(pT_ji)]"
         CASE DEFAULT
            print *,"unreasonable scale choice for this process: &
                 id_muf = ",id_muf
            STOP
...
!... dynamic scale = 1/3 sum( pT_j*exp(|y_j-y12| ))  ( see arXiv 1311.6738 )
!... for H+3jet process in gluon fusion
      CASE(16)                  !before merged 11 !
         SELECT CASE(procID)
         CASE(GFHjjj)
            if (lglobalprint) print*,"Dynamical factorization scale chosen:"
            if (lglobalprint) print*,"  mu_F = 1/3 sum( pT_j*exp(|y_j-y12|))"
         CASE DEFAULT
...
! user defined constant scale
      CASE(0)
         ...
         if(.not.resalpha) then
         if (lglobalprint)  print*, "Change to agree with Madgraph LO",alphareplace
          als(1,1)= alphareplace
         endif
...
! dynamical scale ET/2,  ET = ET(jj) + sum( ET V )
      CASE(10)
         SELECT CASE(procID)
         CASE(QCDWMWMjj,QCDWPWPjj,QCDWPjj,QCDWMjj,QCDWPZjj,QCDWMZjj,QCDAAjj,QCDAjj, &
              QCDZjj_l, QCDZjj_nu,QCDWPAjj,QCDWMAjj,QCDZZjj_ll,QCDZZjj_lnu,QCDZAjj_l,QCDZAjj_n, &
...
! dynamical scale ET_V,  ET_V = sum( ET V )
      CASE(11)
         SELECT CASE(procID)
         CASE(QCDWMWMjj,QCDWPWPjj,QCDWPjj,QCDWMjj,QCDWPZjj,QCDWMZjj,QCDAAjj,QCDAjj, &
              QCDZjj_l, QCDZjj_nu,QCDWPAjj,QCDWMAjj,QCDZZjj_ll,QCDZZjj_lnu,QCDZAjj_l,QCDZAjj_n, &
            ...
            if (lglobalprint) print*,"Dynamical renormalization scale chosen:"
            if (lglobalprint) print*,"      mu_R = ET(VV)"
         CASE DEFAULT
            print *,"unreasonable scale choice for this process:  &
                  id_mur = ",id_mur
            STOP
...
! dynamical scale sqrt( pt_j1 * pt_j2 )
      CASE(12)
         ...
         CASE(QCDWMWMjj,QCDWPWPjj,QCDWPjj,QCDWMjj,QCDWPZjj,QCDWMZjj,QCDAAjj,QCDAjj, &
              ...
              Sp2jj_WW, Sp2jj_ZZ_ll, Sp2jj_ZZ_lnu, WPAjj, WMAjj, ZAjj, ZAjj_n, EWAAjj)
            if (lglobalprint) print*,"Dynamical factorization scale chosen:"
            if (lglobalprint) print*,"      mu_R =   sqrt( pt_j1 * pt_j2 )"
         CASE DEFAULT
            print *,"unreasonable scale choice for this process:  &
                  id_mur = ",id_mur
            STOP
...
!... dynamic scale alpha(pt_j1)*alpha(pt_j2)*alpha(pt_j3)*alpha^2(m_H)
!... for H+3jet process in gluon fusion
      CASE(13)                  ! before merged 8
         SELECT CASE(procID)
         CASE(GFHjjj)
            if (lglobalprint) print*,"Dynamical scale for alphas chosen:"
            if (lglobalprint) print*,"alphas = alpha(pt_j1)*alpha(pt_j2)*alpha(pt_j3) &
      *alpha^2(m_H)"
         CASE DEFAULT
            print *,"unreasonable scale choice for this process: &
                 id_mur = ",id_mur
...
!... dynamic scale = 1/3*[ PT(H)^2 + sum(pT_ji^2) ]^(1/2) 
!... for H+3jet process in gluon fusion
      CASE(14)                  ! before merged 9
         SELECT CASE(procID)
         CASE(GFHjjj,GFHjj)
            if (lglobalprint) print*,"Dynamical scale for alphas chosen:"
            if (lglobalprint) print*,"   alphas = alphas((1/3*[pt(H)+sum(pT_j)])**2)"
         CASE DEFAULT
            print *,"unreasonable scale choice for this process: &
                 id_mur = ",id_mur
            STOP
...
!... dynamic scale = 1/3*{ ET(H)) + sum(pT_ji)}
!... for H+3jet process in gluon fusion
      CASE(15)                  ! before merged 10
         SELECT CASE(procID)
         CASE(GFHjjj,GFHjj)
            if (lglobalprint) print*,"Dynamical scale for alphas chosen:"
            if (lglobalprint) print*,"alphas = alphas(((1/3*[ET(H)+sum_pT_ji^2]^(1/2) &
        + sum(pT_j)])**2)"
         CASE DEFAULT
            print *,"unreasonable scale choice for this process: &
                 id_mur = ",id_mur
...
!... for H+3jet process in gluon fusion
      CASE(16)                  ! before merged 11
         ...
         CASE(GFHjjj)
            if (lglobalprint) print*,"Dynamical scale for alphas chosen:"
            if (lglobalprint) print*,"alphas = alphas((1/3 sum( pT_j*exp(|y_j-y12|)))**2)"
         CASE DEFAULT
            print *,"unreasonable scale choice for this process: &
                 id_mur = ",id_mur
            STOP
...
!* Setting LO alfas(MZ), for printout
...
! Gluon fusion with 2 jets
      CASE(GFHjj, GFHjj_AA, GFHjj_mu, GFHjj_tau, GFHjj_bbar, &
              GFHjj_WW, GFHjj_ZZ_ll, GFHjj_ZZ_lnu)
         call calcGGFScales(p)

! Gluon fusion with 3 jets
      CASE(GFHjjj)
         call calcGGFScales3j(p,jets,v)
...
#include "koppln.inc"
      ...
      real*8 p(0:3,max_p),ph(0:3),pH_Tsq
      integer l,mu
...
!cfc      DOUBLE PRECISION CLR,XM2,XMG,Bk,Vk,Ak
!      COMMON /BKOPOU/ CLR(4,5,-1:1),XM2(6),XMG(6),Bk(6,6,6), &
 !          Vk(4,5),Ak(4,5)
...
!... dynamic scale = 1/3*[ pT(H) + sum(pT_ji) ]
      CASE(14)
!fc here pt, not pt**2
         qsq  = sqrt(p(1,3)**2+p(2,3)**2)
         qsq2 = sqrt(p(1,4)**2+p(2,4)**2)
         ...
         do mu=0,3
            pH(mu)=(p(mu,1)+p(mu,2)-p(mu,3)-p(mu,4))
         enddo
!fc        print*," phsq", sqrt(ph(0)**2-ph(1)**2-ph(2)**2-ph(3)**2),xm2(6),xmh

         pH_Tsq=0d0
         pH_Tsq=sqrt(pH(1)**2+pH(2)**2)
...
!... dynamic scale = 1/3*{ sqrt[ mH^2 + spT_H^2) ]^(1/2) + sum(pT_ji)}
      CASE(15)
         ...
         enddo
         pH_Tsq=0d0
         pH_Tsq=pH(1)**2+pH(2)**2
         qsq = (sqrt(xm2(6)+pH_Tsq)+qsq+qsq2)**2
         qsq = MAX(qsq,5D0)* xif**2
          do L = 1,3
            mufsq(1,L) = qsq
            mufsq(2,L) = qsq
...
!fc here pt, not pt**2
         ...
         do mu=0,3
            pH(mu)=(p(mu,1)+p(mu,2)-p(mu,3)-p(mu,4))
         enddo
!fc         print*," phsq renorm", sqrt(ph(0)**2-ph(1)**2-ph(2)**2-ph(3)**2),xm2(6),xmh
!fc         pause
         pH_Tsq=0d0
         pH_Tsq=sqrt(pH(1)**2+pH(2)**2)
...
!****************************************************************************
      SUBROUTINE calcGGFScales3j(p,jets,v)
!****************************************************************************
!... author   : Michael Kubocz 
!... date     : 03.08.09
!... modified : 13.10.16
!... Calculates factorization and renormalization scales for 
!... Higgs + 3jet processes in gluon fusion
!****************************************************************************
      implicit none
...
#include "koppln.inc"
      ...
      real*8 p(0:3,max_p), pH(0:3), pH_Tsq, temp, y12, &
            jets(0:7,max_jets,max_kin)
      real*8 v(0:3,max_v,max_kin)
      integer L, i, mu
      real*8 alphas5, qsq, qsq2, qsq3, pt1, pt2, pt3
      external alphas5
...
!... dynamic scale = (pT_j1*pT_j2*pT_j3)^(1/3)
      CASE(13)
         qsq  = p(1,3)**2+p(2,3)**2
         qsq2 = p(1,4)**2+p(2,4)**2
         qsq3 = p(1,5)**2+p(2,5)**2

         qsq  = MAX(qsq,4D0)
         qsq2 = MAX(qsq2,4D0)
         qsq3 = MAX(qsq3,4D0)

         qsq = (qsq*qsq2*qsq3)**(1d0/3d0)
         qsq = MAX(qsq,5D0)* xif**2
          do L = 1,3
            mufsq(1,L) = qsq
            mufsq(2,L) = qsq
...
!fc here pt, not pt**2
         ...
         do mu=0,3
            pH(mu)=(p(mu,1)+p(mu,2)-p(mu,3)-p(mu,4)-p(mu,5))
!fc            print*, v(mu,1,1), pH(mu),v(mu,1,1)/pH(mu)
         enddo
!fc        print*," phsq norm", sqrt(ph(0)**2-ph(1)**2-ph(2)**2-ph(3)**2),sqrt(v(0,1,1)**2-v(1,1,1)**2-v(2,1,1)**2-v(3,1,1)**2),xm2(6),xmh

         pH_Tsq=0d0
         pH_Tsq=sqrt(pH(1)**2+pH(2)**2)

!fc Here HT, not HT**2
         qsq =(pH_Tsq + qsq+qsq2+qsq3)
         qsq = qsq*qsq
         qsq = MAX(qsq,5D0)* xif**2
...
!... dynamic scale = 1/3*{ sqrt[ mH^2 + spT_H^2) ]^(1/2) + sum(pT_ji)}
      CASE(15)
         ...
         pH_Tsq=0d0
         pH_Tsq=pH(1)**2+pH(2)**2
         qsq =(sqrt(xm2(6)+pH_Tsq)+qsq+qsq2+qsq3)**2
         qsq = MAX(qsq,5D0)* xif**2
          do L = 1,3
            mufsq(1,L) = qsq
            mufsq(2,L) = qsq
...
!... dynamic scale = 1/3 sum( pT_j*exp(|y_j-y12| ))  ( see arXiv 1311.6738 )
      CASE(16)
          do L=1,3
            temp=0d0
!... avg rapidity of two hardest jets
            y12 = 0.5d0*(jets(6,1,L)+jets(6,2,L))               
!... sum over jets                
            do i = 3,5
              qsq = p(1,i)**2+p(2,i)**2
              temp = temp + qsq*exp(abs(jets(6,i,L)-y12))
            enddo
            temp=temp*xif
            temp = temp**2
            mufsq(1,L) = temp
...
!... dynamic scale alpha(pt_j1)*alpha(pt_j2)*alpha(pt_j3)*alpha^2(m_H)
      CASE(13)
         ...
         PT1=MAX(PT1,4D0)* xir**2
         PT2=MAX(PT2,4D0)* xir**2
         PT3=MAX(PT3,4D0)* xir**2
         qsq =  xm2(6) * xir**2
         do L = 1,3
            als(1,L) = alphas5(pt1,0)
            als(2,L) = alphas5(pt2,0)
            als(3,L) = alphas5(pt3,0)
            als(4,L) = alphas5(qsq,0)
         enddo
...
!... dynamic scale = 1/3*{ sqrt[ mH^2 + spT_H^2) ]^(1/2) + sum(pT_ji)}
      CASE(15)
         ...
         pH_Tsq=0d0
         pH_Tsq=pH(1)**2+pH(2)**2
         qsq = (sqrt(xm2(6)+pH_Tsq)+qsq+qsq2 &
                     +qsq3)**2
...
!----------------------------      
      ...
      if(isnan(als(1,1))) then
         if (lglobalprint) print*, 'als nan in scales.F calcvxjscales'
         if (lglobalprint) print*, 'setting als=0'
         als(1,1) = 0d0
      endif
...
! dynamical scale = ptj_min
      CASE(2)                 ! this scale makes only sense for the processes with a jet at LO,
         ...
         if (.not.atLeastOneJet) then
            if (lglobalprint) print*, "The scale choice ID = 2 is not valid for this process!"
            if (lglobalprint) print*, "There are no jets defined in any kinematic!"
            stop
         endif
...
! default
      CASE DEFAULT
         if (lglobalprint) print*,"in calcvxjscales undefined scale choice "
         stop
      END SELECT
...
! sqrt( pt_j1 * pt_j2)
      CASE(12)
          do L=1,Lmax
            temp = sqrt(jets(5,1,L)*jets(5,2,L))
