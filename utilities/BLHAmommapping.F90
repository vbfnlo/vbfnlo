...
!      print*, "npairs", npairs !borrar
!      print*, "listpairs", listpairs
!      print*, "nother", nother
...
! qbar -> 2
      ...
      if (np == 3 .and. nother == 1) then
        ...
        blha_fsign(1,nsp,blha_numproc) = int(sign(1.,2.5-listpairs(1,1)))
        blha_physdiagmap(listpairs(2,1),nsp,blha_numproc) = 2 
        blha_fsign(2,nsp,blha_numproc) = int(sign(1.,listpairs(2,1)-2.5))
        blha_physdiagmap(listpairs(3,1),nsp,blha_numproc) = 3
        blha_fsign(3,nsp,blha_numproc) = int(sign(1.,listpairs(3,1)-2.5))
      end if
      blha_idsubproc(nsp,blha_numproc) = 1

!      print*, "blha_physdiagmap", blha_physdiagmap(:,nsp,blha_numproc) !borrar
!      print*, "blha_fsign", blha_fsign(:,nsp,blha_numproc)
...
!      print*, "invmap", invmap !borrar


! choose right subproc
      if (abs(pdgparton(invmap(1))).eq.abs(pdgparton(invmap(2)))) then
        if (mod(abs(pdgparton(invmap(1))),2) .eq. 0) then
          blha_idsubproc(nsp,blha_numproc) = 1 ! uu
        else
          blha_idsubproc(nsp,blha_numproc) = 2 ! dd
        endif
      else ! parton(1) != parton(2)
        if(mod(abs(pdgparton(invmap(1))), 2) .eq. 0) then
          blha_idsubproc(nsp,blha_numproc) = 1 !ud
        else
          blha_idsubproc(nsp,blha_numproc) = 2 !du
        end if
      endif

!      print*, "blha_idsubproc", blha_idsubproc(nsp,blha_numproc) !borrar
!      print*, "nsp", nsp
!      print*, "******************" !borrar
!      print*, "* END MomMapping QCD *"
!      print*, "******************"
!      pause
      blha_numsubproc(blha_numproc) = nsp
...
! get number of W,Z,H,gamma
        ...
        CASE(IDwp + IDwm) ! W+ W-
          if (lVBFlepdec) then
            ...
            if (nparton.eq.2) then
              blha_procsubproc((i-1)*nsp+1,blha_numproc) = WW
              blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .false.
        ...
        CASE(IDwm + IDzl) ! W- Z_l
          if (lVBFlepdec) then
            ...
            if (nparton.eq.2) then
              blha_procsubproc((i-1)*nsp+1,blha_numproc) = WMZ
              blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .false.
            else if (nparton.eq.3) then
              blha_procsubproc((i-1)*nsp+1,blha_numproc) = WMZ
              blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .true.
        ...
        CASE(2*IDzl) ! Z_l Z_l
          if (lVBFlepdec) then
            ...
            if (nparton.eq.2) then
              blha_procsubproc((i-1)*nsp+1,blha_numproc) = ZZ
              blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .false.
            else if (nparton.eq.3) then
              blha_procsubproc((i-1)*nsp+1,blha_numproc) = ZZj
              blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .false.
            endif
          else 
            blha_numsubproc(blha_numproc) = -1
        ...
        CASE(2*IDgamma) ! A A
          if (lVBFlepdec) then
            blha_procsubproc((i-1)*nsp+1,blha_numproc) = EWAAjj
            if (nparton.eq.4) then
              blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .false.
            else if (nparton.eq.5) then
              blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .true.
            ...
            blha_procsubproc((i-1)*nsp+1,blha_numproc) = AA
            if (nparton.eq.2) then
              blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .false.
            else if (nparton.eq.3) then
              blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .true.
         ...
         CASE(IDwm + 2*IDwp) !WpWmWp
          if (nparton.eq.2) then
            blha_procsubproc((i-1)*nsp+1,blha_numproc) = WWWP
            blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .false.
          else if (nparton.eq.3) then
            blha_procsubproc((i-1)*nsp+1,blha_numproc) = WWWP
            blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .true.
         ...
         CASE(IDwp + 2*IDwm) !WmWpWm
          if (nparton.eq.2) then
            blha_procsubproc((i-1)*nsp+1,blha_numproc) = WWWM
            blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .false.
          else if (nparton.eq.3) then
            blha_procsubproc((i-1)*nsp+1,blha_numproc) = WWWM
            blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .true.
         ...
         CASE(IDwp + IDwm + IDzl) !WmWpZ
          if (nparton.eq.2) then
            blha_procsubproc((i-1)*nsp+1,blha_numproc) = WWZ
            blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .false.
          else if (nparton.eq.3) then
            blha_procsubproc((i-1)*nsp+1,blha_numproc) = WWZ
            blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .true.
         ...
         CASE(IDwp + 2*IDzl) !WpZZ
          if (nparton.eq.2) then
            blha_procsubproc((i-1)*nsp+1,blha_numproc) = ZZWP
            blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .false.
          else if (nparton.eq.3) then
            blha_procsubproc((i-1)*nsp+1,blha_numproc) = ZZWP
            blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .true.
         ...
         CASE(IDwm + 2*IDzl) !WmZZ
          if (nparton.eq.2) then
            blha_procsubproc((i-1)*nsp+1,blha_numproc) = ZZWM
            blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .false.
          else if (nparton.eq.3) then
            blha_procsubproc((i-1)*nsp+1,blha_numproc) = ZZWM
            blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .true.
         ...
         CASE(IDwp + IDgamma + IDzl) !WpZA
          if (nparton.eq.2) then
            blha_procsubproc((i-1)*nsp+1,blha_numproc) = WPZA
            blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .false.
          else if (nparton.eq.3) then
            blha_procsubproc((i-1)*nsp+1,blha_numproc) = WPZA
            blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .true.
         ...
         CASE(IDwm + IDgamma + IDzl) !WmZA
          if (nparton.eq.2) then
            blha_procsubproc((i-1)*nsp+1,blha_numproc) = WMZA
            blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .false.
          else if (nparton.eq.3) then
            blha_procsubproc((i-1)*nsp+1,blha_numproc) = WMZA
            blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .true.
         ...
         CASE(IDwm + IDgamma + IDwp) !WpWmA
          if (nparton.eq.2) then
            blha_procsubproc((i-1)*nsp+1,blha_numproc) = WWA
            blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .false.
          else if (nparton.eq.3) then
            blha_procsubproc((i-1)*nsp+1,blha_numproc) = WWA
            blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .true.
         ...
         CASE(IDzl + 2*IDgamma) !ZAA
          if (nparton.eq.2) then
            blha_procsubproc((i-1)*nsp+1,blha_numproc) = ZAA
            blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .false.
          else if (nparton.eq.3) then
            blha_procsubproc((i-1)*nsp+1,blha_numproc) = ZAA
            blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .true.
         ...
         CASE(IDzn + 2*IDgamma) !ZAA_nu
          if (nparton.eq.2) then
            blha_procsubproc((i-1)*nsp+1,blha_numproc) = ZnAA
            blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .false.
          else if (nparton.eq.3) then
            blha_procsubproc((i-1)*nsp+1,blha_numproc) = ZnAA
            blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .true.
        ...
        CASE(2*IDzl + IDgamma) !ZZA
          if (nparton.eq.2) then
            blha_procsubproc((i-1)*nsp+1,blha_numproc) = ZZA
            blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .false.
          else if (nparton.eq.3) then
            blha_procsubproc((i-1)*nsp+1,blha_numproc) = ZZA
            blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .true.
        ...
        CASE(3*IDgamma) !AAA
          if(lQCDlepdec) then
            if (nparton.eq.2) then
              blha_procsubproc((i-1)*nsp+1,blha_numproc) = AAA
              blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .false.
            else if (nparton.eq.3) then
              blha_procsubproc((i-1)*nsp+1,blha_numproc) = AAA
              blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .true.
        ...
        CASE(3*IDzl) !ZZZ
          if (lQCDlepdec) then
            if (nparton.eq.2) then
              blha_procsubproc((i-1)*nsp+1,blha_numproc) = ZZZ
              blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .false.
            else if (nparton.eq.3) then
              blha_procsubproc((i-1)*nsp+1,blha_numproc) = ZZZ
              blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .true.
        ...
        CASE(IDwp + 2*IDgamma) !WPAA
          if (lQCDlepdec) then
            if (nparton.eq.2) then
              blha_procsubproc((i-1)*nsp+1,blha_numproc) = WpAA
              blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .false.
            else if (nparton.eq.3) then
              blha_procsubproc((i-1)*nsp+1,blha_numproc) = WpAAj
              blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .false.
            end if
          else
            blha_numsubproc(blha_numproc) = -1
        ...
        CASE(IDwm + 2*IDgamma) !WmAA
          if (lQCDlepdec) then
            if (nparton.eq.2) then
              blha_procsubproc((i-1)*nsp+1,blha_numproc) = WmAA
              blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .false.
            else if (nparton.eq.3) then
              blha_procsubproc((i-1)*nsp+1,blha_numproc) = WmAAj
              blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .false.
            end if
          else
            blha_numsubproc(blha_numproc) = -1
        ...
        CASE(IDwp + IDgamma) ! W+ A
          ...
          blha_procsubproc((i-1)*nsp+1,blha_numproc) = WpAjj
            ...
            if (nparton.eq.2) then
              blha_procsubproc((i-1)*nsp+1,blha_numproc) = WPA
              blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .false.
            else if (nparton.eq.3) then
              blha_procsubproc((i-1)*nsp+1,blha_numproc) = WPAj
              blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .false.
            else if (nparton.eq.4) then
        ...
        CASE(IDwm + IDgamma) ! W- A
          if (lVBFlepdec) then
            ...
            if (nparton.eq.2) then
              blha_procsubproc((i-1)*nsp+1,blha_numproc) = WMA
              blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .false.
            else if (nparton.eq.3) then
              blha_procsubproc((i-1)*nsp+1,blha_numproc) = WMAj
              blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .false.
            else if (nparton.eq.4) then
        ...
        CASE(IDzl + IDgamma) ! Z_l A
          ...
          blha_procsubproc((i-1)*nsp+1,blha_numproc) = ZAjj
            ...
            if (nparton.eq.2) then
              blha_procsubproc((i-1)*nsp+1,blha_numproc) = ZA
              blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .false.
            else if (nparton.eq.3) then
              blha_procsubproc((i-1)*nsp+1,blha_numproc) = ZA
              blha_lojsubproc((i-1)*nsp+1,blha_numproc) = .true.
