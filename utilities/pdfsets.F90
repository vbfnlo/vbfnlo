...
!      call loadfile("pdfs.dat")
   ...
   if (lglobalprint.and.first_time) write(*, *) "            Information on PDF sets            "
   if (lglobalprint.and.first_time) write(*, *) "-----------------------------------------------"
   if (lglobalprint.and.first_time) then
      select case(pdflib)
       case(1)
         if (lglobalprint.and.first_time) then
            write(*,*) "          Using LHAPDF            "
            write(*,*) " "
            write(*,*) "  Please make sure to cite the following paper if you use LHAPDF6:"
            write(*,*) "  LHAPDF6: parton density access in the LHC precision era"
            write(*,*) "  Eur.Phys.J. C75 (2015) 3, 132"
            write(*,*) "  http://arxiv.org/abs/1412.7420"
            write(*,*) " "
         endif
      end select
   endif
   SELECT CASE(pdflib)
    ...
    CASE(1)
      ...
      if (lglobalprint.and.first_time) write(*,*)'  LO_PDFNAME            = ', trim(pdfname1)
      if (lglobalprint.and.first_time) write(*,*)'  NLO_PDFNAME           = ', trim(pdfname2)
#else
    CASE(1)
...
! Case(3) is hard-wired mstw2008
   ...
   SELECT CASE(pdflib)
    CASE(0)
      if (value .eq. 18) then
         if (lglobalprint.and.first_time) then
            write(*,*)'Initializing PDF set CT18'
         endif
         if (nlo.eq.0) then
            call SetCT18(0)
         else
         ...
         write(*,*) "Error: Unknown type of PDFs (value = ", value, ") in pdfsets.F"
         stop
      endif
#ifdef WITH_LHA
    CASE(1)
...
! Determine how far up we actually go
   SELECT CASE(procID)
    CASE(Hjj, Hjj_AA, Hjj_mu, Hjj_tau, Hjj_bbar, &
       ...
       Zjj_l, Zjj_nu, Ajj, &
       WPWMjj, ZZjj_ll, ZZjj_lnu, ZAjj, ZAjj_n, AAjj, HHjj, EWAAjj, &
