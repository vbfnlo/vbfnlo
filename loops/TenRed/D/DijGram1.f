      subroutine ten_red4_Gram1(p1sq,p2sq,p3sq,p4sq,s12,s23,musq,C0234,C0134,C0124,C0123,D0,Dij,order)
c ************************************************************************************
...
c***********************************************************************************
      ...
      Real*8 p1sqsq,p2sqsq,p3sqsq,p4sqsq,s23sq
      Real*8 r10,r21,r32
      ...
      Integer k,l,jj
      Real*8 F(7),det4,F1
      Real*8 Inv12,Inv16,Inv24,Inv48,Inv240cte
      Real*8 Inv20,Inv30,Inv40,Inv60,Inv80,Inv120
      Real*8 Inv111,Inv222,Inv333,Inv211,Inv311,Inv221
      Real*8 Inv331,Inv322,Inv332,Inv321,Inv0000001,Inv0000002,Inv0000003
      Real*8 InvD480,Inv6033,Inv6032,Inv6031,Inv6022,Inv6021,Inv6011
      Real*8 Inv401111,Inv402111,Inv403111,Inv402211,Inv403211,Inv403311
      Real*8 Inv402221,Inv403221,Inv403321,Inv403331,Inv402222
      Real*8 Inv403222,Inv403322,Inv403332,Inv403333
           include 'DijGram.inc'
      ...
      Save/DDecide/
      Common/Dinit/jjinit1,kinit,linit
      real*8 tempCjj,tempCkl,IXC,ZMaxC,ratio
      Integer jjinitC,jjinit1
...
c      print*, 'ratio1',ratio
      if(ratio.lt.1d-2) then
        call MyCgetGram(p1sq,p2sq,s12,musq,C0123R,C0123I,Cij123)
        C0123=DCMPLX(C0123R,C0123I)
c        print*, "tempCkl",tempCkl,"tempCjj",tempCjj,IXC,ZMaxC,jjinitC
...
c         call MyCget_QUAD_D1(p1sq,p2sq,s12,musq,C0123R,C0123I,Cij123)
         call MyCget(p1sq,p2sq,s12,musq,C0123R,C0123I,Cij123)
c         C0123=DCMPLX(C0123R,C0123I)
      endif
c#########################
...
c      print*, 'ratio2',ratio
      ...
      if(ratio.lt.1d-2) then
         call MyCgetGram(p1sq,s23,p4sq,musq,C0124R,C0124I,Cij124)
...
c          call MyCget_QUAD_D1(p1sq,s23,p4sq,musq,C0124R,C0124I,Cij124)
          call MyCget(p1sq,s23,p4sq,musq,C0124R,C0124I,Cij124)
c          C0124=DCMPLX(C0124R,C0124I)
      endif
c#########################
...
c      print*, 'ratio3',ratio
      if(ratio.lt.1d-2) then
c        print*, "tempCkl",tempCkl,"tempCjj",tempCjj,IXC,ZMaxC,jjinitC
            call MyCgetGram(s12,p3sq,p4sq,musq,C0134R,C0134I,Cij134)
...
c         call MyCget_QUAD_D1(s12,p3sq,p4sq,musq,C0134R,C0134I,Cij134)
         call MyCget(s12,p3sq,p4sq,musq,C0134R,C0134I,Cij134)
c         C0134=DCMPLX(C0134R,C0134I)
       endif
c#########################
...
c      print*, 'ratio4',ratio
      if(ratio.lt.1d-2) then
         call MyCgetGram(p2sq,p3sq,s23,musq,C0234R,C0234I,Cij234)
c       print*, "tempCkl",tempCkl,"tempCjj",tempCjj,IXC,ZMaxC,jjinitC
...
c         call MyCget_QUAD_D1(p2sq,p3sq,s23,musq,C0234R,C0234I,Cij234)
         call MyCget(p2sq,p3sq,s23,musq,C0234R,C0234I,Cij234)
c         C0234=DCMPLX(C0234R,C0234I)
       endif  
c#########################
c       Print*, 'HERE'
c       return
     ...
     &  12-s23)
      ...
      r21=s12-p1sq
      r32=p4sq-s12
       TX1=2*p1sqsq*p3sq+p2sqsq*p4sq-p2sq*(p3sq*p4sq-2*p4sqsq+s12*s23+p4
     &  sq*(2*s12+s23))+s12*(p3sq*(2*p4sq-s23)+s23*(-2*p4sq+2*s12+s23))-
     &  p1sq*(-p3sqsq+p2sq*(p3sq+2*p4sq-2*s12)+2*(-p4sq+s12)*s23+p3sq*(2
     &  *p4sq+2*s12+s23))
       TX2=2*p1sqsq*p3sq+p2sqsq*p4sq+s12*s23*(-p4sq+s12+s23)+p2sq*(p4sqs
     &  q-s12*s23-p4sq*(s12+s23))-p1sq*(p2sq*(p3sq+2*p4sq-2*s12)+2*(-p4s
     &  q+s12)*s23+p3sq*(p4sq+s12+s23))
       TX3=p1sqsq*p3sq+(p2sq-s12)*(p2sq*p4sq-s12*s23)-p1sq*(p2sq*(p3sq+p
     &  4sq-2*s12)+s12*(p3sq+s23))
...
c       if(jj.eq.2)  IX=1/TX2
c       if(jj.eq.3)  IX=1/TX3
c FC %
c FC %       if((k+l).eq.2) Zmax=Z(1,1)
c FC %       if((k+l).eq.3) Zmax=Z(2,1)
c FC %       if((k*l).eq.4) Zmax=Z(2,2)
c FC %       if((k*l).eq.3) Zmax=Z(3,1)
c FC %       if((k+l).eq.5) Zmax=Z(3,2) 
c FC %      if((k+l).eq.6) Zmax=Z(3,3) 
...
c$$$      print*, 'IZax4', I4Z
       ...
       Inv80=1d0/80d0
       Inv120=1d0/120d0
       Inv240cte=(1d0/240d0)*(p1sq+p2sq+p3sq+p4sq+s12+s23)
       Inv333=1d0/240d0
       Inv332=1d0/180d0
       Inv331=1d0/144d0
       Inv322=1d0/120d0
       ...
       Inv211=1d0/36d0
       Inv111=1d0/24d0
       Inv0000001=(-4*p1sq-5*p2sq-5*p3sq-4*p4sq-4*s12-5*s23)/2880d0
       Inv0000002=(-2*p1sq-3*p2sq-4*p3sq-3*p4sq-3*s12-3*s23)/2880d0
       Inv0000003=(-p1sq-p2sq-2*p3sq-2*p4sq-s12-2*s23)/2880d0
       InvD480=(s12*s23 + 2*(p1sq**2 + p2sq**2 + p3sq**2 + p3sq*p4sq + p4sq**2 + p3sq*s12 +
     &       p4sq*s12 + s12**2 + p3sq*s23 + p4sq*s23 + s23**2) +
     &    p2sq*(p4sq + 2*(p3sq + s12 + s23)) + p1sq*(p3sq + 2*(p2sq + p4sq + s12 + s23)))/
     &  20160d0
      Inv6033=(-p1sq - p2sq - s12 - 3*(p3sq + p4sq + s23))/5040d0
      Inv6032=(-3*p1sq - 10*p3sq - 4*(p2sq + s12) - 8*(p4sq + s23))/10080d0
      Inv6031=(-6*p2sq - 10*p4sq - 5*(p1sq + s12) - 12*(p3sq + s23))/10080d0
      Inv6022=(-3*p1sq - 10*p3sq - 6*(p2sq + p4sq + s12 + s23))/5040d0
      Inv6021=(-10*p1sq - 3*(8*p3sq + 5*(p4sq + s12) + 6*(p2sq + s23)))/10080d0
      Inv6011=(-2*(p1sq + p4sq + s12) - 3*(p2sq + p3sq + s23))/1008d0
      Inv401111=1d0/14d0
      Inv402111=1d0/21d0
      Inv403111=1d0/42d0
      Inv402211=1d0/28d0
      ...
      Inv403332=1d0/168d0
      Inv403333=1d0/210d0
      Inv801=(-5*(p1sq**2+p4sq**2+p4sq*s12+s12**2)-3*s12*s23-6*(p3sq*(p
     &  4sq+s12)+p4sq*s23)-7*(p2sq**2+p3sq**2+p3sq*s23+s23**2)-p1sq*(3*p
     &  3sq+5*(p4sq+s12)+6*(p2sq+s23))-p2sq*(3*p4sq+6*s12+7*(p3sq+s23)))
     &  /80640.0d0
      Inv802=(-6*p3sq**2-5*p3sq*(p4sq+s12+s23)-2*(p1sq**2+s12*s23)-4*(p
     &  2sq**2+p4sq**2+s12**2+s23**2+p4sq*(s12+s23))-p1sq*(2*p3sq+3*(p2s
     &  q+p4sq+s12+s23))-p2sq*(5*p3sq+2*(p4sq+2*(s12+s23))))/80640.d0
      Inv803=(-p1sq**2-p2sq**2-2*(p3sq+p4sq)*s12-s12**2-s12*s23-p2sq*(p
     &  4sq+s12+2*(p3sq+s23))-p1sq*(p2sq+p3sq+s12+2*(p4sq+s23))-3*(p3sq*
     &  *2+p4sq**2+p4sq*s23+s23**2+p3sq*(p4sq+s23)))/80640.d0
      Inv60111=(4*(p1sq+p4sq+s12)+7*(p2sq+p3sq+s23))/2688.d0
      Inv60211=(10*p1sq+28*p3sq+15*(p4sq+s12)+21*(p2sq+s23))/13440.d0
      Inv60221=(9*p1sq+35*p3sq+18*(p4sq+s12)+21*(p2sq+s23))/20160.d0
      Inv60222=(2*p1sq+5*(p2sq+2*p3sq+p4sq+s12+s23))/6720.d0
      Inv60311=(7*p2sq+10*p4sq+5*(p1sq+s12)+14*(p3sq+s23))/13440.d0
      Inv60321=(9*p1sq+14*p2sq+35*p3sq+24*p4sq+12*s12+28*s23)/40320.d0
      Inv60322=(3*p1sq+5*(p2sq+3*p3sq+s12+2*(p4sq+s23)))/20160.d0
      Inv60331=(7*p2sq+18*p4sq+6*(p1sq+s12)+21*(p3sq+s23))/40320.d0
      Inv60332=(4*p1sq+18*p3sq+5*(p2sq+s12)+15*(p4sq+s23))/40320.d0
      Inv60333=(p1sq+p2sq+s12+4*(p3sq+p4sq+s23))/13440.d0
      Inv4011111=-1d0/16d0
      Inv4021111=-1d0/24d0
      Inv4022111=-1d0/32d0
      Inv4022211=-1d0/40d0
 ...
 100  I4Z=1d0/(ZMax*4d0)
     ...
     &      +r10*r32*ZZ(k,3,l,1)+r21*r32*ZZ(k,3,l,2)+r32**2*ZZ(k,3,l,3)
     ...
     3    Abs(p2sq*(p2sq + p4sq - s12 - s23)**2))
       ...
       Print*, 'l',l
       print*, "T1",TX1
       print*, "T1",TX1/(    2*Abs(p1sqsq*p3sq) + 
     -  Abs(p1sq*p2sq*p3sq) + 
     -  Abs(p1sq*p3sqsq) + 
     -  2*Abs(p1sq*p2sq*p4sq) + 
     -  Abs(p2sqsq*p4sq) + 
     -  2*Abs(p1sq*p3sq*p4sq) + 
     -  Abs(p2sq*p3sq*p4sq) + 
     -  2*Abs(p2sq*p4sqsq) + 
     -  2*Abs(p1sq*p2sq*s12) + 
     -  2*Abs(p1sq*p3sq*s12) + 
     ...
     -  Abs(p3sq*s12*s23) + 
     -  2*Abs(p4sq*s12*s23) + 
     -  2*Abs(s12**2*s23) + Abs(s12*s23**2))

       print*, "T2",TX2
       print*, "T2ABS",TX2/(   2*Abs(p1sqsq*p3sq) + 
     -  Abs(p1sq*p2sq*p3sq) + 
     -  2*Abs(p1sq*p2sq*p4sq) + 
     -  Abs(p2sqsq*p4sq) + 
     -  Abs(p1sq*p3sq*p4sq) + 
     ...
     -  2*Abs(p1sq*s12*s23) + 
     -  Abs(p2sq*s12*s23) + 
     -  Abs(p4sq*s12*s23) + Abs(s12**2*s23) + 
     -  Abs(s12*s23**2)) 
       print*, "T3",TX3
       print*, "T3",TX3/(  Abs(p1sqsq*p3sq) + 
     -  Abs(p1sq*p2sq*p3sq) + 
     -  Abs(p1sq*p2sq*p4sq) + 
     -  Abs(p2sq**2*p4sq) + 
     -  2*Abs(p1sq*p2sq*s12) + 
     ...
     -  Abs(p2sq*p4sq*s12) + 
     -  Abs(p1sq*s12*s23) + 
     -  Abs(p2sq*s12*s23) + Abs(s12**2*s23))
...
c                Step2
       ...
       if(printmy) then
       print*, "tempD40 1",tempD40,order,k,l,jj
       endif
...
c FC %
     ...
     &      +r10*r32*ZZ(ktemp,3,ltemp,1)+r21*r32*ZZ(ktemp,3,ltemp,2)+r32**2*ZZ(ktemp,3,ltemp,3)
                      Zmaxtemp=Z(1,1)
...
c       Print*,'tempjj=',tempjj
c       Print*,'tempkl=',tempkl
c FC %
...
!       endif    
       IF(abs(IX*det4).ge.1.0d0) then! .and.abs(det4/det4Abs).gt.1.5d-2) then
          if(printmy) then
          print*, "CAUTION SMALL CAYLEY,IX*det4 in D", abs(IX*det4)
          endif
       !return
       endif
c       Print*,"IX*det4", IX*det4
c FC %       print*, 'k',k
c FC %       print*, 'l',l
c FC %       print*, 'jj',jj
c FC %       print*, 'cont',cont
...
c                Step1
     ...
     &  (k)*Z(1,l)+S3h0022(k)*Z(2,l)+S3h0023(k)*Z(3,l)+(2*Inv12+S30000)*
     &  Z(k,l)-S3h0021(1)*Z(k,l)-S3h0022(2)*Z(k,l)-S3h0023(3)*Z(k,l)
...
c                Step1
     ...
     &  h00311(k)*Z(1,l)+S3h00321(k)*Z(2,l)+S3h00331(k)*Z(3,l)+S300001(1
     &  )*Z(k,l)-S3h00311(1)*Z(k,l)-S3h00322(1)*Z(k,l)-S3h00333(1)*Z(k,l
     &  )-2*(Inv16*Z(k,l)+S3h00001(1)*ZZ(k,1,l,1))-2*S3h00001(2)*ZZ(k,1,
     &  l,2)-2*S3h00001(3)*ZZ(k,1,l,3)
       aux00001(2)=-(F(1)*S3h0021(2))-F(2)*S3h0022(2)-F(3)*S3h0023(2)+S3
     &  h00312(k)*Z(1,l)+S3h00322(k)*Z(2,l)+S3h00332(k)*Z(3,l)+S300001(2
     &  )*Z(k,l)-S3h00312(1)*Z(k,l)-S3h00322(2)*Z(k,l)-S3h00333(2)*Z(k,l
     &  )-2*(Inv24*Z(k,l)+S3h00001(1)*ZZ(k,2,l,1))-2*S3h00001(2)*ZZ(k,2,
     &  l,2)-2*S3h00001(3)*ZZ(k,2,l,3)
       aux00001(3)=-(F(1)*S3h0021(3))-F(2)*S3h0022(3)-F(3)*S3h0023(3)+S3
     &  h00313(k)*Z(1,l)+S3h00323(k)*Z(2,l)+S3h00333(k)*Z(3,l)+S300001(3
     &  )*Z(k,l)-S3h00313(1)*Z(k,l)-S3h00323(2)*Z(k,l)-S3h00333(3)*Z(k,l
     &  )-2*(Inv48*Z(k,l)+S3h00001(1)*ZZ(k,3,l,1))-2*S3h00001(2)*ZZ(k,3,
     &  l,2)-2*S3h00001(3)*ZZ(k,3,l,3)
...
c                Step1
     ...
     &  +S3h000021(k)*Z(1,l)+S3h000022(k)*Z(2,l)+S3h000023(k)*Z(3,l)+(-I
     &  nv240cte+S3000000)*Z(k,l)-S3h000021(1)*Z(k,l)-S3h000022(2)*Z(k,l
     &  )-S3h000023(3)*Z(k,l)
     ...
     &  2*Inv20+S3000021(1)-S3h004111(1)-S3h004221(1)-S3h004331(1))*Z(k,
     ...
     &  2*Inv30+S3000022(1)-S3h004121(1)-S3h004222(1)-S3h004332(1))*Z(k,
     ...
     &  2*Inv40+S3000022(2)-S3h004122(1)-S3h004222(2)-S3h004332(2))*Z(k,
     ...
     &  2*Inv60+S3000023(1)-S3h004131(1)-S3h004232(1)-S3h004333(1))*Z(k,
     ...
     &  2*Inv80+S3000023(2)-S3h004132(1)-S3h004232(2)-S3h004333(2))*Z(k,
     ...
     &  2*Inv120+S3000023(3)-S3h004133(1)-S3h004233(2)-S3h004333(3))*Z(k
     &  ,l)-4*S3h000021(3)*ZZ(k,3,l,1)-4*S3h000022(3)*ZZ(k,3,l,2)-4*S3h0
     &  00023(3)*ZZ(k,3,l,3)
...
c                Step1
     ...
     &  3,l)+(-2*Inv0000001+S30000001(1)-S3h0000311(1)-S3h0000322(1)-S3h
     &  0000333(1))*Z(k,l)-2*S3h0000001(1)*ZZ(k,1,l,1)-2*S3h0000001(2)*Z
     &  Z(k,1,l,2)-2*S3h0000001(3)*ZZ(k,1,l,3)
       aux0000001(2)=-(F(1)*S3h000021(2))-F(2)*S3h000022(2)-F(3)*S3h0000
     &  23(2)+S3h0000312(k)*Z(1,l)+S3h0000322(k)*Z(2,l)+S3h0000332(k)*Z(
     &  3,l)+(-2*Inv0000002+S30000001(2)-S3h0000312(1)-S3h0000322(2)-S3h
     &  0000333(2))*Z(k,l)-2*S3h0000001(1)*ZZ(k,2,l,1)-2*S3h0000001(2)*Z
     &  Z(k,2,l,2)-2*S3h0000001(3)*ZZ(k,2,l,3)
       aux0000001(3)=-(F(1)*S3h000021(3))-F(2)*S3h000022(3)-F(3)*S3h0000
     &  23(3)+S3h0000313(k)*Z(1,l)+S3h0000323(k)*Z(2,l)+S3h0000333(k)*Z(
     &  3,l)+(-2*Inv0000003+S30000001(3)-S3h0000313(1)-S3h0000323(2)-S3h
     &  0000333(3))*Z(k,l)-2*S3h0000001(1)*ZZ(k,3,l,1)-2*S3h0000001(2)*Z
     &  Z(k,3,l,2)-2*S3h0000001(3)*ZZ(k,3,l,3)
     ...
     &  Z(3,l)+(-2*Inv111+S30000311(1)-S3h0051111(1)-S3h0052211(1)-S3h00
     &  53311(1))*Z(k,l)-6*S3h0000311(1)*ZZ(k,1,l,1)-6*S3h0000321(1)*ZZ(
     &  k,1,l,2)-6*S3h0000331(1)*ZZ(k,1,l,3)
       aux00003(2,1,1)=-(F(1)*S3h004121(1))-F(2)*S3h004221(1)-F(3)*S3h00
     &  4321(1)+S3h0051211(k)*Z(1,l)+S3h0052211(k)*Z(2,l)+S3h0053211(k)*
     &  Z(3,l)+(-2*Inv211+S30000321(1)-S3h0051211(1)-S3h0052221(1)-S3h00
     &  53321(1))*Z(k,l)-4*S3h0000312(1)*ZZ(k,1,l,1)-4*S3h0000322(1)*ZZ(
     &  k,1,l,2)-4*S3h0000332(1)*ZZ(k,1,l,3)-2*S3h0000311(1)*ZZ(k,2,l,1)
     ...
     &  Z(3,l)+(-2*Inv221+S30000322(1)-S3h0051221(1)-S3h0052222(1)-S3h00
     &  53322(1))*Z(k,l)-2*S3h0000312(2)*ZZ(k,1,l,1)-2*S3h0000322(2)*ZZ(
     &  k,1,l,2)-2*S3h0000332(2)*ZZ(k,1,l,3)-4*S3h0000312(1)*ZZ(k,2,l,1)
     ...
     &  Z(3,l)+(-2*Inv222+S30000322(2)-S3h0051222(1)-S3h0052222(2)-S3h00
     &  53322(2))*Z(k,l)-6*S3h0000312(2)*ZZ(k,2,l,1)-6*S3h0000322(2)*ZZ(
     &  k,2,l,2)-6*S3h0000332(2)*ZZ(k,2,l,3)
       aux00003(3,1,1)=-(F(1)*S3h004131(1))-F(2)*S3h004231(1)-F(3)*S3h00
     &  4331(1)+S3h0051311(k)*Z(1,l)+S3h0052311(k)*Z(2,l)+S3h0053311(k)*
     &  Z(3,l)+(-2*Inv311+S30000331(1)-S3h0051311(1)-S3h0052321(1)-S3h00
     &  53331(1))*Z(k,l)-4*S3h0000313(1)*ZZ(k,1,l,1)-4*S3h0000323(1)*ZZ(
     &  k,1,l,2)-4*S3h0000333(1)*ZZ(k,1,l,3)-2*S3h0000311(1)*ZZ(k,3,l,1)
     ...
     &  Z(3,l)+(-2*Inv321+S30000332(1)-S3h0051321(1)-S3h0052322(1)-S3h00
     &  53332(1))*Z(k,l)-2*(S3h0000313(2)*ZZ(k,1,l,1)+S3h0000323(2)*ZZ(k
     &  ,1,l,2)+S3h0000333(2)*ZZ(k,1,l,3)+S3h0000313(1)*ZZ(k,2,l,1)+S3h0
     &  000323(1)*ZZ(k,2,l,2)+S3h0000333(1)*ZZ(k,2,l,3)+S3h0000312(1)*ZZ
     ...
     &  Z(3,l)+(-2*Inv322+S30000332(2)-S3h0051322(1)-S3h0052322(2)-S3h00
     &  53332(2))*Z(k,l)-4*S3h0000313(2)*ZZ(k,2,l,1)-4*S3h0000323(2)*ZZ(
     &  k,2,l,2)-4*S3h0000333(2)*ZZ(k,2,l,3)-2*S3h0000312(2)*ZZ(k,3,l,1)
     ...
     &  Z(3,l)+(-2*Inv331+S30000333(1)-S3h0051331(1)-S3h0052332(1)-S3h00
     &  53333(1))*Z(k,l)-2*S3h0000313(3)*ZZ(k,1,l,1)-2*S3h0000323(3)*ZZ(
     &  k,1,l,2)-2*S3h0000333(3)*ZZ(k,1,l,3)-4*S3h0000313(1)*ZZ(k,3,l,1)
     ...
     &  Z(3,l)+(-2*Inv332+S30000333(2)-S3h0051332(1)-S3h0052332(2)-S3h00
     &  53333(2))*Z(k,l)-2*S3h0000313(3)*ZZ(k,2,l,1)-2*S3h0000323(3)*ZZ(
     &  k,2,l,2)-2*S3h0000333(3)*ZZ(k,2,l,3)-4*S3h0000313(2)*ZZ(k,3,l,1)
     ...
     &  Z(3,l)+(-2*Inv333+S30000333(3)-S3h0051333(1)-S3h0052333(2)-S3h00
     &  53333(3))*Z(k,l)-6*S3h0000313(3)*ZZ(k,3,l,1)-6*S3h0000323(3)*ZZ(
     &  k,3,l,2)-6*S3h0000333(3)*ZZ(k,3,l,3)
...
c                Step7
      ...
      DO I1=0,4
           accuracyD(i1,ac)=accuracyDR(1,i1,ac)
            DO I2=2,INDEX(I1)  
c         print*, "accuracyD(i1,ac)",accuracyD(i1,ac),Dij(i2,i1)
       if(accuracyDR(i2,i1,ac).gt.accuracyD(i1,ac)) then
          accuracyD(i1,ac)=accuracyDR(i2,i1,ac)
c          print*, "accuracyD(i1,ac)",accuracyD(i1,ac)
       endif
          enddo
        enddo
...
c                Step1
     ...
     &  (k)*Z(3,l)+(InvD480+S300000000)*Z(k,l)-S3h00000021(1)*Z(k,l)-S3h
     &  00000022(2)*Z(k,l)-S3h00000023(3)*Z(k,l)
...
c                Step1
     ...
     &  2*S3h000000001(2)*ZZ(k,3,l,2)-2*S3h000000001(3)*ZZ(k,3,l,3)
       temp000000001(1)=I20Z*(aux000000001(1)+r10*(4*tempD400000000+r10*
     &  temp0000001(1))*ZZ(k,1,l,1)+r21*(2*tempD400000000+r10*temp000000
     &  1(1))*ZZ(k,1,l,2)+r10*r32*temp0000001(1)*ZZ(k,1,l,3)+r10*r21*tem
     &  p0000001(1)*ZZ(k,2,l,1)+2*(r32*tempD400000000*ZZ(k,1,l,3)+r21*te
     &  mpD400000000*ZZ(k,2,l,1))+r21**2*temp0000001(1)*ZZ(k,2,l,2)+r21*
     &  r32*temp0000001(1)*ZZ(k,2,l,3)+2*r32*tempD400000000*ZZ(k,3,l,1)+
     &  r10*r32*temp0000001(1)*ZZ(k,3,l,1)+r21*r32*temp0000001(1)*ZZ(k,3
     &  ,l,2)+r32**2*temp0000001(1)*ZZ(k,3,l,3))
       temp000000001(2)=I20Z*(aux000000001(2)+r10**2*temp0000001(2)*ZZ(k
     &  ,1,l,1)+r10*(2*tempD400000000+r21*temp0000001(2))*ZZ(k,1,l,2)+r1
     &  0*r32*temp0000001(2)*ZZ(k,1,l,3)+r10*r21*temp0000001(2)*ZZ(k,2,l
     &  ,1)+4*r21*tempD400000000*ZZ(k,2,l,2)+r21**2*temp0000001(2)*ZZ(k,
     &  2,l,2)+r21*r32*temp0000001(2)*ZZ(k,2,l,3)+2*(r10*tempD400000000*
     &  ZZ(k,2,l,1)+r32*tempD400000000*ZZ(k,2,l,3))+r10*r32*temp0000001(
     &  2)*ZZ(k,3,l,1)+2*r32*tempD400000000*ZZ(k,3,l,2)+r21*r32*temp0000
     &  001(2)*ZZ(k,3,l,2)+r32**2*temp0000001(2)*ZZ(k,3,l,3))
       temp000000001(3)=I20Z*(aux000000001(3)+r10**2*temp0000001(3)*ZZ(k
     &  ,1,l,1)+r10*r21*temp0000001(3)*ZZ(k,1,l,2)+r10*r32*temp0000001(3
     &  )*ZZ(k,1,l,3)+r10*r21*temp0000001(3)*ZZ(k,2,l,1)+r21**2*temp0000
     &  001(3)*ZZ(k,2,l,2)+r21*r32*temp0000001(3)*ZZ(k,2,l,3)+2*(r10*tem
     &  pD400000000*ZZ(k,1,l,3)+r21*tempD400000000*ZZ(k,2,l,3))+2*r10*te
     &  mpD400000000*ZZ(k,3,l,1)+r10*r32*temp0000001(3)*ZZ(k,3,l,1)+2*r2
     &  1*tempD400000000*ZZ(k,3,l,2)+r21*r32*temp0000001(3)*ZZ(k,3,l,2)+
     &  r32*(4*tempD400000000+r32*temp0000001(3))*ZZ(k,3,l,3))
...
c FC %      PRINT*, 'temp0072111(1,1,1)',temp0072111(1,1,1)
...
c                Step9
      ...
      enddo
cFC         D0=tempD40
cFC c        Dij(1,1)=temp1(1)
cFC c        Dij(2,1)=temp1(2)
cFC c        Dij(3,1)=temp1(3)
cFC c        Dij(1,2)=temp2(1,1)
cFC c        Dij(2,2)=temp2(2,2)
cFC c        Dij(3,2)=temp2(3,3)
cFC c        Dij(4,2)=temp2(2,1)
cFC c        Dij(5,2)=temp2(3,1)
cFC c        Dij(6,2)=temp2(3,2)
cFC c        Dij(7,2)=tempD400
cFC c        Dij(1,3)=temp3(1,1,1)
cFC c        Dij(2,3)=temp3(2,2,2)
cFC c        Dij(3,3)=temp3(3,3,3)
cFC c        Dij(4,3)=temp3(2,1,1)
cFC c        Dij(5,3)=temp3(3,1,1)
cFC c        Dij(6,3)=temp3(2,2,1)
cFC c        Dij(7,3)=temp3(3,3,1)
cFC c        Dij(8,3)=temp3(3,2,2)
cFC c        Dij(9,3)=temp3(3,3,2)
cFC c        Dij(10,3)=temp3(3,2,1)
cFC c        Dij(11,3)=temp001(1)
cFC c        Dij(12,3)=temp001(2)
cFC c        Dij(13,3)=temp001(3)
cFC c        Dij(7,1)=temp002(1,1)
cFC c        Dij(8,1)=temp002(2,2)
cFC c        Dij(9,1)=temp002(3,3)
cFC c        Dij(10,1)=temp002(2,1)
cFC c        Dij(11,1)=temp002(3,1)
cFC c        Dij(12,1)=temp002(3,2)
cFC c        Dij(13,1)=tempD40000
cFC c        Dij(1,4)=temp41(1,1,1)
cFC c         Dij(2,4)=temp42(2,2,2)
cFC c         Dij(3,4)=temp43(3,3,3)
cFC c         Dij(4,4)=temp42(1,1,1)
cFC c         Dij(5,4)=temp43(1,1,1)
cFC c         Dij(6,4)=temp42(2,1,1)
cFC c         Dij(7,4)=temp43(2,1,1)
cFC c         Dij(8,4)=temp43(3,1,1)
cFC c         Dij(9,4)=temp42(2,2,1)
cFC c         Dij(10,4)=temp43(2,2,1)
cFC c         Dij(11,4)=temp43(3,2,1)
cFC c         Dij(12,4)=temp43(3,3,1)
cFC c         Dij(13,4)=temp43(2,2,2)
cFC c         Dij(14,4)=temp43(3,2,2)
cFC c         Dij(15,4)=temp43(3,3,2)
cFC c         Dij(16,4)=temp002(1,1)
cFC c         Dij(17,4)=temp002(2,2)
cFC c         Dij(18,4)=temp002(3,3)
cFC c         Dij(19,4)=temp002(2,1)
cFC c         Dij(20,4)=temp002(3,1)
cFC c         Dij(21,4)=temp002(3,2)
cFC c         Dij(22,4)=tempD40000
cFC


c FC %       PRINT*, '2_temp0072111(1,1,1)',temp0072111(1,1,1)
 500    D0=tempD40
        Dij(1,1)=temp1(1)
        Dij(2,1)=temp1(2)
        Dij(3,1)=temp1(3)
