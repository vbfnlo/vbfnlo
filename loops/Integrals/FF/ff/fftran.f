*###[ vbfffai:
	subroutine vbfffai(ai,daiaj,aai,laai,del2s,sdel2s,xpi,dpipj,piDpj,
     +		ier)
***#[*comment:***********************************************************
*									*
...
*  #] declarations: 
*  #[ get ai:
	if ( lwrite ) print *,'vbfffai: xpi = ',(xpi(i),i=1,10),ier
*
...
*
	    ier0 = ier
	    call vbfffroot(aaim(3),aim(3),xpi(4),piDpj(4,3),xpi(3),
     +							sdel2s,ier0)
	    ier2 = max(ier2,ier0)
	    if ( aim(3) .eq. 0 ) then
*		choose the other root
		if ( lwrite ) print *,'vbfffai: 1/A_3 = 0'
		ier = ier + 100
		return
	    endif
	    ai(3) = ai(4)/aim(3)
...
*
	    ier1 = ier
	    ier0 = ier
	    call vbfffdl2s(del2sa,xpi,piDpj, 2,4,10,1, 3,4,7,1, 10,ier0)
	    ier1 = max(ier1,ier0)
	    ier0 = ier
	    call vbfffdl3m(del3mi(2),.FALSE.,x0,x0,xpi,dpipj,piDpj,10,
     +						3,4,7, 2,1,ier0)
	    ier1 = max(ier1,ier0)
	    call vbfffroot(aim(2),aaim(2),xpi(4),piDpj(4,2),del3mi(2)/del2s
     +		,del2sa/sdel2s,ier1)
	    ier2 = max(ier2,ier1)
	    if ( aim(2) .eq. 0 ) then
		if ( lwrite ) print *,'vbfffai: 1/A_2 = 0'
		ier = ier + 100
		return
	    endif
	    ai(2) = ai(4)/aim(2)
...
*
	    ...
	    ier0 = ier
	    ier1 = ier
	    call vbfffdl2s(del2sb,xpi,piDpj, 1,4,8,-1, 3,4,7,1, 10,ier0)
	    ier1 = max(ier1,ier0)
	    ier0 = ier
	    call vbfffdl3m(del3mi(1),.FALSE.,x0,x0,xpi,dpipj,piDpj,10,
     +						3,4,7, 1,1,ier0)
	    ier1 = max(ier1,ier0)
	    call vbfffroot(aim(1),aaim(1),xpi(4),piDpj(4,1),del3mi(1)/del2s
     +		,del2sb/sdel2s,ier1)
	    ier2 = max(ier2,ier1)
	    if ( aim(1) .eq. 0 ) then
		if ( lwrite ) print *,'vbfffai: 1/A_1 = 0'
		ier = ier + 100
		return
	    endif
	    ai(1) = ai(4)/aim(1)
     ...
     +			DBLE(2*xpi(4))
		ca1 = DBLE(ai(4))/ca1m
		if ( lwrite ) print *,'vbfffai: A1 = ',ca1
		ai(1) = ai(4)/DBLE(ca1m)
	    endif
	else
*
...
*  #[		    del2s=0:
		    ...
		    ier1 = ier
		    call vbfffwarn(146,ier1,daiaj(j,i),ai(i))
		    goto 105
*  #]		    del2s=0: 
...
*
*		    as the vertex p1,s4,? does not exist we use vbfffdl2t
*
		    call vbfffdl2t(delps,piDpj, 5,4, 3,4,7,1,+1, 10,ier0)
		    ier1 = max(ier1,ier0)
		    ier0 = ier
		    call vbfffdl3m(del3m,.FALSE.,x0,x0,xpi,dpipj,piDpj,
     +			10, 3,4,7, 5,1, ier0)
		    ier1 = max(ier1,ier0)
		    call vbfffroot(dum,da2a1m,xpi(4),piDpj(4,5),
...
*
		    call vbfffdl2t(delps,piDpj, 9,4, 3,4,7,1,+1, 10,ier0)
		    ier1 = ier0
		    ier0 = ier
		    call vbfffdl3m(del3m,.FALSE.,x0,x0,xpi,dpipj,piDpj,
     +			10, 3,4,7, 9,1, ier0)
		    ier1 = max(ier1,ier0)
		    call vbfffroot(dum,da1a3m,xpi(4),-piDpj(4,9),
     +			del3m(1)/del2s,delps/sdel2s,ier1)
...
*		    Again, the solution of a simple quadratic equation
*
		    call vbfffdl2s(delps,xpi,piDpj,4,1,8,1,3,4,7,1,10,ier0)
		    ier1 = ier0
		    ier0 = ier
		    call vbfffdl3m(del3m,.FALSE.,x0,x0,xpi,dpipj,piDpj,
     +			10, 3,4,7, 8,1, ier0)
		    ier1 = max(ier0,ier1)
		    call vbfffroot(dum,da1a4m,xpi(4),piDpj(4,8),del3m(1)/
...
*		    Again, the solution of a simple quadratic equation
*
		    call vbfffdl2t(delps,piDpj, 6,4, 3,4,7,1,+1, 10,ier0)
		    ier1 = ier0
		    ier0 = ier
		    call vbfffdl3m(del3m,.FALSE.,x0,x0,xpi,dpipj,piDpj,
     +			10, 3,4,7, 6,1, ier0)
		    ier1 = max(ier1,ier0)
		    call vbfffroot(dum,da2a3m,xpi(4),-piDpj(4,6),
     +			del3m(1)/del2s,delps/sdel2s,ier1)
...
*		    Again, the solution of a simple quadratic equation
*
		    call vbfffdl2s(delps,xpi,piDpj,2,4,10,1,3,4,7,1,10,
     +								ier0)
		    ier1 = ier0
		    ier0 = ier
		    call vbfffdl3m(del3m,.FALSE.,x0,x0,xpi,dpipj,piDpj,
     +			10, 3,4,7, 10,1, ier0)
		    ier1 = max(ier0,ier1)
		    call vbfffroot(dum,da2a4m,xpi(4),piDpj(4,10),del3m(1)/
     +			del2s,delps/sdel2s,ier1)
...
*
		    ier1 = ier
		    call vbfffroot(dum,da3a4m,xpi(4),-piDpj(4,7),
     +			xpi(7),sdel2s,ier1)
...
*###[ vbffftran:
	subroutine vbffftran(ai,daiaj,aai,laai,xqi,dqiqj,qiDqj,
...
*
			if ( kj .eq. 5 ) then
			    call vbfffdl2t(delps,piDpj, 7,i, 1,2,5,
     +							1,1,10,ier0)
...
*
			    call vbfffwarn(144,ier1,qiDqj(kj,i),s(1))
...
*
		    ier1 = ier0
		    ier0 = ier
		    call vbfff2dl2(del2d2,delps,xpi,dpipj,piDpj, i,
     +			j,k,kj,is, 4, 3,4,7,+1, 10, ier0)
		    ier1 = max(ier1,ier0)
		    ier0 = ier
		    call vbfff2d22(dl2d22,xpi,dpipj,piDpj, i, j,k,kj,is,
     +			3,4,7,+1, 10,ier0)
		    ier1 = max(ier1,ier0)
		    call vbfffroot(dum,aijk,xpi(4),delps,dl2d22/del2s,
     +			-del2d2/sdel2s,ier1)
*		    the minus sign is because we have aijk, not aikj.
   ...
   90	continue
	if ( ltest ) then
     ...
     +		    abs(qiDqj(i,3)))print *,'vbffftran: error: t3.q',i,
...
*
			if ( del2s .eq. 0 ) then
				...
				call vbfffdl3m(s(1),.FALSE.,x0,x0,xpi,dpipj
     ...
     +				  ')+ =',qiDqj(lk,ji),max(ier2,ier1)
				...
				call vbfffwarn(145,ier1,qiDqj(lk,ji),s(1))
			    endif
			    goto 145
			endif
*
...
*
			call vbfffabcd(aijkl,xpi,dpipj,piDpj,del2s,
     ...
     +				qiDqj(lk,ji),max(ier2,ier1)
			goto 145
*			print *,'vbffftran: warning: numerical problems ',
*     +				'in qiDqj(',lk,ji,')'
  145			continue
			if ( lk .ne. ji ) then
  ...
  180	continue
	...
	if ( ltest ) then
     ...
     +			s(1)),abs(s(2))) ) print *,'vbffftran: error: q',i,
  ...
  810	    continue
     ...
     +			'vbffftran: error: qiDqj(',i,j,')/= qiDqj(',j,i,')'
  820		continue
  830	    continue
	    do 840 i=1,10
     ...
     +			'vbffftran: error: No momentum conservation in ',
...
*		give up
*
		if ( lwarn ) call vbfffwarn(125,ier0,dqiqj(j,i),smax)
...
*		    give up
*
		    if ( lwarn ) call vbfffwarn(126,ier0,dqiqj(kj,i),xmax)
...
*			give up
*
			if ( lwarn ) call vbfffwarn(127,ier0,dqiqj(lk,ji),
     +				smax)
...
*  #[ debug:
	if ( lwrite ) then
	    print *,'vbffftran: transformed momenta'
	    print *,xqi
	    print '(10e16.8)',qiDqj
	    print *,'ier = ',ier
	endif
