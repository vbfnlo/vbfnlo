*###[ vbfffdcc0:
	subroutine vbfffdcc0(cs3,ipi12,isoort,clogi,ilogi,xpi,dpipj,piDpj,
...
*  #[ check input:
	if ( ltest ) then
	    call vbfffchck(xpi(1,3),dpipj(1,1,3),6,ier)
	    call vbfffchck(xpi(1,4),dpipj(1,1,4),6,ier)
	    call vbfffchck(xqi,dqiqj,10,ier)
	    if ( ldel2s ) print *,'vbfffdcc0: error: cannot handle this ',
     +		'case yet!!'
	endif
*  #] check input:
*  #[   get y,z-roots:
	if ( lwrite ) print '(a)','  ##[ get roots: (vbfffdcc0)'
	do 20 k=3,4
	do 10 i=1,3
*
*	get roots (y,z)
...
*	    then handle the special case Si = 0
     ...
     +		     i .eq. 3 .and. alph(1) .eq. 0 ) then
	    ...
	    call vbfffccyz(y(1,k,i),z(1,k,i),dyz(1,1,k,i),d2yzz(k,i),i,
...
*  #[       vertices (2):
     ...
     +			absc(s(5)))/DBLE(xqi(6))**2
		...
		if ( lwarn .and. absc(dyzzy(ii,2)).lt.xloss*xmax ) then
		   call vbfffwarn(142,ieri(2*k+j-2),absc(dyzzy(ii,2)),xmax)
		endif
   21	    continue
*
*	    get dyyzz
*
 ...
 1002		format(a,i1,a,2g22.14,g12.4)
     ...
     +			xmax
		print *,'vbfffdcc0: under construction!'
*
*		(could be copied from real case)
*
		if ( lwarn .and. absc(dyyzz(j,2)).lt.xloss*xmax ) then
		    call vbfffwarn(147,ieri(7+j),absc(dyyzz(j,2)),xmax)
		endif
	    endif
*
*	    bookkeeping
...
*  #[       vertices (3):
     ...
     +			absc(s(5)))/DBLE(xqi(8))**2
		...
		if ( lwarn .and. absc(dyzzy(ii,3)).lt.xloss*xmax ) then
		   call vbfffwarn(142,ieri(2*k+j+2),absc(dyzzy(ii,3)),xmax)
		endif
   25	    continue
*
*	    get dyyzz
...
*
     ...
     +			xmax
		if ( lwarn .and. absc(dyyzz(j,3)).lt.xloss*xmax ) then
		    call vbfffwarn(147,ieri(9+j),absc(dyyzz(j,3)),xmax)
		endif
	    endif
*
*	    bookkeeping
...
*  #[   check differences:
	if ( ltest ) then
     ...
     +			'vbfffdcc0: error: ','dyzzy(',j,i,') <> terms, ',
     ...
     +			'vbfffdcc0: error: ','dyzzy(',j+2,i,') <> terms, ',
...
*  #[ integrals:
	do 100 i=1,3
		...
		if ( lwrite ) then
		    print *,'vbfffdcc0: xk=0, ma=mb/Si-0 -> S3 = 0'
		...
		if ( isoort(j+8) .eq. 0 ) then
		    ...
		    call vbfffcs3(cs3(20*i+61),ipi12(j+8),y(1,4,i),
     ...
     +			xpi(1,4),piDpj(1,1,4),i,6,isoort(j+8),ier)
		...
		call vbfffcs3(cs3(20*i-19),ipi12(j),y(1,3,i),
     +		    z(1,3,i),dyz(1,1,3,i),d2yzz(3,i),
     +		    xpi(1,3),piDpj(1,1,3),i,6,isoort(j),ier)
	    else
		call vbfffdcs(cs3(20*i-19),ipi12(j),y(1,3,i),z(1,3,i),
