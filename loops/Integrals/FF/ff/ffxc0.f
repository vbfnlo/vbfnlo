*	$Id: vbfffxc0.f,v 1.5 1996/08/15 09:36:47 gj Exp $
*###[ vbfffxc0:
	subroutine vbfffxc0(cc0,xpi,ier)
***#[*comment:***********************************************************
*									*
...
*  #[ convert input:
	...
	if ( lwarn ) then
     ...
     +			xpi(i) .ne. xpi(j) ) then
			ier0 = 0
			call vbfffwarn(87,ier0,dpipj(j,i),xpi(i))
...
*  #] convert input:
*  #[ call vbfffxc0a:
	call vbfffxc0a(cc0,xpi,dpipj,ier)
*  #] call vbfffxc0a:
*###] vbfffxc0:
	end
*###[ vbfffxc0a:
	subroutine vbfffxc0a(cc0,xpi,dpipj,ier)
***#[*comment:***********************************************************
*									*
*	See vbfffxc0.							*
*									*
...
*	    I have had too many problems here ...
		...
		print *,'vbfffxc0: error: ab(isgnal) /= 1!',isgnal
		if ( isgnal .eq. 0 ) then
		    isgnal = 1
		else
...
*	    check input dot products if present
	    if ( idot.gt.0 ) then
	    	...
	    	idotsa = idot
	    	idot = 0
	    	call vbfffdot3(qiDqj,xpi,dpipj,6,ier0)
	    	idot = idotsa
     ...
     +			    'vbfffxc0a: error: input dotproduct piDpj(',j,
...
*
	if ( dpipj(2,4).eq.0 .and. dpipj(3,6).eq.0 .and. xpi(1).eq.0
     ...
     +	then
	    call vbfffxc0i(cc0,xpi,dpipj,ier)
	    return
	endif
**
...
*     +		dpipj(1,2) .ne. 0 ) then
*	    call vbfffxb0p(cs1,-xpi(5),xpi(1),xpi(3),dpipj(1,6),dpipj(3,5),
*     +			dpipj(1,3),ier)
*	    call vbfffxb0p(cs2,-xpi(5),xpi(2),xpi(3),dpipj(2,5),dpipj(3,5),
*     +			dpipj(2,3),ier)
*	    cs = cs1 - cs2
*	    cc0 = cs/dpipj(1,2)
*	elseif ( xpi(6) .eq. 0 .and. dpipj(4,5) .eq. 0 .and.
*     +		dpipj(3,1) .ne. 0 ) then
*	    call vbfffxb0p(cs1,-xpi(4),xpi(3),xpi(2),dpipj(3,5),dpipj(2,4),
*     +			dpipj(3,2),ier)
*	    call vbfffxb0p(cs2,-xpi(4),xpi(1),xpi(2),dpipj(1,4),dpipj(2,4),
*     +			dpipj(1,2),ier)
*	    cs = cs1 - cs2
*	    cc0 = cs/dpipj(3,1)
*	elseif ( xpi(5) .eq. 0 .and. dpipj(6,4) .eq. 0 .and.
*     +		dpipj(2,3) .ne. 0 ) then
*	    call vbfffxb0p(cs1,-xpi(6),xpi(2),xpi(1),dpipj(2,4),dpipj(1,6),
*     +			dpipj(2,1),ier)
*	    call vbfffxb0p(cs2,-xpi(6),xpi(3),xpi(1),dpipj(3,6),dpipj(1,6),
*     +			dpipj(3,1),ier)
*	    cs = cs1 - cs2
*	    cc0 = cs/dpipj(2,3)
*	else
...
*     +		call vbfffwarn(28,ier,absc(cs),absc(cs1))
**
*	debug output
**
*	if (lwrite) then
...
*  #[ rotate to alpha in (0,1):
	call vbfffrot3(irota3,xqi,dqiqj,qiDqj,xpi,dpipj,dum66,6,2,3,ier)
*  #] rotate to alpha in (0,1):
*  #[ look in memory:
	ierini = ier+ner
...
*		(maybe check differences as well)
		if ( lwrite ) print *,'vbfffxc0: using previous result'
		cc0 = cc0mem(i)
		ier = ier+iermem(i)
		if ( ldot ) then
		    fdel2 = dl2mem(i)
*		    we forgot to recalculate the stored quantities
		...
		return
   70	    continue
*	    if ( lwrite ) print *,'vbfffxc0: not found in memory'
	elseif ( lmem ) then
	    delta0 = delta
	endif
   71	continue
...
*###[ vbfffxc0b:
	subroutine vbfffxc0b(cc0,xqi,dqiqj,qiDqj,ier)
***#[*comment:***********************************************************
*									*
*	See vbfffxc0.							*
*									*
...
*
   ...
   98	continue
	...
	if ( lwrite ) print *,'vbfffxc0: del2 = ',del2
	if ( ldot ) fdel2 = del2
	if ( del2 .gt. 0 ) then
*	    shouldn't occur ...
*	    12-10-1993 three spacelike momenta are OK
     ...
     +	    		) then
     		call vbffferr(41,ier)
     	    	print *,'xpi = ',xqi
     	    endif
	elseif ( del2 .eq. 0 ) then
	    call vbffferr(42,ier)
	    return
	endif
	call vbfffdel3(del3,xqi,qiDqj,6,ier)
	call vbfffdl3m(del3mi,.TRUE.,del3,del2,xqi,dqiqj,qiDqj,6, 4,5,6,
     +							1,3,ier)
	do 101 i=1,3
	    j = i+1
	    if ( j .eq. 4 ) j = 1
	    call vbfffdel2(del2s(i),qiDqj,6, i+3,i,j, 1,ieri(i))
	    k = i-1
	    if ( k .eq. 0 ) k = 3
	    call vbfffdl2p(delpsi(i),xqi,dqiqj,qiDqj,i+3,j+3,k+3,i,j,k,6,
     +							ieri(i+3))
  101	continue
	ier0 = 0
	do 99 i=1,nerr
...
*	    get alpha,1-alpha
*
	    call vbfffcoot(cblph(1),calph(1),cqi(5),-cqiDqj(5,6),cqi(6),
     +							csdel2,ier)
	    call vbfffcoot(calph(3),cblph(3),cqi(5),-cqiDqj(5,4),cqi(4),
     +							csdel2,ier)
	    cslam = 2*csdel2
	    if (lwrite) then
		print '(a)','  ##[ get roots: (vbfffxc0)'
		print *,'cslam   =',2*csdel2
		ier0 = ier
*		call vbfffclmb(clamp,cqi(4),cqi(5),cqi(6),cdqiqj(4,5),
...
*		print *,'cetap   =',eta
	    ...
	    if ( lwrite ) print '(a)','  ##] determinants:'
	    call vbfffcc0p(cs3,ipi12,isoort,clogi,ilogi,cqi,cdqiqj,cqiDqj,
...
*  #] complex case:
  ...
  106	continue
	if ( abs(isgnal).ne.1 ) then
	    print *,'vbfffxc0b: error: isgnal should be +/-1, not ',isgnal
	    print *,'        forgot to call vbfffini?'
	    call vbfffini
	endif
...
*	get alpha,1-alpha
*
	call vbfffroot(blph(1),alph(1),xqi(5),-qiDqj(5,6),xqi(6),sdel2,ier)
	call vbfffroot(alph(3),blph(3),xqi(5),-qiDqj(5,4),xqi(4),sdel2,ier)
	...
	if (lwrite) then
	    ...
	    print *,'slam   =',2*sdel2
*	    ier0 = ier
*	    call vbfffxlmb(xlamp,xqi(4),xqi(5),xqi(6),dqiqj(4,5),
...
*
	call vbfffxc0p(cs3,ipi12,isoort,clogi,ilogi,xqi,dqiqj,qiDqj,
...
*###[ vbfffrot3:
	subroutine vbfffrot3(irota,xqi,dqiqj,qiDqj,xpi,dpipj,piDpj,ns,
     +		iflag,npoin,ier)
***#[*comment:***********************************************************
*									*
...
*  #] declarations:
*  #[ check input:
	if ( ltest .and. ns .ne. 6 ) print *,'vbfffrot3: error: ns /= 6'
*  #] check input:
*  #[ get largest cancellation:
	...
	if ( lwrite ) print *,'vbfffrot3: rotated over ',irota,' positions'
*  #] get largest cancellation:
*  #[ rotate:
	do 20 i=1,6
...
*  #[ test output:
	if ( ltest ) then
	    call vbfffxhck(xqi,dqiqj,6,ier)
	    if ( iflag .eq. 3 .and. xqi(3) .ne. 0 ) print *,
     +		'vbfffrot3: IR divergent C0 rotated wrongly!',xqi
	endif
*  #] test output:
*###] vbfffrot3:
	end
*###[ vbfffdot3:
	subroutine vbfffdot3(piDpj,xpi,dpipj,ns,ier)
***#[*comment:***********************************************************
*									*
...
*  #] declarations:
*  #[ check input:
	if ( ns .ne. 6 ) print *,'vbfffdot3: error: ns /= 6 '
	if ( ltest ) call vbfffxhck(xpi,dpipj,6,ier)
*  #] check input:
*  #[ copy if known:
*
	if ( idot.ge.3 ) then
...
*
     ...
     +			xpi(is2))/2 ) call vbfffwarn(100,ier0,piDpj(is1,
...
*
     ...
     +			xpi(is1))/2) call vbfffwarn(101,ier0,
...
*
     ...
     +			xpi(is2))/2) call vbfffwarn(102,ier0,
...
*
     ...
     +			ip2)),abs(dpipj(ip3,is1)))/2 ) call vbfffwarn(103,
...
*
     ...
     +			vbfffwarn(104,ier0,piDpj(ip1,ip2),
...
*  #[ check:
	if ( ltest ) then
     ...
     +			'vbfffdot3: error: dotproducts with p(',i,
...
*###[ vbfffxc0r:
	subroutine vbfffxc0r(cc0,xpi,ier)
***#[*comment:***********************************************************
*									*
