*	$Id: vbfffdel6.f,v 1.4 1996/03/14 15:53:15 gj Exp $
*###[ vbfffdel6:
	subroutine vbfffdel6(del6s,xpi,piDpj,ns,ier)
***#[*comment:***********************************************************
*									*
...
*  #[ check input:
	if ( ltest ) then
	    if ( ns.ne.21 ) then
		print *,'vbfffdel6: only for ns=21 for the time being'
		stop
	    endif
	endif
	if ( lwrite ) then
	    print *,'vbfffdel6: input '
	    print *,'xpi = ',xpi
	endif
*  #] check input:
*  #[ work:
...
*
    ...
    5	continue
	call vbfffsort(som,ii,15)
	do 6 i=1,15
	    ii(i) = ii(i)+6
    6	continue
	call vbfff5ind(ip,ii,0,ier)
*
...
*
   ...
   10	continue
	if ( lwrite ) then
	    print *,'vbfffdel6s: del6s',is,' = ',sum,xmx
	endif
	if ( is.eq.1 ) then
	    del6s = sum
	    xmax = xmx
  ...
  100	continue
	if ( lwarn ) call vbfffwarn(187,ier,sum,xmx)
  110	continue
*
*  #] work:
*###] vbfffdel6:
...
*  #[ work:
	if ( nn.gt.1024 ) then
	    print  *,'vbfffsort: can only sort up to 1024 elments, not ',nn
	    stop
	endif
	do 10 i=1,nn
	    jj(i,1) = i
...
*###[ vbfff5ind:
	subroutine vbfff5ind(ip,ii,ngiven,ier)
***#[*comment:***********************************************************
*									*
...
*
		...
		if ( in.le.ngiven ) then
			...
			print *,'vbfff5ind: error: given vectors already ',
...
**     +				depi(oldk),' give ',t
  ...
  100	continue
  101	continue
	call vbffferr(69,ier)
	do 110 i=1,5
	    ip(i) = i+6
  110	continue
  120	continue
	if ( lwrite ) then
	    print *,'vbfff5ind: found lin. independent combination ',ip
	endif
*  #] work:
*###] vbfff5ind:
	end
