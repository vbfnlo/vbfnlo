*###[ vbfffxd1:
	subroutine vbfffxd1(cd1i,cd0,cc0i,xpi,piDpj,del3,del2i,ier)
***#[*comment:***********************************************************
*									*
...
*  #[ check input:
	...
	if ( ltest ) then
		...
		if ( xpi(i) .ne. piDpj(i,i) ) then
		    print *,'vbfffxd1: error: xpi and piDpj do not agree'
		endif
    1	    continue
	    if ( xpi(11).eq.0 ) then
    ...
    4	    continue
	    ier0 = ier
	    call vbfffdot4(piDpjp,xpi,dpipj,10,ier0)
    ...
    7	    continue
     ...
     +		'vbfffxd1: error: del3 is not correct',del3,som,xmax
	endif
*  #] check input:
*  #[ call vbfffxd1a:
*
	...
	call vbfffxd1a(cd1i,md1i,cd0,md0,cc0i,mc0i,xpi,piDpj,del3,del2i,
     +		ier)
*
*  #] call vbfffxd1a:
*###] vbfffxd1:
...
*###[ vbfffxd1a:
	subroutine vbfffxd1a(cd1i,md1i,cd0,md0,cc0i,mc0i,xpi,piDpj,del3,
     +		del2i,ier)
***#[*comment:***********************************************************
*									*
*	calculate the D1(mu) = D11*p1(mu) + D12*p2(mu) + D13*p3(mu)	*
...
*  #] Form-ula:
*  #[ D11:
	if ( lwrite ) print *,'vbfffxd1: D11'
	cs(1) = - cc0i(1)*DBLE(del2i(1))
	ms(1) = mc0i(1)*abs(del2i(1))
	if ( lwrite ) print *,'vbfffdl2i 1'
	ier1 = ier
	call vbfffdl2i(del2,piDpj,10, 6,7,10,+1,7,8,9,+1,ier1)
	cs(2) = + cc0i(2)*DBLE(del2)
	ms(2) = mc0i(2)*abs(del2)*DBLE(10)**mod(ier1-ier,50)
	if ( lwrite ) print *,'vbfffdl2i 2'
	ier0 = ier
	call vbfffdl2i(del2,piDpj,10, 6,7,10,+1,8,5,10,-1,ier0)
	ier1 = max(ier1,ier0)
	cs(3) = - cc0i(3)*DBLE(del2)
	ms(3) = mc0i(3)*abs(del2)*DBLE(10)**mod(ier0-ier,50)
	if ( lwrite ) print *,'vbfffdl2i 3'
	ier0 = ier
	call vbfffdl2i(del2sa,piDpj,10, 6,7,10,+1,5,6,9,-1,ier0)
	ier1 = max(ier1,ier0)
	cs(4) = + cc0i(4)*DBLE(del2sa)
	mdelsa = abs(del2sa)*DBLE(10)**mod(ier0-ier,50)
	ms(4) = mc0i(4)*mdelsa
	ier0 = ier
	call vbfffdl3q(dl3q,piDpj, 1,6,7, 0,10,0, 0,-1,0, 0,+1,0, ier0)
	ier1 = max(ier1,ier0)
   ...
   10	continue
	if ( lwarn .and. absc(cd1i(1)) .lt. xloss*xmax ) then
	    call vbfffwarn(164,ier1,absc(cd1i(1)),xmax)
...
*  #[ D12:
*
	if ( lwrite ) print *,'vbfffxd1: D12'
	ier1 = ier
	call vbfffdl2t(del2,piDpj,7,5, 6,7,10,-1,-1, 10,ier1)
	cs(1) = - cc0i(1)*DBLE(del2)
	ms(1) = mc0i(1)*abs(del2)*DBLE(10)**mod(ier-ier1,50)
	ier0 = ier
	call vbfffdl2t(del2,piDpj,7,5, 7,8,9,-1,-1, 10,ier0)
	ier1 = max(ier1,ier0)
	cs(2) = + cc0i(2)*DBLE(del2)
	ms(2) = mc0i(2)*abs(del2)*DBLE(10)**mod(ier-ier0,50)
	ier0 = ier
	call vbfffdl2t(del2,piDpj,7,5, 8,5,10,+1,-1, 10,ier0)
	ier1 = max(ier1,ier0)
	cs(3) = - cc0i(3)*DBLE(del2)
	ms(3) = mc0i(3)*abs(del2)*DBLE(10)**mod(ier-ier0,50)
	ier0 = ier
	call vbfffdl2t(del2,piDpj,7,5, 5,6,9,+1,-1, 10,ier0)
	ier1 = max(ier1,ier0)
	cs(4) = + cc0i(4)*DBLE(del2)
	ms(4) = mc0i(4)*abs(del2)*DBLE(10)**mod(ier-ier0,50)
	ier0 = ier
	call vbfffdl3q(dl3q,piDpj, 1,7,5, 0,0,2, 0,0,-1, 0,0,+1, ier0)
	ier1 = max(ier1,ier0)
	cs(5) = + 2*cd0*DBLE(dl3q)
   ...
   20	continue
	if ( lwarn .and. absc(cd1i(2)) .lt. xloss*xmax ) then
	    call vbfffwarn(164,ier1,absc(cd1i(2)),xmax)
...
*  #[ D13:
*
	if ( lwrite ) print *,'vbfffxd1: D13'
	cs(1) = - cc0i(1)*DBLE(del2sa)
	ms(1) = mc0i(1)*mdelsa
	if ( lwrite ) print *,'vbfffdl2i 1'
	ier1 = ier
	call vbfffdl2i(del2,piDpj,10, 5,6,9,-1,7,8,9,+1,ier1)
	cs(2) = + cc0i(2)*DBLE(del2)
	ms(2) = mc0i(2)*abs(del2)*DBLE(10)**mod(ier-ier1,50)
	if ( lwrite ) print *,'vbfffdl2i 2'
	ier0 = ier
	call vbfffdl2i(del2,piDpj,10, 5,6,9,-1,8,5,10,-1,ier0)
	ier1 = max(ier1,ier0)
	cs(3) = - cc0i(3)*DBLE(del2)
	ms(3) = mc0i(3)*abs(del2)*DBLE(10)**mod(ier-ier0,50)
	...
	ier0 = ier
	call vbfffdl3q(dl3q,piDpj, 1,5,6, 2,9,0, -1,-1,0, +1,-1,0, ier0)
	ier1 = max(ier1,ier0)
	cs(5) = + 2*cd0*DBLE(dl3q)
	ms(5) = 2*md0*abs(dl3q)*DBLE(10)**mod(ier-ier0,50)
   ...
   30	continue
	if ( lwarn .and. absc(cd1i(3)) .lt. xloss*xmax ) then
	    call vbfffwarn(164,ier1,absc(cd1i(3)),xmax)
	    if ( lwrite .or. awrite ) then
		print *,'cs  = ',cs
