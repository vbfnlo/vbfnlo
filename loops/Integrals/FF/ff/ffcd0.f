...
*
	...
	if ( ltest ) then
	    do 50 i=1,4
		if ( DIMAG(cpi(i)) .gt. 0 ) call vbffferr(50,ier)
   50	    continue
	    do 60 i=5,13
		if ( DIMAG(cpi(i)) .ne. 0 ) call vbffferr(50,ier)
   60	    continue
	endif
	if ( lwrite ) then
...
*  #[ check input:
	if ( ltest ) then
	    ier0 = 0
	    call vbfffchck(cpi,cpipj,10,ier0)
...
*
    ...
    5	    continue
	    sprec = precx
	    precx = precc
	    call vbfffxd0b(cs,cfac,xpi,dpipj,ndiv,ier)
...
*
		do 20 i=1,3
		    do 19 j=i+1,4
			call vbfffthre(ithres(j,i),cpi,cpipj,10,i,j,
     +				inx(i,j))
...
*  #[ D0:
*
	if ( lwrite ) print *,'ffcd0c: calling vbfffxd0b'
	ier1 = ier
	ldotsa = ldot
	ldot = .TRUE.
	lwrisa = lwrite
*	lwrite = .FALSE.
	call vbfffxd0b(csr,cfac,xpi,dpipj,0,ier1)
	lwrite = lwrisa
	cs = csr
*
*  #] D0:
...
*
   ...
   25	    continue
	    ier0 = ier
	    if ( lwrite ) then
		print *,'ffcd0c: calling vbfffxc0a'
		print *,'xpi3 = ',xpi3
	    endif
	    ldot = .TRUE.
	    call vbfffxc0a(cc0r,xpi3,dpipj3,ier0)
	    ier1 = max(ier1,ier0)
	    del2 = fdel2
	    if ( lwrite ) then
		print *,'ffcd0c: calling vbfffcc0a'
		print *,'cpi3 = ',cpi3
	    endif
	    nscsav = nschem
	    nschem = 7
	    ldot = .FALSE.
	    call vbfffcc0a(cc0c,cpi3,cpipj3,ier0)
	    nschem = nscsav
	    ier1 = max(ier1,ier0)
...
*	    never come here
     ...
     +			absc(cc0c)) ) then
	      call vbfffwarn(211,ier1,absc(cs),max(absc(cc0r),absc(cc0c)))
	    endif
	    if ( lwrite ) then
	    	print *
...
*
		if ( lwrite ) print *,'ffcd0c: calling vbfffxb0p ',i,j,ij
		ier0 = ier
		call vbfffxb0p(cb0r,xpi(ij),xpi(i),xpi(j),dpipj(i,ij),
     +			dpipj(j,ij),dpipj(i,j),ier0)
		ier1 = max(ier1,ier0)
*
		if ( lwrite ) print *,'ffcd0c: calling vbfffcb0p ',i,j,ij
		ier0 = ier
		call vbfffcb0p(cb0c,cpi(ij),cpi(i),cpi(j),cpipj(i,ij),
...
*
     ...
     +			call vbfffwarn(212,ier1,p1,xmax)
*
		l = notijk(i,j,k)
		if ( abs(a(1)) .lt. abs(a(2)) ) then
     ...
     +			call vbfffwarn(213,ier1,p2,xmax)
*
...
*
	...
	call vbfffcel4(cfdl4s,cpi,cfpij4,10,ier0)
...
*		this prec-juggling should not be necessary, but it is...
		sprec = precx
		precx = precc
		call vbfffdl3p(fodel3,piDpj,10,ii,ii,ier0)
		precx = sprec
	    else
		fodel3 = fdel3
	    endif
...
*	simulate the differences in the masses etc..
	...
	if ( cpi(11)  .eq. 0 ) then
     ...
     +		    call vbfffwarn(153,ier,absr(cpi(11)),xmax)
	    endif
	    luvw(1) = .TRUE.
	else
	    luvw(1) = .FALSE.
	...
	if ( cpi(12)  .eq. 0 ) then
     ...
     +		    call vbfffwarn(154,ier,absr(cpi(12)),xmax)
	    endif
	    luvw(2) = .TRUE.
	else
	    luvw(2) = .FALSE.
	...
	if ( cpi(13)  .eq. 0 ) then
     ...
     +		    call vbfffwarn(155,ier,absr(cpi(13)),xmax)
	    endif
	    luvw(3) = .TRUE.
	else
	    luvw(3) = .FALSE.
...
*		    we do not need the differences of s,t,u,v,w accurately
     ...
     +					.and. cpi(i) .ne. cpi(j) ) then
			ier0 = 0
			call vbfffwarn(135,ier0,absc(cpipj(j,i)),
     +				absc(cpi(i)))
