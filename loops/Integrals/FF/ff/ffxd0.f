*--#[ log:
*	$Id: vbfffxd0.f,v 1.4 1996/01/22 13:32:52 gj Exp $
*	$Log: vbfffxd0.f,v $
...
c Added warnings to ffwarn.dat; slightly changed debug output in vbfffxd0.f
c
...
*--#] log:
*###[ vbfffxd0:
	subroutine vbfffxd0(cd0,xpi,ier)
***#[*comment:***********************************************************
*									*
*			  1   /						*
...
*
	include 'ff.h'
*  #] declarations:
*  #[ call vbfffdif4, vbfffxd0a:
*
	call vbfffdif4(dpipj,luvw,xpi,ier)
	call vbfffxd0a(cd0,xpi,dpipj,ier)
*
...
*###[ vbfffxd0a:
	subroutine vbfffxd0a(cd0,xpi,dpipj,ier)
*
*	glue routine which calls vbfffxd0b with ndiv=0
*
	implicit none
*
*	arguments
...
*	and go!
*
	call vbfffxd0b(cs,cfac,xpi,dpipj,0,ier)
	cd0 = cs*cfac
*
*###] vbfffxd0a:
	end
*###[ vbfffxd0b:
	subroutine vbfffxd0b(cs,cfac,xpi,dpipj,ndiv,ier)
***#[*comment:***********************************************************
*									*
*			  1   /						*
*	calculate cd0 =	----- \dq [(q^2 + 2*s_1.q)*(q^2 + 2*s2.q)	*
...
*	calls:	vbfffcxs3,vbfffcxr,vbfffcrr,...					*
*									*
***#]*comment:***********************************************************
*  #[ declarations:
	implicit none
...
*
	if ( ltest .and. idot.gt.0 ) then
	    if ( lwrite ) print *,'vbfffxd0b: checking input dotproducts'
	    ier0 = ier
	    idotsa = idot
	    idot = 0
	    call vbfffdot4(qiDqj,xpi,dpipj,10,ier0)
	    idot = idotsa
     ...
     +			    'vbfffxd0b: error: input dotproduct piDpj(',j,
   ...
   40		continue
	...
	if ( ltest ) then
		...
		if ( lwrite ) print *,'vbfffxd0b: checking input fdel3 ',
     +			fdel3,ier0
		if ( idot.lt.0 ) then
		    ier0 = ier
		    idotsa = idot
		...
		ii(5) =  9
		ii(6) = 10
		call vbfffdl3p(vgl,qiDqj,10,ii,ii,ier0)
		rloss = xloss**2*DBLE(10)**(-mod(ier0,50))
		xnul = fdel3 - vgl
		xmax = abs(vgl)
		if ( abs(rloss*xnul).gt.precx*xmax ) print *,
     +			'vbfffxd0b: error: input del3p wrong: ',fdel3,vgl,
     +			xnul,ier0
	    endif
	    if ( idot.ge.4 ) then
		if ( lwrite ) print *,'vbfffxd0b: checking input fdel4s'
		call vbfffdel4(vgl,xpi,qiDqj,10,ier0)
		xnul = fdel4s - vgl
		xmax = abs(vgl)
		if ( abs(rloss*xnul).gt.precx*xmax ) print *,
     +			'vbfffxd0b: error: input del4s wrong: ',fdel4s,vgl,
     +			xnul,ier0
	    endif
	endif
*
...
*
	call vbfffxdir(cs,cfac,idone,xpi,dpipj,4,ndiv,ier)
	if ( idone .le. 0 .and. ndiv .gt. 0 ) then
	    if ( lwrite ) print *,'vbfffxd0b: at most log divergence'
	    cs = 0
	    cfac = 1
	    ier = 0
	    return
...
*  #[ rotate to calculable position:
	call vbfffrot4(irota4,del2s,xpip,dpipjp,piDpjp,xpi,dpipj,qiDqj,4,
     +		itype,ier)
	if ( itype .lt. 0 ) then
	    print *,'vbfffxd0b:  error:  Cannot handle this ',
     +		' masscombination yet:'
	    print *,(xpi(i),i=1,13)
	    return
	endif
...
*
	    if ( ini2ir .eq. 0 ) then
		ini2ir = 1
		print *,'vbfffxd0b: using the log(lam) prescription to'
		print *,'        regulate the 2 infrared poles to match'
		print *,'        with soft gluon massive, lam^2 =',delta
	    endif
	    if ( ltest .and. idone .ne. 2 ) then
		print *,'vbfffxd0: error: itype=2 but idone != 2'
	    endif
	    ier2 = 0
	    call vbfffx2ir(cs1,cs2,xpip,dpipjp,ier2)
	    del2s = -delta**2/4
*
...
*
     ...
     +		call vbfffwarn(172,ier,absc(cs),xmax)
	    if ( .not.ldot ) return
	endif
*
*  #] treat doubly IR divergent case:
...
*		(maybe check differences as well)
		if ( lwrite ) print *,'vbfffxd0b: using previous result'
		cs = csmem(i)
		cfac = cfcmem(i)
		ier = ier+iermem(i)
		if ( ldot ) then
...
*		    we forgot to calculate the dotproducts
		...
		return
  150	    continue
*	    if ( lwrite ) print *,'vbfffxd0b: not found in memory'
	elseif ( lmem ) then
	    delta0 = delta
	endif
   51	continue
...
*
	ier0 = ier
	call vbfffgdt4(piDpjp,xpip,dpipjp,xpi,dpipj,itype,ier0)
...
*  #[ calculations:
*
	call vbfffxd0e(cs,cfac,xmax, .FALSE.,ndiv,xpip,dpipjp,piDpjp,del2s,
     +		ldel2s,ier)
*
*	Finally ...
...
*
	if ( lwarn .and. 2*absc(cs) .lt. xloss*xmax )
     +		call vbfffwarn(84,ier,absc(cs),xmax)
*
*  #] calculations:
*  #[ add to memory:
*
...
*###[ vbfffxd0e:
	subroutine vbfffxd0e(cs,cfac,xmax,lir,ndiv,xpip,dpipjp,piDpjp,
     +		del2s,ldel2s,ier)
***#[*comment:***********************************************************
*									*
...
*
	...
	logical laai
	integer i,j,ier0,itime,maxlos,init,isoort(16),ipi12(28),
...
*
	    ier0 = ier
	    call vbfffxdir(cs,cfac,idone,xpip,dpipjp,4,0,ier)
	    if ( idone .le. 0 .and. ndiv .gt. 0 ) then
		if ( lwrite ) print *,'vbfffxd0e: at most log divergence'
		cs = 0
		cfac = 1
		xmax = 0
		ier = 0
...
*
	if ( ltest ) then
		...
		print *,'vbfffxd0e: error: found ier = ',ier
...
*
	...
	ier0 = ier
	call vbfffai(ai,daiaj,aai,laai,del2s,sdel2s,xpip,dpipjp,piDpjp,
     +		ier0)
	if ( ier0 .ge. 100 ) goto 70
	call vbffftran(ai,daiaj,aai,laai,xqi,dqiqj,qiDqj,del2s,sdel2s,
	...
	if ( .not.ldel2s ) then
     ...
     +			abs(dt3t4),abs(xqi(3)),abs(xqi(4))) ) then
		    print *,'vbfffxd0a:  error:  dt3t4 <> t3 - t4',dt3t4,
     +			xqi(3),xqi(4),dt3t4-xqi(3)+xqi(4)
		endif
	    endif
	    if ( dt3t4 .eq. 0 ) then
*		don't know what to do...
		call vbffferr(85,ier)
		return
	    endif
	else
...
*	    go ahead if we have half the digits left
	    ...
	    if ( lwrite ) print *,'vbfffxd0a: redo trans if loss > ',maxlos
	endif
	if ( ier0-ier .gt. maxlos ) then
		...
		isgnal = -isgnal
		if ( lwrite ) print *,'vbfffxd0a: trying other root, ier=',
     +			ier0
		goto 25
	    else
		if ( ier0-ier .lt. 100 ) then
*		    it does not make any sense to go on, but do it anyway
		    if ( lwrite ) print *,'vbfffxd0a:  both roots rotten ',
     +			'going on'
		elseif ( itime.eq.100 ) then
		    if ( lwrite ) print *,'vbfffxd0a:  both roots rotten ',
     +			'giving up'
		    call vbffferr(72,ier)
		    cfac = 1
		    return
...
*  #] transform the masses and momenta:
*  #[ calculations:
	call vbfffxd0p(cs4,ipi12,isoort,cfac,xpip,dpipjp,piDpjp,
...
*
	if ( lwarn .and. absc(cs) .lt. xalogm/precc .and. cs .ne. 0 )
     +		call vbfffwarn(119,ier,absc(cs),xalogm/precc)
*
...
*###[ vbfffxd0r:
	subroutine vbfffxd0r(cd0,xpi,ier)
***#[*comment:***********************************************************
*									*
...
*###[ vbfffxd0d:
	subroutine vbfffxd0d(cd0,xpi,piDpj,del3p,del4s,info,ier)
***#[*comment:***********************************************************
*									*
...
*###[ vbfffdif4:
	subroutine vbfffdif4(dpipj,luvw,xpi,ier)
***#[*comment:***********************************************************
*									*
...
*	simulate the differences in the masses etc..
	if ( lwrite ) print *,'vbfffdif4: input xpi: ',xpi
	ier1 = ier
	if ( xpi(11)  .eq. 0 ) then
     ...
     +		    call vbfffwarn(153,ier1,xpi(11),xmax)
	    endif
	    luvw(1) = .TRUE.
	else
	    luvw(1) = .FALSE.
	...
	if ( xpi(12)  .eq. 0 ) then
     ...
     +		    call vbfffwarn(154,ier0,xpi(12),xmax)
		ier1 = max(ier1,ier0)
	    endif
	    luvw(2) = .TRUE.
	else
...
*	    xpi(13) = xpi(5)-xpi(6)+xpi(7)-xpi(8)+xpi(9)+xpi(10)
     ...
     +		    call vbfffwarn(155,ier0,xpi(13),xmax)
		ier1 = max(ier1,ier0)
	    endif
	    luvw(3) = .TRUE.
	else
...
*		    we do not need the differences of s,t,u,v,w accurately
     ...
     +					.and. xpi(i) .ne. xpi(j) ) then
			ier0 = ier
			call vbfffwarn(121,ier0,dpipj(j,i),xpi(i))
