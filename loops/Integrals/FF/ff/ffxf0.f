*	$Id: vbfffxf0.f,v 1.5 1996/02/07 09:28:49 gj Exp $
*###[ vbfffxf0:
	subroutine vbfffxf0(cf0,ce0i,cd0ij,xpi,ier)
***#[*comment:***********************************************************
*									*
*	calculate							*
*									*
...
*
	...
	if ( lwarn ) then
     ...
     +			.and. xpi(i) .ne. xpi(j) ) then
			call vbfffwarn(193,ier0,dpipj(j,i),xpi(i))
...
*  #] get differences: 
*  #[ call vbfffxf0a:
	call vbfffxf0a(cf0,ce0i,cd0ij,xpi,dpipj,ier)
*  #] call vbfffxf0a: 
*###] vbfffxf0: 
	end
*###[ vbfffxf0a:
	subroutine vbfffxf0a(cf0,ce0i,cd0ij,xpi,dpipj,ier)
***#[*comment:***********************************************************
*									*
*	calculate							*
*									*
...
*
	idsub = idsub + 1
	call vbfffdot6(piDpj,xpi,dpipj,ier)
	if ( ldot ) then
	    do 6 i=1,21
		do 5 j=1,21
		    fpij6(j,i) = piDpj(j,i)
	...
	if ( ltest ) then
	    ...
	    ii(4) = 10
	    ii(5) = 11
	    call vbfffdl5p(xpi,piDpj,21,ii,ier)
	endif
...
*
	    ier1 = ier
	    call vbfffpi65(xpi5,dpipj5,piDpj5,xpi,dpipj,piDpj,i,ier1)
*
...
*
     ...
     +		    '####[ vbfffxf0a: fourpoint function nr ',i,j+1,': '
		if ( k.lt.i ) then
*		    we already have it
		else
		    j = j+1
...
*
		    call vbfffpi54(xpi4,dpipj4,piDpj4,xpi5,dpipj5,piDpj5,k,
     +			ier0)
		    if ( ltest ) then
			idum = ier
			call vbfffpi64(xqi4,dqiqj4,qiDqj4,xpi,dpipj,piDpj,
     +				i,j,idum)
			xlosn = xloss*DBLE(10)**(-mod(ier0,50))
			do 12 l=1,13
			    if ( xlosn*abs(xpi4(l)-xqi4(l)).gt.precx*abs
     +			      (xpi4(l)) ) print*,'vbfffxf0a: error: xpi4(',
     +			      l,') != xqi4(',l,'): ',xpi4(l),xqi4(l)
			    do 11 m=1,10
				if ( xlosn*abs(dpipj4(m,l)-dqiqj4(m,l))
     +				    .gt.precx*abs(xpi4(l)) ) print *,
     +				    'vbfffxf0a: error: dpipj4(',m,l,') !=',
...
*
			call vbfffrot4(irota,del2s,xqi4,dqiqj4,qiDqj4,xpi4,
     +				dpipj4,piDpj4,5,itype,ier0)
			if ( itype .lt. 0 ) then
			    print *,'vbfffxf0:  error:  Cannot handle '//
     +				'this 4point masscombination yet:'
			...
			if ( itype .eq. 1 ) then
			    ldel2s = .TRUE.
			    isgnal = +1
			    print *,'vbfffxf0a: Cannot handle del2s=0 yet'
			    stop
			else
			    ldel2s = .FALSE.
			endif
			if ( itype .eq. 2 ) then
			    print *,'vbfffxf0a: Cannot handle doubly IR ',
...
*
			if ( lwrite ) then
			    print *,'xpi for vbfffxd0e: '
			    print '(i3,e24.16)',(m,xqi4(m),m=1,13)
			endif
			lwsav = lwrite
			lwrite = .FALSE.
			idsub = idsub + 1
			call vbfffxd0e(cs,cfac,xmax, .FALSE.,ndiv,xqi4,
...
*
		idsub = idsub + 1
		ier0 = ier
		call vbfffdl4r(dl4rij(i,j),xpi5,piDpj5,15,k,ier0)
		ier1 = max(ier1,ier0)
*
...
*
		...
		if ( lwrite ) then
		    print '(a,2i2,a)',
     +			'####] vbfffxf0a: fourpoint function nr ',i,j,': '
...
*
     ...
     +		'####[ vbfffxf0a: fivepoint function nr ',i,': '
	    call vbfffxe00(ce0i(i),cd0i,dl4ri,xpi5,piDpj5,ier1)
	    if ( lwrite ) print '(a,i2,a)',
     +		'####] vbfffxf0a: fivepoint function nr ',i,': '
...
*
	ier1 = 0
	call vbfffdel6(del6,xpi,piDpj,21,ier1)
	csum = 0
	xmax = 0
	do i=1,6
	    if ( ce0i(i) .ne. 0 ) then
		ier0 = 0
		call vbfffdl5r(dl5ri(i),xpi,piDpj,21,i,ier0)
...
*###[ vbfffxf0r:
	subroutine vbfffxf0r(cf0,ce0i,cd0ij,xpi,ier)
***#[*comment:***********************************************************
*									*
...
*  #[ calculations:
   ...
   10		continue
     ...
     +			',isgnal ',isgnal
		...
		id = id + 1
		isgnal = ialsav
		call vbfffxf0(cf0p,ce0ip,cd0ijp,xqi,ier1)
		ier1 = ier1 + ner
		if ( ier.gt.5 ) call vbfffwarn(998,ier,x0,x0)
		print '(a,i2,a,i2)','---#] rotation ',irota,': isgnal ',
     +			isgnal
