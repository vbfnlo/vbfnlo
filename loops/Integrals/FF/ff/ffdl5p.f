*--#[ log:
*	$Id: vbfffdl5p.f,v 1.3 1996/02/12 21:06:19 gj Exp $
*	$Log: vbfffdl5p.f,v $
c Revision 1.3  1996/02/12  21:06:19  gj
c Added safety check on ns in vbfffdl5r, updated comment
c
...
*--#] log:
*###[ vbfffdl5p:
	subroutine vbfffdl5p(xpi,pDp,ns,ii,ier)
***#[*comment:***********************************************************
*	check that							*
*									*
...
*
   ...
   80	continue
	...
	if ( xlosn*abs(som) .gt. precx*xmax )
     +	    print *,'vbfffdl5p: error: dl5p != 0: ',som,xmax
	if ( lwrite ) print *,'vbfffdl5p: dl5p = ',som,xmax
*
*  #] del5(p):
*###] vbfffdl5p:
	end
*###[ vbfffdl5r:
	subroutine vbfffdl5r(dl5r,xpi,piDpj,ns,inum,ier)
***#[*comment:***********************************************************
...
*  #[ check input:
	if ( ltest ) then
		...
		print *,'vbfffdl5r: error: inum < 1 or > 6: ',inum
		stop
	    endif
	endif
	if ( ns.ne.21 ) then
	    print *,'vbfffdl5r: only for 6point pi, ns should be 21, not ',
     +		ns
	    stop
	endif
*  #] check input:
...
*
	if ( .not.lagain ) then
	    ...
	    if ( lwarn ) call vbfffwarn(188,ier,dl5r,xmax)
	    if ( ltest ) then
		do 900 i=2,5
		    k = inum + i - 1 + 6
		    if ( k.gt.12 ) k = k-6
  ...
  900		continue
		...
		print *,'vbfffdl5r: error: is not what it should be: ',
