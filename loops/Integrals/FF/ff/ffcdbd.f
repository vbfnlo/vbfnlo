...
*
  ...
  122	    continue
	    ...
	    ii(5)= 9
	    ii(6)= 10
	    call vbfffcl3p(dl3p,cfpij4,10,ii,ii,ier0)
	    fodel3 = DBLE(dl3p)
	    fdel3 = fodel3
	endif
*
...
*
	...
	DOUBLE COMPLEX c,xxt(3),xx2(3),xx3(3),xm0,xm1,xm4,xlam,
     +		csi(21),z,zlg,vbfzfflog,vbfzfflo1,vbfzxfflg
	save init
*for Absoft
...
*  #[ real case:
   ...
   20	    continue
	    ...
	    c2sisj(i4,ilam) = 0
	    c2sisj(ilam,i4) = 0
	    call vbfffxdbd(cs,cfac,xpi,dpipj,ilam,i1,i4,ic,ier)
	    lsmug = .FALSE.
	    return
	endif
*  #] real case:
...
*	this one is real!
	...
	xma = DBLE(xm1)
	xmb = DBLE(xm4)
	call vbfffxkfn(xxs,iepss,xp,xma,xmb,ier0)
	ier1 = max(ier0,ier1)
	ier0 = 0
...
*
	...
	if ( abs(d) .lt. xloss ) then
	    csi(1) = 2*zlg*DBLE(vbfdfflo1(d,ier0))
	else
	    csi(1) = 2*zlg*vbfzxfflg(xxs(2)*xxs(3),-iepss,x0,ier0)
	endif
	ier1 = max(ier0,ier1)
	ier0 = 0
...
*
	ier0 = 0
	call vbfffzxdl(csi(3),ip,zlg,xxs(1)**2,iepss,ier0)
	ipi12 = ipi12 + ip
	ier1 = max(ier0,ier1)
*
	ier0 = 0
	z = vbfzfflog(xx2(1),0,c0,ier0)
	csi(4) = z**2
	ier1 = max(ier0,ier1)
*
	ier0 = 0
	z = vbfzfflog(xx3(1),0,c0,ier0)
	csi(5) = z**2
	ier1 = max(ier0,ier1)
*
	is = 6
...
*
		ier0 = 0
		call vbfffzzdl(csi(is),ip,zlg,DBLE(xxs(1))*xx2(1)**i2*
     +			xx3(1)**i3,ier0)
		csi(is) = -csi(is)
		ipi12 = ipi12 - ip
		is = is + 1
...
*
		...
		if ( abs(xxs(2)) .gt. xloss ) then
		     csi(is) = -zlg*vbfzxfflg(xxs(1),iepss,x0,ier0)
		else
		     csi(is) = -zlg*DBLE(vbfdfflo1(xxs(2),ier0))
		endif
		is = is + 1
		ier1 = max(ier0,ier1)
*
		ier0 = 0
		csi(is) = -zlg*vbfzfflog(xx2(1)**i2,0,c0,ier0)
		is = is + 1
		ier1 = max(ier0,ier1)
*
		ier0 = 0
		csi(is) = -zlg*vbfzfflog(xx3(1)**i3,0,c0,ier0)
		is = is + 1
		ier1 = max(ier0,ier1)
*
  100	    continue
...
*
  ...
  200	continue
	...
	if ( lwarn .and. absc(cs) .lt. xloss*xmax )
     +		call vbfffwarn(177,ier,absc(cs),xmax)
*
*  #] sum:
*  #[ overall factors:
*
...
*
	...
	if ( lwarn .and. absc(cc1) .lt. xloss*max(absc(cpi),absc(xm)**2)
     +		) then
	    call vbfffwarn(178,ier,absc(cc1),max(absc(cpi),absc(xm)**2))
	    if ( lwrite ) print *,'need extra input'
	endif
	cc2 = 1 - 4*xm*xmp/cc1
	if ( lwarn .and. absc(cc2) .lt. xloss )
     +	    call vbfffwarn(179,ier,absc(cc1),x1)
	wortel = sqrt(cc2)
	cc3 = 1/(1+wortel)
