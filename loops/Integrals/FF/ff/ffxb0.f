*###[ vbfffxb0:
	subroutine vbfffxb0(cb0,d0,xmu,xp,xma,xmb,ier)
***#[*comment:***********************************************************
*									*
...
*  #[ check input:
	if ( lwrite ) then
	    print *,'vbfffxb0: nevent,id = ',nevent,id,' input:'
	    print *,'xma,xmb,xp,ier = ',xma,xmb,xp,ier
	endif
	if ( ltest ) then
	    if ( xma .lt. 0 .or. xmb .lt. 0 ) then
		print *,'vbfffxb0: error: xma,b < 0: ',xma,xmb
		stop
	    endif
	endif
*  #] check input:
*  #[ get differences:
	...
	if ( lwarn ) then
     ...
     +		call vbfffwarn(97,ier0,dmamb,xma)
	    if ( abs(dmap) .lt. xloss*abs(xp) .and. xp .ne. xma )
     +		call vbfffwarn(98,ier0,dmap,xp)
	    if ( abs(dmbp) .lt. xloss*abs(xp) .and. xp .ne. xmb )
     +		call vbfffwarn(99,ier0,dmbp,xp)
	endif
*  #] get differences:
*  #[ calculations:
	...
	if ( abs(xm) .gt. xalogm ) then
     ...
     +		call vbfffwarn(150,ier,absc(cb0),max(abs(d0),absc(cb0p)))
	else
	    call vbffferr(4,ier)
	    cb0 = DBLE(d0) - cb0p
	endif
...
*###[ vbfffxb0p:
	subroutine vbfffxb0p(cb0p,xp,xma,xmb,dmap,dmbp,dmamb,ier)
***#[*comment:***********************************************************
*									*
...
*
	...
	DOUBLE PRECISION ax,ay,vbfffbnd,
...
*  #[ check input:
	if (ltest) then
     ...
     +			dmamb))/xloss ) then
		print *,'vbfffxb0q: input not OK, dmamb <> xma-xmb',xcheck
	    endif
	    xcheck = -xp + xma - dmap
	    if ( abs(xcheck) .gt. precx*max(abs(xp),abs(xma),abs(
     +			dmap))/xloss ) then
		print *,'vbfffxb0q: input not OK, dmap <> xma - xp',xcheck
	    endif
	    xcheck = -xp + xmb - dmbp
	    if ( abs(xcheck) .gt. precx*max(abs(xp),abs(xmb),abs(
     +			dmbp))/xloss ) then
		print *,'vbfffxb0q: input not OK, dmbp <> xmb - xp',xcheck
	    endif
	endif
*  #] check input:
*  #[ fill some dotproducts:
	if ( ldot ) then
	    call vbfffdot2(fpij2,xp,xma,xmb,dmap,dmbp,dmamb,ier)
	endif
*  #] fill some dotproducts:
*  #[ which case:
*
...
*
	...
	if ( lwarn .and. absc(cb0p) .lt. xloss*x2 )
     +		call vbfffwarn(14,ier,absc(cb0p),x2)
	goto 990
*  #] one mass equal to zero:
*  #[ both masses equal:
  300	continue
...
*--#]	    data and bounds:
		...
		call vbfffwarn(15,ier,precx,abs(xpneq(25))*ax**25)
	    endif
     ...
     +		xpneq(5) + som ))))
	    ...
	    cb0p = x*(xpneq(1)+som)
	    if (lwrite) then
		print *,'vbfffxb0q: m1 = m2, Taylor expansion in ',x
	    endif
	    goto 990
* -#]	    taylor expansion:
	endif
...
*	normal case
*
	call vbfffxlmb(xlam,-xp,-xm,-xm,dmp,dmp,x0,ier)
	if ( xlam .ge. 0 ) then
*	    cases 1,2 and 4
	    slam = sqrt(xlam)
	    s2a = dmp + xm
...
*		looks fine
		...
		if ( lwarn ) call vbfffwarn(16,ier,ax,xalogm)
		s = 0
...
*		the following line is superfluous.
     ...
     +			call vbfffwarn(17,ier,s2,slam)
		s = -slam/xp*vbfdfflo1(s2/(2*xm),ier)
	    else
...
*	    the root is complex (k^2 between 0 and (2*m1)^2)
	...
	if (lwrite) print *,'s =   ',s
	xx = s - 2
	if ( lwarn .and. abs(xx).lt.xloss*2 ) call vbfffwarn(18,ier,xx,x2)
...
* -#[	get log(xm2/xm1):
  400	continue
	...
	elseif ( abs(x-1) .lt. xloss ) then
	    xlogmm = vbfdfflo1(dm1m2/xm1,ier)
	else
	    xlogmm = log(x)
	endif
* -#]	get log(xm2/xm1):
...
*		    determine the boundaries for 1,5,10,15 terms
			...
			bdn120 = vbfffbnd(1,20,xpnn1)
		    endif
*--#]		    data and bounds:
*		    calculate:
		    if ( lwarn .and. ax .gt. bdn120 )
     +			call vbfffwarn(19,ier,precx,abs(xpnn1(20))*ax**19)
     ...
     +			(xpnn1(5) +s))))
		    ...
		    s = x*x*(xpnn1(1) + s)
		    if (lwrite) then
			print *,'vbfffxb0q: xp = 0, simple Taylor exp'
			print *,'	 in ',x
			print *,'	 gives s ',s/2
		    endif
		else
...
*		    calculate:
     ...
     +			call vbfffwarn(20,ier,precx,xinfac(23)*ay**23)
     ...
     +			    y*(xinfac(8) + s))))
		    ...
		    s = x*y**2*(1+y)/12 - s
		    s = - 2*vbfdfflo1(s,ier)/y
		    if (lwrite) then
			print *,'vbfffxb0q: xp = 0, other Taylor expansion'
			print *,'	 in ',y
			print *,'	 s = ',s/2
		    endif
		endif
...
*
	call vbfffxlmb(xlam,-xp,-xm2,-xm1,dm2p,dm1p,dm1m2,ier)
	if ( xlam .gt. 0 ) then
...
*		looks fine
	    ...
	    xx = s1+s2-2
	    if (lwrite) then
		print *,'vbfffxb0q: lam>0, first try, xx  = ',xx,s1,s2,-2
	    endif
*--#]	    first try:
...
*		    choose a quick way to get the logarithm
		...
		xx = s1 + s2 - 2
		if (lwrite) then
		    print *,'vbfffxb0q: lam>0, second try, xx  = ',xx
...
*			determine the boundaries for 1,5,10,15 terms
			if ( xprcn3 .ne. precx ) then
			    xprcn3 = precx
			    bdn301 = vbfffbnd(3,1,xinfac)
			    bdn305 = vbfffbnd(3,5,xinfac)
			    bdn310 = vbfffbnd(3,10,xinfac)
			    bdn315 = vbfffbnd(3,15,xinfac)
			endif
* -#]			bounds:
			xnoe = -xp + 2*xm1 + 2*xm2
			x = 4*dm1m2/xnoe
			...
			if ( lwarn .and. ax .gt. bdn315 ) then
			    call vbfffwarn(21,ier,precx,xinfac(17)*ax**14)
			endif
     ...
     +			    call vbfffwarn(22,ier,s1p,s1a)
			s1p = xnoe*vbfdfflo1(s1p,ier)/(slam - dm1m2)/2
			if (lwrite) then
			    print *,'vbfffxb0q: Taylor exp. of s1-2(1-a)'
			    print *,'	     in x = ',x
			    print *,'	     gives s1p = ',s1p
			endif
		    endif
...
*			determine the boundaries for 1,5,10,15 terms
			if ( xprcn5 .ne. precx ) then
			    xprcn5 = precx
			    bdn501 = vbfffbnd(4,1,xinfac)
			    bdn505 = vbfffbnd(4,5,xinfac)
			    bdn510 = vbfffbnd(4,10,xinfac)
			    bdn515 = vbfffbnd(4,15,xinfac)
			endif
* -#]			bounds:
			xnoe = slam - dm1m2
			x = 2*xp/xnoe
...
*			    do not do the Taylor expansion
			    if ( lwarn ) call vbfffwarn(23,ier,s2p,s2) 
			    goto 495
			endif
     ...
     +				call vbfffwarn(24,ier,s2p,s2a)
			s2p = -slam/xp*vbfdfflo1(s2p,ier)
			if (lwrite) then
			    print *,'vbfffxb0q: Taylor expansion of s2-2a'
			    print *,'	     in x = ',x
			    print *,'	     gives s2p = ',s2p
			endif
		    endif
...
*--#[	    first try:
	    ...
	    xx = s1 + s2 - 2
	    if (lwrite) then
		print *,'vbfffxb0q: lam<0, first try, xx  = ',xx
...
*			do not do the Taylor expansion
			if ( lwarn ) call vbfffwarn(21,ier,s1p,s1)
			goto 590
		    endif
		    if ( ax .gt. bdn310 ) then
			s1a = x*(xinfac(13) + x*(xinfac(14) + x*(
     ...
     +			xp**3*(dm1m2+xp))/(xm1*dm1m2**2*xnoe**2)
		    ...
		    if ( lwarn .and. abs(s1p) .lt. xloss*abs(s1a) )
     +			call vbfffwarn(22,ier,s1p,s1a)
		    s1p = -dm1m2*vbfdfflo1(s1p,ier)/(2*xp)
		    if (lwrite) then
			print *,'vbfffxb0q: Taylor expansion of s1-2(1-a)'
			print *,'	 in x = ',x
			print *,'	 gives s1p = ',s1p
		    endif
		endif
...
* -#]		    bounds:
     ...
     +			call vbfffwarn(24,ier,absc(cs2p),absc(cs2b))
...
*###[ vbfffxlmb:
	subroutine vbfffxlmb(xlambd,a1,a2,a3,a12,a13,a23,ier)
***#[*comment:***********************************************************
...
*	    xcheck input
     ...
     +		print *,'vbfffxlmb: input not OK, a12 /= a1 - a2',a12,a1,
     +		a2,xcheck
	    xcheck = a1 - a3 - a13
	    if ( xloss*abs(xcheck) .gt. precx*max(aa1,aa3,aa13) )
     +		print *,'vbfffxlmb: input not OK, a13 /= a1 - a3',a13,a3,
     +		a3,xcheck
	    xcheck = a2 - a3 - a23
	    if ( xloss*abs(xcheck) .gt. precx*max(aa2,aa3,aa23) )
     +		print *,'vbfffxlmb: input not OK, a23 /= a2 - a3',a23,a2,
     +		a3,xcheck
	endif
*
...
*
  ...
  100	continue
	...
	if ( lwarn .and. abs(xlambd) .lt. xloss*asq )
     +		call vbfffwarn(69,ier,xlambd,asq)
*  #] calculations:
*###] vbfffxlmb:
	end
*###[ vbfffclmb:
	subroutine vbfffclmb(clambd,cc1,cc2,cc3,cc12,cc13,cc23,ier)
***#[*comment:***********************************************************
...
*	    check input
		...
		print *,'vbfffclmb: input not OK, cc12 /= cc1 - cc2',check
	    endif
	    check = cc1 - cc3 - cc13
	    if ( xloss*absc(check) .gt. precc*max(aa1,aa3,aa13) ) then
		print *,'vbfffclmb: input not OK, cc13 /= cc1 - cc3',check
	    endif
	    check = cc2 - cc3 - cc23
	    if ( xloss*absc(check) .gt. precc*max(aa2,aa3,aa23) ) then
		print *,'vbfffclmb: input not OK, cc23 /= cc2 - cc3',check
	    endif
	endif
*
*	first see if there are input parameters with opposite sign:
...
*
  ...
  100	continue
	...
	if ( lwarn .and. absc(clambd) .lt. xloss*absc(csq) )
     +		call vbfffwarn(68,ier,absc(clambd),absc(csq))
*  #] calculations (rather old style ...):
*###] vbfffclmb:
	end
*###[ vbfffdot2:
	subroutine vbfffdot2(piDpj,xp,xma,xmb,dmap,dmbp,dmamb,ier)
***#[*comment:***********************************************************
*									*
...
*  #[ work:
	...
	if ( lwarn .and. abs(piDpj(1,2)) .lt. xloss*min(xma,xmb)/2
     +			) then
		call vbfffwarn(24,ier1,piDpj(1,2),min(xma,xmb)/2)
	endif
	...
	if ( lwarn .and. abs(piDpj(1,3)) .lt. xloss*min(xma,
     +			abs(xp))/2) then
     		ier0 = ier
		call vbfffwarn(25,ier0,piDpj(1,3),min(xma,abs(xp))/2)
		ier1 = max(ier0,ier1)
	endif
	...
	if ( lwarn .and. abs(piDpj(2,3)) .lt. xloss*min(xmb,
     +			abs(xp))/2) then
     		ier0 = ier
		call vbfffwarn(25,ier0,piDpj(2,3),min(xmb,abs(xp))/2)
		ier1 = max(ier0,ier1)
	endif
	ier = ier1
*  #] work:
