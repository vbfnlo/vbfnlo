*###[ vbfffdel2:
	subroutine vbfffdel2(del2,piDpj,ns,i1,i2,i3,lerr,ier)
*************************************************************************
...
*		we know we have another chance
		...
		if ( lwarn ) call vbfffwarn(71,ier,del2,s1)
	    endif
	endif
*###] vbfffdel2:
	end
*###[ vbfffdl2p:
	subroutine vbfffdl2p(delps1,xpi,dpipj,piDpj,
...
*	NO possibility
	delps1 = som
	if ( lwarn ) call vbfffwarn(92,ier,delps1,xmax)
	if ( lwrite ) then
	    print *,'xpi = ',xpi
...
*###[ vbfffdl2s:
	subroutine vbfffdl2s(delps1,xpi,piDpj,in,jn,jin,isji,
...
*  #[ check input:
	if ( ltest ) then
	    if ( abs(isji) .ne. 1 ) print *,'vbfffdl2s: error: abs(isji) ',
     +		' <> 1 but ',isji
	    if ( abs(islk) .ne. 1 ) print *,'vbfffdl2s: error: abs(islk) ',
     +		' <> 1 but ',islk
	endif
*  #] check input:
*  #[ stupid tree:
...
*	    and the ii's
   ...
   25	continue
	delps1 = som
	if ( lwarn ) call vbfffwarn(83,ier,delps1,smax)
   30	continue
...
*###[ vbfffdl2t:
	subroutine vbfffdl2t(delps,piDpj,in,jn,kn,ln,lkn,islk,iss,ns,ier)
***#[*comment:***********************************************************
*									*
*	calculate in a numerically stable way				*
*									*
...
*  #[ check input:
	if ( ltest ) then
	    if ( abs(islk) .ne. 1 )
     +		print *,'vbfffdl2i: error: |islk| != 1 ',islk
	    if ( abs(iss) .ne. 1 )
     +		print *,'vbfffdl2i: error: |iss| != 1 ',iss
     ...
     +			piDpj(lkn,i)
		...
		if ( xlosn*abs(xnul) .gt. precx*smax ) then
		    print *,'vbfffdl2t: error: dotproducts ',islk*iss*ln,
...
*
	delps = som
	if ( lwarn ) call vbfffwarn(93,ier,delps,smax)
...
*###[ vbfffdl3m:
	subroutine vbfffdl3m(del3mi,ldel,del3,del2,xpi,dpipj,piDpj,ns,ip1n,
...
*  #] initialisations:
*  #[ superfluous code:
*	if ( ns .ne. 6 ) print *,'vbfffdl3m: called with ns <> 6 !!'
*	if ( ip1n .lt. 4 ) then
*	    lsign = .TRUE.
*	else
*	    lsign = .FALSE.
...
*	    goto 3
*    2	    continue
*	    print *,'vbfffdl3m: unexpected combination of indices',ip1,ip2,
*     +					ip3
*    3	    continue
*	endif
*	this went at he end:
...
*nor does this
*		if ( j .eq. 1 )
*     +			call vbfffdel2(del2s,piDpj,6,ip1,ip2,ip3,1,ier)
*		call vbfffdl2t(delps,piDpj,isi,ip2,ip1,ip2,ip3,+1,+1,6,ier)
...
*
	    som = xsom
	    smax = xmax
	    if ( lwarn ) call vbfffwarn(75,ier,som,smax)
	    if ( lwrite ) then
		print *,'vbfffdl3m: giving up:'
