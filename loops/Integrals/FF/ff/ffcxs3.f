*###[ vbfffcxs3:
	subroutine vbfffcxs3(cs3,ipi12,y,z,dyz,d2yzz,dy2z,xpi,piDpj,ii,ns,
     +					isoort,ier)
***#[*comment:***********************************************************
*									*
...
*  #[ get counters:
	if ( ltest .and. ns .ne. 6 )
     +	    print *,'vbfffcxs3: error: only for ns=6, not ',ns
	ip = ii+3
...
*
	    if ( lwrite ) then
		print *,'vbfffcxs3: special case |z| >> |y|'
		print *,'  y,y1  = ',y(2),y(4)
		print *,'  z,z1- = ',z(1),z(3)
		print *,'  z,z1+ = ',z(2),z(4)
	    endif
     ...
     +		call vbfffwarn(44,ier,abs(zz),x1)
   ...
   10	    continue
     ...
     +		call vbfffwarn(44,ier,abs(zz),x1)
	    ...
	    call vbfffcxr(cs3(8),ipi12(2),yy,yy1,zz,zz1,dyyzz,.FALSE.,x0,
     +			x0,x0,.FALSE.,x00,0,ier)
	    do 20 i=8,14
   20		cs3(i) = -cs3(i)
   30	    continue
*	    And now the remaining Li_2(x^2) terms
	    call vbfffxli2(xdilog,xlog,(y(2)/dyz(2,1))**2,ier)
	    cs3(15) = +xdilog/2
	    call vbfffxli2(xdilog,xlog,(y(4)/dyz(2,1))**2,ier)
	    cs3(16) = -xdilog/2
	    if ( lwrite ) then
		lwrite = .FALSE.
		ipi12p(1) = 0
		...
		do 40 i=1,14
   40		    cs3p(i) = 0
		call vbfffcxr(cs3p(1),ipi12p(1),y(2),y(4),z(1),z(3),
     +		    dyz(2,1),.FALSE.,x0,x0,x0,.FALSE.,x00,ieps(1),ier0)
		call vbfffcxr(cs3p(8),ipi12p(2),y(2),y(4),z(2),z(4),
...
*		both roots are equal: multiply by 2
   ...
   60		continue
		...
		call vbfffcxr(cs3(8),ipi12(2),y(2),y(4),z(2),z(4),dyz(2,2),
...
*###[ vbfffcs3:
	subroutine vbfffcs3(cs3,ipi12,cy,cz,cdyz,cd2yzz,cpi,cpiDpj,ii,ns,
     +		isoort,ier)
***#[*comment:***********************************************************
*									*
*	calculates the s3 as defined in appendix b.			*
...
*									*
*	calls:	vbfffcrr,DIMAG,DBLE,vbfzfflog					*
*									*
***#]*comment:***********************************************************
*  #[ declarations:
	implicit none
...
*  #[ get ieps:
	if ( ltest ) then
	    if ( ns .ne. 6 ) then
		print *,'vbfffcs3: error: only for ns=6, not ',ns
		stop
	    endif
	endif
	ip = ii+3
	call vbfffieps(ieps,cz(1),cpi(ip),cpiDpj(ip,ii),isoort)
	i2 = mod(ii,3) + 1
	i3 = mod(i2,3) + 1
*  #] get ieps:
*  #[ special case |cz| >> |cy|:
...
*		    the answer is rounded off to zero
		    if (lwarn) call vbfffwarn(42,ier,absc(cy(2)),xclogm)
		endif
	    else
		czz = cz(2)*cyy/cy(2)
     ...
     +		call vbfffwarn(43,ier,absc(czz),x1)
...
*		do not know the im part
	    ...
	    call vbfffcrr(cs3(1),ipi12(1),cyy,cyy1,czz,czz1,cdyyzz,.FALSE.,
...
*		    the answer is rounded off to zero
		    if (lwarn) call vbfffwarn(42,ier,absc(cy(4)),xclogm)
		endif
	    else
		czz = cz(4)*cyy/cy(4)
	    ...
	    if ( lwarn .and. absc(czz) .lt. xloss )
     +		call vbfffwarn(43,ier,absc(czz),x1)
	    call vbfffcrr(cs3(8),ipi12(2),cyy,cyy1,czz,czz1,cdyyzz,.FALSE.,
     +		c0,c0,c0,-1,ieps0,ier)
	    do 40 i=8,14
		cs3(i) = -cs3(i)
   40	    continue
...
*
	    ...
	    call vbfffzli2(zdilog,zlog,c,.FALSE.,ier)
	    cs3(15) = +zdilog/2
*	    stupid Gould NP1
		...
		do 60 i=1,14
		    cs3p(i) = 0
   60		continue
		call vbfffcrr(cs3p(1),ipi12p(1),cy(2),cy(4),cz(1),
     +			cz(3),cdyz(2,1),.TRUE.,cd2yzz,cz(2),
     +			cz(4),isoort(1),ieps(1),ier0)
		call vbfffcrr(cs3p(8),ipi12p(2),cy(2),cy(4),cz(2),
...
*	    do nothing
	elseif ( mod(isoort(1),10).eq.0 .or. mod(isoort(1),10).eq.-1
     +		.or. mod(isoort(1),10).eq.-3 ) then
	    call vbfffcrr(cs3(1),ipi12(1),cy(2),cy(4),cz(1),cz(3),
	...
	elseif ( mod(isoort(1),10) .eq. -5 .or. mod(isoort(1),10) .eq.
     +		-6 ) then
	    ...
	    sprec = precx
	    precx = precc
	    call vbfffcxr(cs3(1),ipi12(1),y,y1,z,z1,dyz,ld2yzz,d2yzz,zz,zz1
     +				,.FALSE.,x00,ieps(1),ier)
	    precx = sprec
	else
	    call vbffferr(12,ier)
	endif
	if ( isoort(2) .eq. 0 ) then
*	    do nothing
	elseif ( mod(isoort(2),5) .eq. 0 ) then
	    if ( lwrite ) print *,'vbfffcs3: skipped next R as it is the ',
     +		'conjugate'
	    do 100 i=1,7
  100		cs3(i) = 2*DBLE(cs3(i))
	...
	elseif ( mod(isoort(2),10).eq.-1 .or. mod(isoort(1),10).eq.-3 )
     +		then
	    call vbfffcrr(cs3(8),ipi12(2),cy(2),cy(4),cz(2),cz(4),
	...
	elseif ( mod(isoort(2),10) .eq. -6 ) then
	    ...
	    sprec = precx
	    precx = precc
	    call vbfffcxr(cs3(8),ipi12(2),y,y1,z,z1,dyz,ld2yzz,d2yzz,zz,zz1
     +				,.FALSE.,x00,ieps(2),ier)
	    precx = sprec
	else
	    call vbffferr(13,ier)
	endif
*  #] normal:
*  #[ eta's:
	if ( mod(isoort(1),10).eq.-5 .or. mod(isoort(1),10).eq.-6 )
     ...
     +			) then
		print *,'vbfffcxs3: error: I assumed both would be real!'
		ier = ier + 50
	    endif
*	    we called vbfffcxr - no eta's
	elseif ( DIMAG(cpi(ip)).eq.0 ) then
	    call vbfffgeta(ni,cz(1),cdyz(1,1),cd2yzz,
     +		cpi(ip),cpiDpj(ii,ip),ieps,isoort,ier)
	    if ( lwrite ) print *,'vbfffcs3: eta''s are ',ni
	    ntot = ni(1) + ni(2) + ni(3) + ni(4)
	    if ( ntot .ne. 0 ) call vbfffclgy(cs3(15),ipi12(2),ntot,
...
*###[ vbfffclgy:
	subroutine vbfffclgy(cs3,ipi12,ntot,cy,cz,cd2yzz,ier)
***#[*comment:***********************************************************
*									*
...
*
	integer ipi
	DOUBLE COMPLEX c,cc,clogy,c2y1,vbfzfflog,vbfzfflo1,csum
...
*  #[ calculations:
	...
	if ( 1 .lt. xloss*absc(cy(2)) ) then
     ...
     +			then
		if ( ntot .ne. 0 ) call vbffferr(15,ier)
		clogy = 0
	    else
		c = -cy(4)/cy(2)
...
*		    take out the factor 2*pi^2
     ...
     +				absc(cz(2)),absc(cz(3))) ) call vbfffwarn(
     +				56,ier,absc(c2y1),absc(cy(2)))
			endif
			csum = -c2y1/cy(2)
			clogy = vbfzfflo1(csum,ier)
			if ( lwrite ) then
			    print *,'c  = ',c
			    print *,'c+ = ',-1+csum
			endif
	...
	if ( ltest .and. cs3 .ne. 0 ) then
	    print *,'vbfffclgy: error: cs3 al bezet! ',cs3
	endif
...
*###[ vbfffieps:
	subroutine vbfffieps(ieps,cz,cp,cpDs,isoort)
***#[*comment:***********************************************************
*									*
...
*###[ vbfffgeta:
	subroutine vbfffgeta(ni,cz,cdyz,cd2yzz,cp,cpDs,ieps,isoort,ier)
***#[*comment:***********************************************************
*									*
...
*
	if ( lwrite ) print *,'vbfffgeta: isoort = ',isoort
*
...
*
		...
		ni(2) = vbfnffet1(-cz(1),-cz(2),cmip,ier)
		if ( cz(3).ne.0 .and. cz(4).ne.0 ) then
		    i = vbfnffet1(cz(3),cz(4),cmip,ier)
		    if ( i .ne. ni(2) ) call vbffferr(53,ier)
		endif
	    endif
*
...
*		follow the i*epsilon prescription as (y-z-)(y-z+) real
     ...
     +		    .eq.0 ) print *,'vbfffgeta: error: calling vbfnffet1',
     +		    ' with im(y-z-)=im(y-z+)=0: ',cdyz(2,1),cdyz(2,2)
		endif
		ni(4) = -vbfnffet1(cdyz(2,1),cdyz(2,2),cmip,ier)
	    else
     ...
     +			cdyz(2,2)) .lt. 0 ) then
		...
		ni(4) = -vbfnffeta(cdyz(2,1),cdyz(2,2),ier)
	    endif
	elseif ( (mod(isoort(1),10).eq.-1 .or. mod(isoort(1),10).eq.-3)
     +		.and. isoort(2) .eq. 0 ) then
	    ni(1) = 0
	    if ( DIMAG(cz(1)) .ne. 0 ) then
		ni(2) = vbfnffet1(-cpDs,-cz(1),DCMPLX(DBLE(0),
     +			DBLE(-1)),ier)
	    else
		ni(2) = vbfnffet1(-cpDs,DCMPLX(DBLE(0),DBLE(1)),
     +			DCMPLX(DBLE(0),DBLE(-1)),ier)
	    endif
	    ni(3) = 0
	    ni(4) = -vbfnffeta(-cpDs,cdyz(2,1),ier)
	else
	    ni(1) = 0
	    ni(2) = 0
	    ni(3) = 0
