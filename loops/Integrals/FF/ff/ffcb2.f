...
*  #[ get differences:
	...
	if ( lwarn ) then
     ...
     +		call vbfffwarn(97,ier0,absc(dmamb),absc(xma))
	    if ( absc(dmap) .lt. xloss*absc(xp) .and. xp .ne. xma )
     +		call vbfffwarn(98,ier0,absc(dmap),absc(xp))
	    if ( absc(dmbp) .lt. xloss*absc(xp) .and. xp .ne. xmb )
     +		call vbfffwarn(99,ier0,absc(dmbp),absc(xp))
	endif
*  #] get differences:
*  #[ call ffcb2a:
...
*
	...
	DOUBLE COMPLEX delsp,xlam,xlo3,xlogmm,vbfzfflo1,vbfzfflo3
...
*  #[ test input:
	if ( ltest ) then
	    ier0 = ier
	    call vbfffcot2(qiDqj,xp,xma,xmb,dmap,dmbp,dmamb,ier0)
...
*  #[ p^2 != 0:
	if ( xp .ne. 0 ) then
* 	#[ normal case:
	    call vbfffclmb(xlam,-xp,-xmb,-xma,dmbp,dmap,dmamb,ier)
	    delsp = -xlam/4
*
...
*
	    ...
	    if ( absc(cb2p) .lt. xloss*absc(cs(2)) ) then
	    	if ( lwarn ) call vbfffwarn(214,ier,absc(cb2p),absc(cs(2)))
	    endif
*
*	    the next one ain't.
*
	    ...
	    if ( lwarn ) then
		call vbfffwarn(214,ier,absc(cb2d),xmax)
		if ( lwrite ) then
...
*
		if ( absc(dmamb).lt.xloss*absc(xma) ) then
		    xlogmm = vbfzfflo1(dmamb/xma,ier)
		else
		    xlogmm = log(xmb/xma)
		endif
		llogmm = .TRUE.
...
*
		if ( lwarn ) then
		    call vbfffwarn(227,ier,absc(cb21),xmax)
		    if ( lwrite ) then
			print *,'xp,xma,xmb = ',xp,xma,xmb
		    endif
		endif
...
*
		if ( lwarn ) then
		    call vbfffwarn(214,ier,absc(cb22),xmax)
		    if ( lwrite ) then
			print *,'xp,xma,xmb = ',xp,xma,xmb
		    endif
		endif
