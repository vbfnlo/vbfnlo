*	$Id: vbfffcc0.f,v 1.2 1996/06/30 19:03:55 gj Exp $
*###[ vbfffcc0:
	subroutine vbfffcc0(cc0,cpi,ier)
***#[*comment:***********************************************************
*									*
...
*
   ...
   10	    continue
	elseif ( init .eq. 0 ) then
	    init = 1
	    print *,'vbfffcc0: disregarding complex masses, nschem= ',
     +			nschem
	endif
	do 20 i = 1,6
	    xpi(i) = DBLE(cpi(i))
   20	continue
	sprecx = precx
	precx = precc
	call vbfffxc0(cc0,xpi,ier)
	precx = sprecx
	if ( ldot ) call vbfffcod3(cpi)
	return
   30	continue
*
*  #] the real case:
...
*
	...
	if ( ltest ) then
	    do 34 i=1,3
		if ( DIMAG(cpi(i)) .gt. 0 ) call vbffferr(49,ier)
   34	    continue
	    do 35 i=4,6
		if ( DIMAG(cpi(i)) .ne. 0 ) call vbffferr(49,ier)
   35	    continue
	endif
	if ( lwrite ) then
	    print *,'vbfffcc0: input = ',cpi
	endif
*
*  #] check input:
*  #[ convert input:
	if ( lwarn ) then
     ...
     +				cpi(i) .ne. cpi(j) ) then
			ier0 = 0
			call vbfffwarn(86,ier0,absc(cdpipj(i,j)),
     +							absc(cpi(i)))
		    endif
...
*  #] convert input:
*  #[ call vbfffcc0a:
	call vbfffcc0a(cc0,cpi,cdpipj,ier)
*  #] call vbfffcc0a:
*  #[ check output:
  ...
  930	    continue
     ...
     +		xmax = xmax + absc((cpi(3)/DBLE(xpi(3))-1)**2)
		...
		print *,'vbfffcc0: result is very different from the real',
...
*###[ vbfffcc0r:
	subroutine vbfffcc0r(cc0,cpi,ier)
***#[*comment:***********************************************************
*									*
...
*###[ vbfffcc0a:
	subroutine vbfffcc0a(cc0,cpi,cdpipj,ier)
***#[*comment:***********************************************************
*									*
*	see vbfffcc0							*
*									*
...
*  #[ handel special cases:
    ...
    4	    continue
	    sprec = precx
	    precx = precc
	    if ( lwrite ) print *,'vbfffcc0a: real masses, calling vbfffxc0a'
	    call vbfffxc0a(cc0,xqi,dqiqj,ier)
	    precx = sprec
	    if ( ldot ) call vbfffcod3(cpi)
	    return
	endif
...
*     +		.ne. 0 ) then
*	    call vbfffcb0p(cs1,-cpi(5),cpi(1),cpi(3),cdpipj(1,6),
*     +					cdpipj(3,5),cdpipj(1,3),ier)
*	    call vbfffcb0p(cs2,-cpi(5),cpi(2),cpi(3),cdpipj(2,5),
*     +					cdpipj(3,5),cdpipj(2,3),ier)
*	    cs = cs1 - cs2
*	    cc0 = cs/cdpipj(1,2)
*	elseif ( cpi(6) .eq. 0 .and. cdpipj(4,5) .eq. 0 .and.
*     +		cdpipj(3,1) .ne. 0 ) then
*	    call vbfffcb0p(cs1,-cpi(4),cpi(3),cpi(2),cdpipj(3,5),
*     +					cdpipj(2,4),cdpipj(3,2),ier)
*	    call vbfffcb0p(cs2,-cpi(4),cpi(1),cpi(2),cdpipj(1,4),
*     +					cdpipj(2,4),cdpipj(1,2),ier)
*	    cs = cs1 - cs2
*	    cc0 = cs/cdpipj(3,1)
*	elseif ( cpi(5) .eq. 0 .and. cdpipj(6,4) .eq. 0 .and.
*     +		cdpipj(2,3) .ne. 0 ) then
*	    call vbfffcb0p(cs1,-cpi(6),cpi(2),cpi(1),cdpipj(2,4),
*     +					cdpipj(1,6),cdpipj(2,1),ier)
*	    call vbfffcb0p(cs2,-cpi(6),cpi(3),cpi(1),cdpipj(3,6),
...
**
*	print *,'vbfffcc0: WARNING: this algorithm has not yet been tested'
*	if ( absc(cs) .lt. xloss*absc(cs1) )
*     +		call vbfffwarn(26,ier,absc(cs),absc(cs1))
**
*	debug output
**
*	if (lwrite) then
...
*  #[ rotate to alpha in (0,1):
	call vbfffcrt3(irota,cqi,cqiqj,cpi,cdpipj,6,2,ier)
*  #] rotate to alpha in (0,1):
*  #[ look in memory:
	ierini = ier+ner
	if ( lmem ) then
...
*		(maybe check differences as well)
		if ( lwrite ) print *,'vbfffcc0: using previous result'
		cc0 = cc0mem(i)
		ier = ier+iermem(i)
		if ( ldot ) then
		    fodel2 = dl2mem(i)
...
*		    we forgot to recalculate the stored quantities
		...
		return
   70	    continue
*	    if ( lwrite ) print *,'vbfffcc0: not found in memory'
	endif
   71	continue
*  #] look in memory:
*  #[ dot products:
	call vbfffcot3(cqiDqj,cqi,cqiqj,6,ier)
*
...
*  #[ handle poles-only approach:
	...
	if (  nschem.le.6 ) then
		...
		if ( nschem.eq.1 .or. nschem.eq.2 ) then
		    print *,'vbfffcc0a: disregarding all complex masses'
		elseif ( nschem.eq.3 ) then
		    print *,'vbfffcc0a: undefined nschem=3'
		elseif ( nschem.eq.4 ) then
		    print *,'vbfffcc0a: using the scheme in which ',
     ...
     +		       'there is a singular log'
		elseif ( nschem.eq.7 ) then
		    print *,'vbfffcc0a: using complex masses'
		endif
		if ( nschem.ge.3 ) then
		    print *,'vbfffcc0a: switching to complex when ',
...
*
		i2 = mod(i1,3)+1
		i3 = mod(i2,3)+1
		call vbfffbglg(ifound,cqi,cqiqj,cqiDqj,6,i1,i2,i3,i1+3,
     +			i3+3)
		if ( ifound .ne. 0 ) goto 11
   10	    continue
	    if ( lwrite ) print *,'vbfffcc0a: no large logs'
	    i1 = 0
   11	    continue
...
*
	    do 20 i=1,3
		i2 = mod(i,3)+1
		call vbfffthre(ithres(i),cqi,cqiqj,6,i,i2,i+3)
   20	    continue
*
...
*
   21	    continue
*
*	    The infrared divergent diagrams are calculated in vbfffxc0i:
*
     ...
     +			) then
		call vbfffxc0i(cc0,xqi,dqiqj,ier)
	    else
		call vbfffxc0b(cc0,xqi,dqiqj,qiDqj,ier)
	    endif
...
*
   30	    continue
	    precx = sprec
	    call vbfffcc0b(cc0,cqi,cqiqj,cqiDqj,ier)
   31	    continue
*
*  #] handle poles-only approach:
*  #[ call vbfffcc0b:
...
*###[ vbfffcc0b:
	subroutine vbfffcc0b(cc0,cqi,cqiqj,cqiDqj,ier)
***#[*comment:***********************************************************
*									*
*	see vbfffcc0							*
*									*
...
*
  ...
  103	    continue
  104	continue
	call vbfffdel2(del2,qiDqj,6,4,5,6,1,ier)
	if ( lwrite ) print *,'vbfffcc0: del2 = ',del2
	fodel2 = del2
	fdel2 = fodel2
	cel2 = DCMPLX(DBLE(del2))
*	if ( lwrite ) print *,'vbfffcc0: calling vbfffcel3'
	call vbfffcel3(cel3,cqi,cqiDqj,6,ier)
	if ( DIMAG(cel3).ne.0 .and. 
     +		abs(DIMAG(cel3)).lt.precc*abs(DBLE(cel3)) ) then
	    if ( lwrite ) print *,'vbfffcc0b: rounded cel3 from ',cel3
	    cel3 = DBLE(cel3)
	    if ( lwrite ) print *,'to ',cel3
	endif
*	if ( lwrite ) print *,'vbfffcc0: calling vbfffcl3m'
	call vbfffcl3m(cel3mi,.TRUE.,cel3,cel2,cqi,cqiqj,cqiDqj,6, 4,5,6,
     +							1,3,ier)
	do 105 i=1,3
	    j = i+1
	    if ( j .eq. 4 ) j = 1
*	    if ( lwrite ) print *,'vbfffcc0: calling vbfffcel2'
	    call vbfffcel2(cel2s(i),cqiDqj,6,i+3,i,j,1,ieri(i))
	    k = i-1
	    if ( k .eq. 0 ) k = 3
*	    if ( lwrite ) print *,'vbfffcc0: calling vbfffcl2p'
	    call vbfffcl2p(celpsi(i),cqi,cqiqj,cqiDqj,i+3,j+3,k+3,i,j,k,6,
     +							ieri(i+3))
  105	continue
...
*	get alpha,1-alpha
*
	call vbfffcoot(cblph(1),calph(1),cqi(5),-cqiDqj(5,6),cqi(6),csdel2,
     +								ier)
	call vbfffcoot(calph(3),cblph(3),cqi(5),-cqiDqj(5,4),cqi(4),csdel2,
     +								ier)
	cs1 = cblph(1) - c05
	cs2 = calph(1) - c05
...
*	    call vbfffclmb(clamp,cqi(4),cqi(5),cqi(6),cqiqj(4,5),
*     +					cqiqj(4,6),cqiqj(5,6),ier)
*	    print *,'cslamp  =',sqrt(clamp)
	    print *,'ceta    =',-4*cel3
*	    ier0 = 0
*	    call vbfffceta(ceta,cpi,cdpipj,6,ier0)
*	    print *,'cetap   =',ceta
	    print *,'cetalm  =',cetalm
...
*
	call vbfffcc0p(cs3,ipi12,isoort,clogi,ilogi,cqi,cqiqj,cqiDqj,
...
*
	if ( lwarn .and. 2*absc(cs) .lt. xloss*xmax )
     +		call vbfffwarn(27,ier,absc(cs),xmax)
*
...
*###[ vbfffcrt3:
	subroutine vbfffcrt3(irota,cqi,cdqiqj,cpi,cdpipj,ns,iflag,ier)
***#[*comment:***********************************************************
*									*
...
*  #[ test output:
	if ( ltest ) then
	    ier0 = 0
	    call vbfffchck(cqi,cdqiqj,6,ier0)
	    if ( ier0 .ne. 0 ) print *,'vbfffcrt3: error: momenta wrong'
	endif
*  #] test output:
*###] vbfffcrt3:
	end
*###[ vbfffcot3:
	subroutine vbfffcot3(cpiDpj,cpi,cdpipj,ns,ier)
***#[*comment:***********************************************************
*									*
...
*
     ...
     +			call vbfffwarn(100,ier0,absc(cpiDpj(is1,is2)),
...
*
     ...
     +			call vbfffwarn(101,ier,absc(cpiDpj(ip1,is1)),
...
*
     ...
     +			call vbfffwarn(102,ier,absc(cpiDpj(ip1,is2)),
...
*
     ...
     +			vbfffwarn(103,ier,absc(cpiDpj(ip1,is3)),min(absc(
...
*
     ...
     +			call vbfffwarn(104,ier,absc(cpiDpj(ip1,ip2)),
...
*  #[ check:
	if ( ltest ) then
     ...
     +	        	'vbfffcot3: error: dotproducts with p(',i,
     +	        	') wrong: ',check
   20	    continue
	endif
*  #] check:
...
*###[ vbfffbglg:
	subroutine vbfffbglg(ifound,cqi,cqiqj,cqiDqj,ns,i1,i2,i3,ip1,ip3)
***#[*comment:***********************************************************
*									*
...
*  #[ work:
	...
	if ( abs(DBLE(cqi(i1))) .lt. -xloss*(DIMAG(cqi(i2)) +
     ...
     +		) then
	    if ( lwrite ) then
		print *,'vbfffbglg: found large logs in ',i1,i2,i3
	...
	if ( nschem.ge.5 .and. cqi(i1).eq.0 ) then
     ...
     +			cqiDqj(i1,i3)*cqiDqj(ip1,i2)
		if ( lwrite ) then
		    print *,'vbfffbglg: examining ',i1,i2,i3
		    print *,'  cel3             = ',cel3
...
*###[ vbfffthre:
	subroutine vbfffthre(ithres,cqi,cqiqj,ns,i1,i2,ip)
***#[*comment:***********************************************************
*									*
...
**     +		call vbfffwarn(182,ier0,x1,x1)
	...
	if ( lwrite .and. ithres .ne. 0 )
     +		print *,'vbfffthre: threshold in vertex ',i1,i2,ip,': ',
     +		ithres,xlam,cqi(i1),cqi(i2),cqi(ip)
*  #] work:
*###] vbfffthre:
	end
*###[ vbfffcod3:
	subroutine vbfffcod3(cpi)
***#[*comment:***********************************************************
*									*
...
*		/vbfffdots/fpij3(6,6)	real		p.p real	*
*									*
...
*
	if ( lwrite ) then
	    print *,'vbfffcod3: converting real to complex dotproducts'
	endif
*
*  #] print info:
*  #[ add widths:
