*###[ vbfffxxyz:
	subroutine vbfffxxyz(y,z,dyz,d2yzz,dy2z,ivert,sdel2p,sdel2s,etalam,
...
*  #[ xk = 0:
	if ( xpi(ip1) .eq. 0 ) then
		...
		isoort(1) = 0
		if ( lwrite ) print *,'vbfffxxyz: xk=0, ma=mb -> S3 =0'
		return
	    endif
	    isoort(1) = 1
...
*		check whether we have the correct root ...
     ...
     +			is2)),abs(disc)) ) call vbffferr(37,ier)
		...
		if ( lwarn .and. abs(dy2z(i)) .lt. xloss*smax ) then
		    ier0 = ier
		    call vbfffwarn(152,ier0,dy2z(i),smax)
		    ier1 = max(ier1,ier0)
...
*  #[ get y(1,2),z(1,2):
	if ( sdel2s .eq. 0 ) then
	    ...
	    isoort(1) = 1
	    isoort(2) = 1
	    call vbfffroot(z(1),z(2),xpi(ip1),piDpj(ip1,is2),xpi(is2),
     +							sdel2s,ier)
	endif
*	if ( ltest ) then
*	    call vbfffdl2p(delps1,xpi,dpipj,piDpj,
*     +		ip1,ip2,ip3,is1,is2,is3,ns,ier)
*	    if ( delps .ne. delps1 ) print *,'vbfffxxyz: error: delps != ',
*     +		'delps1: ',delps,delps1
*	endif
	disc = delps/sdel2p
	ier0 = ier
	call vbfffroot(y(1),y(2),xpi(ip1),piDpj(ip1,is2),etami(is2),disc,
     +								ier)
*  #] get y(1,2),z(1,2):
*  #[ get y(3,4),z(3,4):
...
*	else
     ...
     +		    call vbfffroot(z(4),z(3),xpi(ip1),-piDpj(ip1,is1),
     +						xpi(is1),sdel2s,ier)
	    endif
	    y(3) = 1-y(1)
	    y(4) = 1-y(2)
	    if ( abs(y(3)) .lt. xloss .or. abs(y(4)) .lt. xloss ) then
		call vbfffroot(y(4),y(3),xpi(ip1),-piDpj(ip1,is1),
     +						etami(is1),disc,ier)
	    endif
*	endif
*  #] get y(3,4),z(3,4):
...
*
*	optimized by Maple (see vbfffxxyz.map)
*
	t1 = xpi(ip1)
	t2 = xpi(ip2)
	t3 = t1*t2
...
*
   ...
   30		continue
     ...
     +			(y(3)-2*z(3))*(y(3)-2*z(4)))
		...
		ier0 = ier
		call vbfffwarn(152,ier0,dy2z(iwarn),xmax)
		ier1 = max(ier1,ier0)
		if ( lwrite ) then
		    print *,'n = ',n
		    print *,'xpi = ',xpi
...
*
	...
	if ( sdel2s .eq. 0 ) then
	    ...
	    isoort(1) = 101
	    isoort(2) = 101
	    call vbfffroot(z(1),z(2),xpi(3),piDpj(is1,3),xpi(is1),hulp,ier)
	endif
	call vbfffroot(y(1),y(2),xpi(3),piDpj(is1,3),etami(is1),disc,ier)
*  #] get ypm,zpm:
*  #[ get ypm1,zpm1:
	z(3) = 1 - z(1)
	z(4) = 1 - z(2)
	if ( abs(z(3)).lt.xloss .or. abs(z(4)).lt.xloss ) then
	    ...
	    if ( ivert.eq.2 ) then
		call vbfffroot(z(4),z(3),xpi(3),piDpj(ip1,3),xpi(ip1),hulp,
     +								ier)
	    else
		call vbfffroot(z(4),z(3),xpi(3),-piDpj(ip1,3),xpi(ip1),hulp
     +								,ier)
	...
	if ( abs(y(3)) .lt. xloss .or. abs(y(4)) .lt. xloss ) then
	    ...
	    if ( ivert .eq. 2 ) then
		call vbfffroot(y(4),y(3),xpi(3),piDpj(ip1,3),etami(ip1),
     +							disc,ier)
	    else
		call vbfffroot(y(4),y(3),xpi(3),-piDpj(ip1,3),etami(ip1),
     +							disc,ier)
...
*		if ( lwrite ) print *,'  dy2z(',i,')+= ',s,y(j-1)
		if ( lwarn .and. abs(dy2z(i)) .lt. xloss*smax ) then
		    call vbfffwarn(152,ier,dy2z(i),abs(y(j-1)))
		endif
*	    endif
  220	continue
*  #] get dypzp, dypzm:
...
*###[ vbfffdwz:
	subroutine vbfffdwz(dwz,w,z,i1,j1,l,alpha,alph1,xpi,dpipj,piDpj,
     +							sdel2i,ns,ier)
***#[*comment:***********************************************************
*									*
...
*  #[ calculations:
	if ( l .eq. 1 ) then
	    if ( lwrite ) print *,'vbfffdwz:  warning: cannot handle',
...
*		    vbfffxc0p)
		    if ( lwrite ) then
			call vbfffwarn(79,ier,sum,xmax)
		    else
			ier = ier + 1
		    endif
...
*  #[ test output:
	if ( ltest .and. ier .eq. 0 ) then
     ...
     +		'vbfffdwz:  error: dwz(',i1,j1,l,') <> w - z :',
     ...
     +		'vbfffdwz:  error: dwz(',i1,j1,l,') <> z1 - w1 :',
