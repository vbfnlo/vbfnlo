*###[ vbfffdel3:
	subroutine vbfffdel3(del3,xpi,piDpj,ns,ier)
***#[*comment:***********************************************************
*									*
...
*  #[ calculations:
   ...
   20	continue
	if ( abs(del3p) .lt. xloss*xmaxp ) then
		...
		if ( lwarn ) call vbfffwarn(73,ier,del3,xmax)
		goto 800
	    endif
	    goto 10
	endif
...
*  #[ check output:
  ...
  820	    continue
		...
		print *,'vbfffdel3: error: result does not agree with',
     +			' normal case'
...
*(##[ vbfffdl3s:
	subroutine vbfffdl3s(dl3s,xpi,piDpj,ii,ns,ier)
***#[*comment:***********************************************************
*									*
...
*  #[ test input:
	if ( ltest ) then
*	    print *,'vbfffdl3s: input: ii(,1) = ',(ii(i,1),i=1,6)
*	    print *,'               ii(,2) = ',(ii(i,2),i=1,6)
	    xlosn = xloss*DBLE(10)**(-mod(ier,50))
	    do 3 j=1,2
	    do 1 i=1,6
		if ( abs(ii(i,j)) .gt. ns ) print *,'vbfffdl3s: error: ',
     +			'|ii(i,j)| > ns: ',ii(i,j),ns
		if ( abs(ii(i,j)) .eq. 0 ) print *,'vbfffdl3s: error: ',
     +			'|ii(i,j)| = 0: ',ii(i,j)
    1	    continue
	    do 2 i=1,6
     ...
     +			   abs(piDpj(abs(ii(i,j)),ii(2,j))))
		if ( xlosn*abs(xhck) .gt. precx*xmax ) print *,'vbfffdl3s:'
...
*  #[ calculations:
   ...
   20	continue
	if ( abs(dl3sp) .lt. xloss*xmaxp ) then
	    ...
	    if ( inow .gt. nperm ) inow = 1
	    if ( inow .eq. imem ) then
		if ( lwarn ) call vbfffwarn(85,ier,dl3s,xmax)
		goto 800
	    endif
	    goto 10
	endif
...
*  #[ check output:
  ...
  820	    continue
		...
		print *,'vbfffdl3s: error: result does not agree with',
     +			' normal case'
