*###[ vbfffcb1:
	subroutine vbfffcb1(cb1,cb0,ca0i,xp,xm1,xm2,piDpj,ier)
***#[*comment:***********************************************************
*									*
...
*  #[ real case:
   ...
   20	    continue
	    sprec = precx
	    precx = precc
	    call vbfffxb1(cb1,cb0,ca0i,rp,rm1,rm2,rpiDpj,ier)
	    precx = sprec
	    return
	endif
*  #] real case:
*  #[ get differences:
	...
	if ( lwarn ) then
     ...
     +		call vbfffwarn(97,ier0,absc(dm1m2),absc(xm1))
	    if ( abs(dm1p) .lt. xloss*abs(xp) .and. xp .ne. xm1 )
     +		call vbfffwarn(98,ier0,absc(dm1p),absc(xp))
	    if ( abs(dm2p) .lt. xloss*abs(xp) .and. xp .ne. xm2 )
     +		call vbfffwarn(99,ier0,absc(dm2p),absc(xp))
	endif
*  #] get differences:
*  #[ call vbfffcb1a:
	call vbfffcb1a(cb1,cb0,ca0i,xp,xm1,xm2,dm1p,dm2p,dm1m2,piDpj,ier)
*  #] call vbfffcb1a:
*###] vbfffcb1:
	end
*###[ vbfffcb1a:
	subroutine vbfffcb1a(cb1,cb0,ca0i,xp,xm1,xm2,dm1p,dm2p,dm1m2,piDpj,
     +		ier)
***#[*comment:***********************************************************
*									*
*	Calculate   1	/	   d^n Q Q(mu)				*
...
*
	...
	DOUBLE PRECISION vbfffbnd
	DOUBLE COMPLEX vbfzfflo1,vbfzfflo3
...
*
	if ( DIMAG(xm1) .eq. 0 .and. DIMAG(xm2) .eq. 0 ) then
	    lreal = .TRUE.
	    if ( lwrite ) print *,'vbfffcb1a: real masses'
	elseif ( nschem.le.4 ) then
	    lreal = .TRUE.
	    if ( lwrite .or. init.eq.0 ) then
		init = 1
		print *,'vbfffcb1a: nschem <= 4, ignoring complex masses:',
     +			nschem
	    endif
	elseif ( nschem.le.6 ) then
	    if ( lwrite .or. init.eq.0 ) then
		init = 1
		print *,'vbfffcb1a: nschem = 5,6 complex masses near ',
     +			'threshold: ',nschem
	    endif
	    cqi(1) = xm1
	    cqi(2) = xm2
		...
		lreal = .TRUE.
		if ( lwrite ) print *,'vbfffcb1a: no threshold'
	    else
		if ( lwrite ) print *,'vbfffcb1a: found threshold'
		lreal = .FALSE.
	    endif
	else
	    lreal = .FALSE.
   ...
   20	    continue
	    sprec = precx
	    precx = precc
	    call vbfffxb1a(cb1,cb0,ca0i,rp,rm1,rm2,rm1p,rm2p,rm1m2,rpiDpj,
     +		ier)
	    precx = sprec
	    return
	endif
...
*  #[ check input:
	if ( ltest ) then
     ...
     +		'vbfffcb1a: error: s1.s2 wrong: ',2*piDpj(1,2),xm1+xm2-xp,
     +		xnul,ier
	    xnul = 2*piDpj(1,3) + xm1 - xm2 + xp
	    if ( rloss*absc(xnul) .gt. precc*xmax ) print *,
     +		'vbfffcb1a: error: s1.p wrong: ',2*piDpj(1,3),-xm1+xm2-xp,
     +		xnul,ier
	    xnul = 2*piDpj(2,3) + xm1 - xm2 - xp
	    if ( rloss*absc(xnul) .gt. precc*xmax ) print *,
     +		'vbfffcb1a: error: s2.p wrong: ',2*piDpj(2,3),-xm1+xm2+xp,
     +		xnul,ier
	endif
*  #] check input:
*  #[ p^2 != 0:
...
* 	#[ almost equal masses:
		...
		cs(2) = dm1m2/xm1*cs(2)
		cs(1) = -xm2*vbfzfflo1(-dm1m2/xm2,ier)
...
*
     ...
     +		+xmb*clogm*(DBLE(x05)-(xma+xmb-xp/2)/(slam-xma+xmb))
		...
		xprec = precx
		precx = precc
		bnd101 = vbfffbnd(2,1,xinfac)
		bnd105 = vbfffbnd(2,5,xinfac)
		bnd110 = vbfffbnd(2,10,xinfac)
		bnd115 = vbfffbnd(2,15,xinfac)
		precx = xprec
	    endif
	    x = xp/slam
     ...
     +		call vbfffwarn(220,ier,precc,xinfac(16)*ax**14)
     ...
     +			then
		call vbfffwarn(221,ier,absc(h),max(absc(s),absc(s1)))
	    endif
	    if ( lwrite ) then
		print *,'arg ',h
     ...
     +			slam)
	    ...
	    if ( absc(h) .lt. .1 ) then
		cs(2) = dmbma*slam/xp*vbfzfflo1(h,ier)
	    else
		print *,'vbfffcb1: warning: I thought this was small: ',h
...
*
  100	    continue
	    if ( lwarn ) then
		call vbfffwarn(167,ier,absc(cb1),xmax)
		if ( lwrite ) then
