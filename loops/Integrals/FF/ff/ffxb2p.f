*###[ vbfffxb2p:
	subroutine vbfffxb2p(cb2i,cb1,cb0,ca0i,xp,xm1,xm2,piDpj,ier)
***#[*comment:***********************************************************
*									*
...
*
	...
	dm2p = xm2 - xp
	dm1m2= xm1 - xm2
	call vbfffxb2q(cb2i,cb1,cb0,ca0i,xp,xm1,xm2,dm1p,dm2p,dm1m2,
     +		piDpj,ier)
*
*  #] work:
*###] vbfffxb2p:
...
*###[ vbfffxb2q:
	subroutine vbfffxb2q(cb2i,cb1,cb0,ca0i,xp,xm1,xm2,dm1p,dm2p,dm1m2,
     +		piDpj,ier)
***#[*comment:***********************************************************
*									*
*	Compute the PV B2, the coefficients of p(mu)p(nu) and g(mu,nu)	*
...
*  #[ test input:
	if ( ltest ) then
	    ier0 = ier
	    call vbfffdot2(qiDqj,xp,xm1,xm2,dm1p,dm2p,dm1m2,ier0)
	    rloss = xloss*DBLE(10)**(-mod(ier0,50))
	    do 20 j=1,3
		do 10 i=1,3
		    if ( rloss*abs(piDpj(i,j)-qiDqj(i,j)).gt.precx*
     +			abs(piDpj(i,j))) print *,'vbfffxb2q: error: piDpj('
...
*
		...
		if ( xp.lt.0 ) then
		    ...
		    xlo3 = vbfdfflo3((xp-slam)/(2*xm1),ier)
		    ...
		    clo3 = vbfzfflo3(DCMPLX(DBLE(xp/(2*xm1)),
     +			DBLE(-slam/(2*xm1))),ier)
...
*     +			call vbfffwarn(298,ier0,absc(cb2i(2)),xmax)
*	    endif
...
*		if ( absc(cb2i(2)) .lt. xloss*xmax )
*     +			call vbfffwarn(298,ier1,absc(cb2i(2)),xmax)
*	    endif
*	    cb2i(2) = cb2i(2)/6
*		#] old code:
*		#[ B21:
