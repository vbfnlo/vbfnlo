*###[ vbfff2dl2:
	subroutine vbfff2dl2(del2d2,del2n,xpi,dpipj,piDpj, i,
...
*		del2n			it is needed in vbffftran anyway	*
*									*
***#]*comment:*********************************************************** 
*  #[ declarations:
	implicit none
...
*  #[ check input:
	if ( ltest ) then
	    if ( abs(iskj) .ne. 1 ) print *,'vbfff2dl2: error: abs(iskj) ',
     +		'<> 1 but ',iskj
	    if ( abs(isnm) .ne. 1 ) print *,'vbfff2dl2: error: abs(isnm) ',
     +		'<> 1 but ',isnm
	    if ( ns .ne. 10 ) print *,'vbfff2dl2: error: ns <> 10 !!'
	    if ( kj.eq.0 ) then
		print *,'vbfff2dl2: error: kj=0:j,k,id,idsub=',j,k,id,idsub
	    endif
	    if ( nm.eq.0 ) then
		print *,'vbfff2dl2: error: nm=0:m,n,id,idsub=',m,n,id,idsub
	    endif
	endif
*  #] check input: 
*  #[ get del2n:
*	we need this in any case !
	...
	elseif ( i .le. 4 ) then
	    ii = inx(n,i)
	    isii = isgn(n,i)
	    call vbfffdl2s(del2n,xpi,piDpj,i,n,ii,isii,j,k,kj,iskj,10,ier1)
	else
	    call vbfffdl2t(del2n,piDpj,i,n,j,k,kj,iskj,+1,10,ier1)
	endif
*  #] get del2n:
*  #[ special cases:
	ier0 = ier
	if ( i .eq. l .and. j .eq. m .and. k .eq. n ) then
	    call vbfffdl3m(s,.FALSE.,x0,x0,xpi,dpipj,piDpj,ns,j,k,kj,
     +		i,1,ier0)
	    del2d2 = -s(1)
*	    if ( lwrite ) print *,'del2d2 = ',del2d2
	...
	if ( k .eq. l .and. j .le. 4 ) then
	    call vbfffdl2s(del2m,xpi,piDpj, j,l,inx(l,j),isgn(l,j),
     +		m,n,nm,isnm, 10,ier0)
...
*	    call vbfffdl2s(del2m,xpi,piDpj, k,l,inx(k,j),isgn(k,j),
*     +		m,n,nm,isnm, 10,ier0)
...
*  #[ calculations:
	...
	elseif ( i .le. 4 ) then
	    ii = inx(m,i)
	    isii = isgn(m,i)
	    call vbfffdl2s(del2m,xpi,piDpj,i,m,ii,isii,j,k,kj,iskj,10,ier1)
	else
	    call vbfffdl2t(del2m,piDpj,i,m,j,k,kj,iskj,+1,10,ier1)
	endif
	s(1) = del2m*piDpj(n,l)
	s(2) = del2n*piDpj(m,l)
	...
	ier0 = ier
	call vbfffdl2t(del2nm,piDpj,i,nm,j,k,kj,iskj,+1,10,ier0)
...
*
	del2d2 = som
	if ( lwarn ) call vbfffwarn(123,ier,del2d2,xmax)
	if ( lwrite ) then
	    print *,'vbfff2dl2: giving up on this case'
...
*  #[ check:
	if ( ltest ) then
		...
		print *,'vbfff2dl2: error: del2d2 not correct: ',del2d2,
     +		som,xmax,del2d2-som
	    endif
	endif
*  #] check: 
...
*###[ vbfff2d22:
	subroutine vbfff2d22(dl2d22,xpi,dpipj,piDpj, i, j,k,kj,iskj,
     +			m,n,nm,isnm, ns, ier)
***#[*comment:***********************************************************
*									*
*	Calculate							*
...
*  #[ check input:
	if ( ltest ) then
	    if ( abs(iskj) .ne. 1 ) print *,'vbfff2d22: error: abs(iskj) ',
     +		'<> 1 but ',iskj
	    if ( abs(isnm) .ne. 1 ) print *,'vbfff2d22: error: abs(isnm) ',
     +		'<> 1 but ',isnm
	    if ( ns .ne. 10 ) print *,'vbfff2d22: error: ns <> 10 !!'
	    if ( m .ne. 3 .or. n .ne. 4 ) print *,'vbfff2d22: error ',
     +		'only for m=3,n=4 !!'
	endif
*  #] check input: 
*  #[ special cases:
	if ( i .eq. n .or. i .eq. m ) then
	    call vbfffdl2s(del2s,xpi,piDpj, j,k,kj,iskj, m,n,nm,isnm,
     +							 10,ier)
...
*	We use the product form
	...
	elseif ( i .le. 4 ) then
	    ii = inx(3,i)
	    isii = isgn(3,i)
	    call vbfffdl2s(del23,xpi,piDpj,i,3,ii,isii,j,k,kj,iskj,10,ier)
	else
	    call vbfffdl2t(del23,piDpj,i,3,j,k,kj,iskj,+1,10,ier)
	endif
	if ( i .eq. 4 ) then
	    del24 = 0
	elseif ( i .le. 4 ) then
	    ii = inx(n,i)
	    isii = isgn(n,i)
	    call vbfffdl2s(del24,xpi,piDpj,i,4,ii,isii,j,k,kj,iskj,10,ier)
	else
	    call vbfffdl2t(del24,piDpj,i,4,j,k,kj,iskj,+1,10,ier)
	endif
...
*	try the special case k=4 (for use in ee->mumu among others)
	...
	call vbfffdl2t(del27,piDpj,i,7,j,k,kj,iskj,+1,10,ier)
	s(1) = xpi(7)*del24**2
...
*
	...
	dl2d22 = som
	if ( lwarn ) call vbfffwarn(122,ier,dl2d22,xmax)
	if ( lwrite ) then
	    print *,'vbfff2d22: give up on this case ...'
...
*###[ vbfff3dl2:
	subroutine vbfff3dl2(del3d2,xpi,dpipj,piDpj, i,
...
*  #[ check input:
	if ( ltest ) then
	    if ( abs(iskj) .ne. 1 ) print *,'vbfff3dl2: error: abs(iskj) ',
     +		'<> 1 but ',iskj
	    if ( abs(isml) .ne. 1 ) print *,'vbfff3dl2: error: abs(isml) ',
     +		'<> 1 but ',isml
	    if ( abs(ispo) .ne. 1 ) print *,'vbfff3dl2: error: abs(ispo) ',
     +		'<> 1 but ',ispo
	    if ( ns .ne. 10 ) print *,'vbfff3dl2: error: ns <> 10 !!'
	endif
*  #] check input: 
*  #[ split up l,m:
	if ( i .eq. l ) then
	...
	elseif ( i .le. 4 ) then
	    ii = inx(l,i)
	    isii = isgn(l,i)
	    call vbfffdl2s(dl2il,xpi,piDpj,i,l,ii,isii,j,k,kj,iskj,10,ier)
	else
	    call vbfffdl2t(dl2il,piDpj,i,l,j,k,kj,iskj,+1,10,ier)
	endif
	if ( m .eq. n ) then
	    dl2mn = 0
	elseif ( i .le. 4 ) then
	    ii = inx(n,m)
	    isii = isgn(n,m)
	    call vbfffdl2s(dl2mn,xpi,piDpj,m,n,ii,isii,o,p,po,ispo,10,ier)
	else
	    call vbfffdl2t(dl2mn,piDpj,m,n,o,p,po,ispo,+1,10,ier)
	endif
	s(1) = dl2il*dl2mn
	if ( i .eq. m ) then
	    dl2im = 0
	elseif ( i .le. 4 ) then
	    ii = inx(m,i)
	    isii = isgn(m,i)
	    call vbfffdl2s(dl2im,xpi,piDpj,i,m,ii,isii,j,k,kj,iskj,10,ier)
	else
	    call vbfffdl2t(dl2im,piDpj,i,m,j,k,kj,iskj,+1,10,ier)
	endif
	if ( l .eq. n ) then
	    dl2ln = 0
	elseif ( i .le. 4 ) then
	    ii = inx(n,l)
	    isii = isgn(n,l)
	    call vbfffdl2s(dl2ln,xpi,piDpj,l,n,ii,isii,o,p,po,ispo,10,ier)
	else
	    call vbfffdl2t(dl2ln,piDpj,l,n,o,p,po,ispo,+1,10,ier)
	endif
	s(2) = dl2im*dl2ln
	del3d2 = s(1) - s(2)
...
*	rotate l,m
*
	call vbfffdl2t(dl2mln,piDpj,ml,n,o,p,po,ispo,+1,10,ier)
	call vbfffdl2t(dl2iml,piDpj,i,ml,j,k,kj,iskj,+1,10,ier)
	s(1) = dl2im*dl2mln
	s(2) = dl2iml*dl2mn
	del3d2 = isml*(s(1) - s(2))
	if ( lwrite ) print *,'  del3d2+ = ',del3d2,s(1),-s(2)
...
*  #] split up l,m: 
*  #[ split up j,k:
	call vbfff2dl2(d2d2k,dum,xpi,dpipj,piDpj, k, l,m,ml,isml, n,
     +						o,p,po,ispo, 10, ier)
	call vbfff2dl2(d2d2j,dum,xpi,dpipj,piDpj, j, l,m,ml,isml, n,
     +						o,p,po,ispo, 10, ier)
	s(1) = piDpj(i,j)*d2d2k
	s(2) = piDpj(i,k)*d2d2j
	del3d2 = s(1) - s(2)
	...
	call vbfff2dl2(d2d2kj,dum,xpi,dpipj,piDpj, kj, l,m,ml,isml, n,
     +						o,p,po,ispo, 10, ier)
...
*  #] split up j,k: 
*  #[ split up o,p:
	call vbfff2dl2(d2d2o,dum,xpi,dpipj,piDpj, i, j,k,kj,iskj, o,
     +						l,m,ml,isml, 10, ier)
	call vbfff2dl2(d2d2p,dum,xpi,dpipj,piDpj, i, j,k,kj,iskj, p,
     +						l,m,ml,isml, 10, ier)
	s(1) = piDpj(p,n)*d2d2o
	s(2) = piDpj(o,n)*d2d2p
	del3d2 = s(1) - s(2)
	...
	call vbfff2dl2(d2d2po,dum,xpi,dpipj,piDpj, i, j,k,kj,iskj, po,
     +						l,m,ml,isml, 10, ier)
...
*  #[ give up:
	del3d2 = som
	if ( lwarn ) call vbfffwarn(124,ier,del3d2,xmax)
*  #] give up: 
*###] vbfff3dl2: 
	end
