*--#[ log:
*	$Id: vbfffcrr.f,v 1.5 1995/11/10 19:04:23 gj Exp $
*	$Log: vbfffcrr.f,v $
...
*--#] log: 
*###[ vbfffcrr:
	subroutine vbfffcrr(crr,ipi12,cy,cy1,cz,cz1,cdyz,ld2yzz,cd2yzz,czz,
...
*
	DOUBLE COMPLEX check,cfact,cc1,cc2,cc1p,cc2p,carg1,carg2,carg3,
     ...
     +		cd2,cd21,cd2n,cd21n1,cc1n,cterm,ctot,vbfzfflo1,clog1,clog2,
     +		cr,cr1,cc,cli4,clo4
	DOUBLE COMPLEX clia,clib,ctroep,vbfzfflog
	DOUBLE PRECISION xa,xr,absc,rloss,xprec,bndtay,vbfffbnd
	DOUBLE PRECISION y,y1,z,z1,dyz,d2yzz,zz,zz1
	integer i,vbfnffeta,vbfnffet1,iclas1,iclas2,ier0,n1,n2,n3,ntot,ipi121,
...
*  #[ check input:
	if ( ltest ) then
	    if ( ipi12.ne.0 ) then
		print *,'vbfffcrr: error: why is ipi12 != 0? ',ipi12
	    endif
     ...
     +		    .and. abs(DIMAG(cz)).gt.precc*abs(DBLE(cz)) ) then
		if ( DIMAG(cz).gt.0 .eqv. ieps.gt.0 ) then
		    print *,'vbfffcrr: error: imaginary signs cz and ',
     ...
     +		absc(cy1),x1)) then
		print *,'vbfffcrr: error: cy <> 1-cy1',cy,cy1,check
	    endif
	    check = cz + cz1 - 1
	    if( rloss*absc(check) .gt. precc*max(absc(cz),
     +		absc(cz1),x1)) then
		print *,'vbfffcrr: error: cz <> 1-cz1',cz,cz1,check
	    endif
	    check = cdyz - cy + cz
	    if ( rloss*absc(check) .gt. precc*max(absc(cy),
     +		absc(cz),absc(cdyz)) ) then
		print *,'vbfffcrr: error: cdyz <> cy-cz',cdyz,cy,cz,check
	    endif
	    if ( ld2yzz ) then
		check = cd2yzz-2*cy+cz+czz
		if( rloss*absc(check).gt.precc*max(absc(cd2yzz),
     +			2*absc(cy),absc(cz),absc(czz)))then
		    print *,'vbfffcrr: error: cd2yzz<>2cy-cz-czz',cd2yzz,
     +			2*cy,cz,czz,check
		endif
		check = czz + czz1 - 1
		if ( rloss*absc(check) .gt. precc*max(absc(czz),
     +			absc(czz1),x1) ) then
		    print *,'vbfffcrr: error: 1-czz <> czz1',czz,czz1,check
		endif
	    endif
	endif
	if ( lwrite ) then
	    print *,'vbfffcrr: input:'
	    print *,'  cy    = ',cy,cy1
	    print *,'  cz    = ',cz,cz1
	    print *,'  cdyz  = ',cdyz
	    if ( ld2yzz ) then
...
*  #[ real case:
	if ( DIMAG(cy).eq.0 .and. DIMAG(cy1).eq.0 .and. DIMAG(cz).eq.0
     +		.and. DIMAG(cz1).eq.0 ) then
	    if ( lwrite ) then
		print *,'vbfffcrr: all arguments are real'
		print *,'       calling vbfffcxr'
	    endif
	    y  = DBLE(cy)
	    y1 = DBLE(cy1)
	    z  = DBLE(cz)
	    ...
	    zz = DBLE(czz)
	    zz1 = DBLE(czz1)
	    call vbfffcxr(crr,ipi12,y,y1,z,z1,dyz,ld2yzz,d2yzz,zz,zz1,
     +		.FALSE.,x0,ieps,ier)
	    return
	endif
*  #] real case:
...
*
	xa = absc(cdyz)
	if ( xa .eq. 0 ) then
	    if ( lwarn ) call vbfffwarn(48,ier,absc(cdyz),x1)
	    return
...
*			n2 = vbfnffet1(DCMPLX(DBLE(0),DBLE(ieps)),cfact,cfact,ier) = 0
			n2 = 0
		    elseif ( cz1 .eq. 0 ) then
			n1 = vbfnffet1(DCMPLX(DBLE(0),DBLE(ieps)),cfact,
     +				-cfact,ier)
			n2 = 0
		    else
			n1 = vbfnffet1(DCMPLX(DBLE(0),DBLE(ieps)),cfact,
     +				-cz*cfact,ier)
			n2 = vbfnffet1(DCMPLX(DBLE(0),DBLE(ieps)),cfact,
     +				cz1*cfact,ier)
		    endif
		else
		    n1 = vbfnffet1(-cz,cfact,-cz*cfact,ier)
		    n2 = vbfnffet1(-cz,cfact,cz1*cfact,ier)
		endif
	    else
		n1 = vbfnffet1(cz1,cfact,-cz*cfact,ier)
		n2 = vbfnffet1(cz1,cfact,cz1*cfact,ier)
	    endif
	else
...
*		    is independent of the sign used.
     ...
     +			isgnal*DBLE(precc)*DIMAG(cy)
		    n1 = vbfnffet1(-cz,cfact,DCMPLX(DBLE(0),ieps*DBLE(cy)),
     +			ier)
		else
		    n1 = vbfnffet1(-cz,cfact,cc1p,ier)
		endif
	    else
		if ( DIMAG(cc1) .eq. 0 ) then
		    if ( DBLE(cy1).eq.0 ) cy1 = cy1 +
     +			isgnal*DBLE(precc)*DIMAG(cy)
		    n1 = vbfnffet1(-cz,cfact,DCMPLX(DBLE(0),
     +					-ieps*DBLE(cy1)),ier)
		else
		    n1 = vbfnffet1(-cz,cfact,-cc1,ier)
		endif
	    endif
	    if ( iclas2 .eq. 2 ) then
		if ( DIMAG(cc2p) .eq. 0 ) then
		    if ( DBLE(cy).eq.0 ) cy = cy +
     +			isgnal*DBLE(precc)*DIMAG(cy)
		    n2 = vbfnffet1(cz1,cfact,DCMPLX(DBLE(0),ieps*DBLE(cy)),
     +			ier)
		else
		    n2 = vbfnffet1(cz1,cfact,cc2p,ier)
		endif
	    else
		if ( DIMAG(cc2) .eq. 0 ) then
		    if ( DBLE(cy1).eq.0 ) cy1 = cy1 +
     +			isgnal*DBLE(precc)*DIMAG(cy)
		    n2 = vbfnffet1(cz1,cfact,DCMPLX(DBLE(0),
     +					-ieps*DBLE(cy1)),ier)
		else
		    n2 = vbfnffet1(cz1,cfact,-cc2,ier)
		endif
	    endif
	endif
	endif
...
*
		...
		call vbfffzli2(cli1,clo1,carg1,lreal,ier)
		call vbfffzli2(cli2,clo2,carg2,lreal,ier)
		call vbfffzli2(cli3,clo3,carg3,lreal,ier)
		if ( absc(cc2p) .lt. xloss ) then
		    clog2p = vbfzfflo1(cc2p,ier)
		else
		    clog2p = vbfzfflog(1-cc2p,0,c0,ier)
		endif
		chill = clo1*clog2p
*debug		the sum of these terms should be Li2(cc1p)-Li2(cc2p)
*		if ( lwrite ) then
*		    csum  = cli1 + cli2 - cli3 + chill
*		    call vbfffzli2(clia,ctroep,cc1p,lreal,ier0)
*		    call vbfffzli2(clib,ctroep,cc2p,lreal,ier0)
*		    print *,'      check Hill'
...
*
   ...
   50		continue
		if ( lwarn ) call vbfffwarn(54,ier,absc(ctot),absc(cterm))
   51		continue
		cli1 = ctot
		cli2 = 0
		cli3 = 0
...
*		for the eta+transformation section we also need
     ...
     +			clo1 = vbfzfflo1(cd2,ier)
		if ( iclas1.eq.2 ) clo2 = vbfzfflo1(1/cy1,ier)
...
*		crr(5) = 0
		if ( n1 .ne. n2 ) then
		    if ( lwarn ) call vbfffwarn(49,ier,x1,x0)
		    if ( absc(cc1) .lt. xclogm ) then
			call vbffferr(23,ier)
		    else
*			imaginary part not checked
			ier = ier + 50
			crr(5) = (n1-n2)*c2ipi*vbfzfflog(cc1,ieps,-cy,ier)
		    endif
		endif
*		crr(6) = 0
*		crr(7) = 0
...
*			crr(7) = 0
		...
		if (lwrite) then
		    clog1 = vbfzfflog(cc1,ieps,-cy,ier)
		    clog2 = vbfzfflog(cc2,ieps,cy1,ier)
...
*			use the ieps instead
			...
			n3 = vbfnffet1(DCMPLX(DBLE(0),ieps*DBLE(cy)),
     +				1/cc1p,cc2p/cc1p,ier)
		    endif
		else
		    if ( DIMAG(cc1p).eq.0 ) then
			n3 =vbfnffet1(cc2p,DCMPLX(DBLE(0),-ieps*DBLE(cy1)),
     +				cc2p/cc1p,ier)
		    else
			n3 = vbfnffet1(cc2p,1/cc1p,cz,ier)
		    endif
		endif
		ntot = n1-n2-n3
		crr(5) = (ntot*c2ipi + clo1)*clog1
		clog2p = vbfzfflog(cc2p,ieps,cy,ier)
		crr(6) = clo2*(n2*c2ipi - clog2p)
*		crr(7) = 0
*		if (lwrite) then
*		    clog1p = vbfzfflog(cc1p,ieps,cy,ier)
*		    clog2 = vbfzfflog(cc2,ieps,cy1,ier)
...
*     +			call vbfffwarn(57,ier,absc(ctroep),absc(czz1))
*		    clog2p = vbfzfflo1(ctroep/cy1,ier)
*		else
		    clog2p = vbfzfflog(-cc2p,ieps,cy1,ier)
*		endif
...
*		    we chose the eta's already equal, no worry.
		    ...
		    n3 = vbfnffet1(-cc2p,-cc1,-cy/cy1,ier)
		    n3p = vbfnffet1(cc2p,cc1,-cy/cy1,ier)
		endif
...
*		    something smarter one day
		    ...
		    crr(5) = -clog1p**2/2
		    crr(6) = +clog2p**2/2
		    crr(7) = (n1*vbfzfflog(cc1,ieps,cy,ier) -
     +				n2*vbfzfflog(cc2,ieps,-cy1,ier))*c2ipi
		else
...
*
		...
		call vbfffzli2(cli1,clo1,cc1p,lreal,ier)
	    else
		cd2 = cd2yzz + czz
     ...
     +			call vbfffwarn(56,ier,absc(cd2),absc(cdyz))
		endif
		cd2 = cd2/cdyz
		cfact = 1/(2-cd2)
		call vbfffzli2(cli1,clo1,cd2*cfact,lreal,ier)
		call vbfffzli2(cli3,clo3,-cd2*cfact,lreal,ier)
		call vbfffzli2(cli4,clo4,cd2,lreal,ier)
	    endif
	    if ( iclas2 .ne. 4 .or. .not. ld2yzz ) then
		call vbfffzli2(cli2,clo2,cc2p,lreal,ier)
	    else
		if ( iclas1 .eq. 4 ) call vbffferr(26,ier)
		cd2 = cd2yzz - czz1
		if ( absc(cd2) .lt. xloss*absc(cd2yzz) ) then
		    if ( lwrite ) print *,'cd2  = ',cd2
		    cd2 = cdyz - cy1
     ...
     +			call vbfffwarn(57,ier,absc(cd2),absc(cdyz))
		endif
		cd2 = cd2/cdyz
		cfact = 1/(2-cd2)
		call vbfffzli2(cli2,clo2,cd2*cfact,lreal,ier)
		call vbfffzli2(cli3,clo3,-cd2*cfact,lreal,ier)
		call vbfffzli2(cli4,clo4,cd2,lreal,ier)
	    endif
* -#]	    handle dilogs:
...
*			take apart the factor i*pi^2
			if ( iclas1 .eq. 4 ) then
			    clog1 = vbfzfflo1(cd2,ier)
			else
			    clog1 = vbfzfflog(-cc1,0,cy,ier)
			endif
...
*		crr(5) = 0
		...
		if ( iclas2 .ne. 2 .or. absc(cc2p) .gt. xloss ) then
			...
			clog2 = vbfzfflog(cc2,ieps,cy,ier)
		    else
*			take apart the factor i*pi^2
			if ( iclas2 .eq. 4 ) then
			    clog2 = vbfzfflo1(cd2,ier)
			else
			    clog2 = vbfzfflog(-cc2,0,c0,ier)
			endif
...
*		crr(4) = 0
		...
		cli2 = -cli2
		ipi12 = ipi12 - 2
		crr(4) = clo2*vbfzfflog(cc2p,ieps,cy,ier)
...
*		Note that this sum does not cause problems as d2<<1
		crr(4) = cli3 + cli4 - clo4*vbfzfflog(cfact,0,c0,ier)
		ipi12 = ipi12 + 1
		if ( lwrite ) then
		    print *,'Check iclas2 = 4'
		    print '(a,2g14.8)','Nu:   ',-cli2+crr(4)
		    call vbfffzli2(clia,ctroep,cc2p,lreal,ier)
		    print '(a,2g14.8)','Eerst:',-clia-DBLE(pi12)
		endif
	    else
		call vbffferr(27,ier)
	    endif
* -#]	    handle transformation terms:
* -#[	    sum and print:
	    crr(1) = cli1
...
*  #[ debug:
	if(lwrite)then
	    ier0 = 0
	    call vbfffzzdl(cli1,ipi121,ctroep,cc1,ier0)
	    call vbfffzzdl(cli2,ipi122,ctroep,cc2,ier0)
