*###[ vbfffdel4:
	subroutine vbfffdel4(del4,xpi,piDpj,ns,ier)
***#[*comment:***********************************************************
*									*
...
*  #[ check input:
	if ( ltest .and. ns .ne. 10 ) then
	    print *,'vbfffdel4: error: only for ns = 10, not ',ns
	    stop
	endif
*  #] check input: 
...
*
	do 5 i=1,mem
		...
		inow = memarr(i,3)
		jnow = memarr(i,4)
		if ( lwrite ) print *,'vbfffcel4: from memory: ',id,idsub,
     +			inow,jnow
		goto 6
	    endif
    5	continue
...
*	    through with large steps
     ...
     +			) then
		if ( lwarn ) call vbfffwarn(143,ier,del4,xmax)
		goto 800
	    endif
	    goto 10
	endif
...
*  #] calculations: 
*  #[ into memory:
	if ( lwrite ) print *,'vbfffcel4: into memory: ',id,idsub,inow,jnow
	memind = memind + 1
...
*
  ...
  820	    continue
		...
		print *,'vbfffdel4: error: result does not agree with',
     +			' normal case'
...
*###[ vbfffdl3p:
	subroutine vbfffdl3p(dl3p,piDpj,ns,ii,jj,ier)
***#[*comment:***********************************************************
...
*  #[ check input:
	if ( lwrite ) then
	    print *,'vbfffdl3p: indices are'
	    print *,ii
	    print *,jj
	endif
	if ( ltest ) then
	    if ( ns .ne. 10 .and. ns .ne. 15 ) print *,'vbfffdl3p: error:',
     ...
     +			'vbfffdl3p: error: momenta i1234 do not add to 0:',
     ...
     +			   abs(piDpj(i,ii(3))))
		if ( xlosn*xheck .gt. precx*xmax ) print *,
     +			'vbfffdl3p: error: momenta i623 do not add to 0:',
     ...
     +			   abs(piDpj(i,ii(2))))
		if ( xlosn*xheck .gt. precx*xmax ) print *,
     +			'vbfffdl3p: error: momenta i512 do not add to 0:',
     ...
     +			   abs(piDpj(i,jj(3))),abs(piDpj(i,jj(4))))
		if ( xlosn*xheck .gt. precx*xmax ) print *,
     +			'vbfffdl3p: error: momenta j1234 do not add to 0:',
     ...
     +			   abs(piDpj(i,jj(3))))
		if ( xlosn*xheck .gt. precx*xmax ) print *,
     +			'vbfffdl3p: error: momenta j623 do not add to 0:',
     ...
     +			   abs(piDpj(i,jj(2))))
		if ( xlosn*xheck .gt. precx*xmax ) print *,
     +			'vbfffdl3p: error: momenta j512 do not add to 0:',
...
*		give up a bit more easily if I have tried many times
  ...
  101	continue
  109	continue
	if ( lwarn ) call vbfffwarn(138,ier,dl3p,xmax)
  110	continue
*  #] calculations:
*###] vbfffdl3p:
	end
