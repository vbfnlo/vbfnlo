*--#[ log:
*	$Id: vbfffdcxs.f,v 1.7 1996/03/22 08:13:30 gj Exp $
*	$Log: vbfffdcxs.f,v $
c Revision 1.7  1996/03/22  08:13:30  gj
c Fixed bug in bugfix of vbfffdcxs.f
c
c Revision 1.6  1996/03/14  15:53:13  gj
c Fixed bug in vbfffcb0: cp in C, cma=cmb=0 was computed incorrectly.
c
...
*--#] log:
*###[ vbfffdcxs:
	subroutine vbfffdcxs(cs3,ipi12,y,z,dyz,d2yzz,dy2z,dyzzy,xpi,piDpj,
     +						ii,ns,isoort,ier)
***#[*comment:***********************************************************
*									*
*	calculates the the difference of two S's with y(3,4),z(3,4) and *
*	y(4)z(3)-y(3)z(4) given.  Note the difference with vbfffdcxs4, in	*
...
*	vbfffcxs3 in its expectations of the order of the roots: it knows	*
...
*									*
*	calls:	vbfffcrr,vbfffcxr,real/dble,DCMPLX,log,ffadd1,ffadd2,ffadd3	*
*									*
***#]*comment:***********************************************************
*  #[ declarations:
	implicit none
...
*
*	check constants
	if ( ltest .and. ns .ne. 6 ) print *,'vbfffdcxs: error: ns <> 6'
*  #] declarations:
*  #[ check input:
	if ( ltest ) then
		...
		do 1 i=1,2
		    ...
		    if ( rloss*xhck .gt. precx*xmax ) print *,'vbfffdcxs:',
...
*  #[ normal case:
   ...
   10	continue
	if ( normal .or. isoort(1) .ne. isoort(9) .or. isoort(1) .lt.
     +		10 ) then
	    if ( lwrite ) print *,'vbfffdcxs: normal case'
	    call vbfffcxs3(cs3( 1),ipi12(1),y(1,3),z(1,3),dyz(1,1,3),
     +		d2yzz(3),dy2z(1,3),xpi(1,3),piDpj(1,1,3),ii,6,
     +		isoort(1),ier)
	    call vbfffcxs3(cs3(81),ipi12(9),y(1,4),z(1,4),dyz(1,1,4),
...
*  #[ rotate R's:
	if ( abs(y(2,3)) .lt. 1/xloss ) then
	    if ( lwrite ) print *,'vbfffdcxs: rotating R''s'
	    do 102 i=1,2
	    do 101 j=1,2
*		iepsi() = /-2,+2,+2,-2/
...
*		    iepsj(j,i) = 0
		    if ( lwrite ) print *,'vbfffdcxs: don''t know ieps ',i
		    normal = .TRUE.
		    goto 10
		endif
  101	    continue
...
*			have been dealt with in vbfffdxc0)
		    if ( isoort(9) .eq. 0 ) then
			if ( lwrite ) then
			    print *,'vbfffdcxs: isoort(9).eq.0, not needed'
			endif
			goto 110
		    endif
*		    or if not needed (isoort=2, two equal roots)
		    if ( mod(isoort(9),10) .eq. 2 ) then
			if ( lwrite ) print *,'vbfffdcxs: skipped next ',
     +				'R as it is equal'
...
*     +				x1) ) print *,'vbfffdcxs: error: 1 - yy ',
*     +				'<> yy1',yy,yy1,yy+yy1-1
*			if ( rloss*abs(zz+zz1-1) .gt. precx*max(abs(zz),
*     +				x1) ) print *,'vbfffdcxs: error: 1 - zz ',
*     +				'<>zz1',zz,zz1,zz+zz1-1
*			if ( rloss*abs(dyyzz-yy+zz) .gt. precx*max(abs(
*     +				yy),abs(zz)) ) print *,'vbfffdcxs: error:',
...
*		    endif
  ...
  109			continue
		    ...
		    call vbfffcxr(cs3(10*l-9),ipi12(l),yy,yy1,zz,zz1,dyyzz,
...
*  #[ other cases (not ready):
	if ( lwrite ) print *,'vbfffdcxs: warning: special case not',
...
*  #[ debug:
  ...
  805	    continue
	    print '(a)','   #[ compare: '
	    call vbfffcxs3(cs3p( 1),ipi12p(1),y(1,3),z(1,3),dyz(1,1,3),
     +		d2yzz(3),dy2z(1,3),xpi(1,3),piDpj(1,1,3),ii,ns,
     +		isoort(1),ier0)
	    call vbfffcxs3(cs3p(21),ipi12p(3),y(1,4),z(1,4),dyz(1,1,4),
     +		d2yzz(4),dy2z(1,4),xpi(1,4),piDpj(1,1,4),ii,ns,
     +		isoort(9),ier0)
	    print '(a)','   #] compare: '
	    cs1 = 0
...
*###[ vbfffdcs:
	subroutine vbfffdcs(cs3,ipi12,cy,cz,cdyz,cd2yzz,cdyzzy,cdyyzz,
     +		cpi,cpiDpj,ii,ns,isoort,ier)
***#[*comment:***********************************************************
*									*
*	calculates the the difference of two S's with cy(3,4),cz(3,4),	*
*	cy(4)cz(3)-cy(3)cz(4) given.  Note the difference with vbfffdcs4,	*
...
*
*	check constants
	if ( ltest .and. ns .ne. 6 ) print *,'vbfffdcs: error: ns <> 6'
*  #] declarations:
*  #[ check input:
	if ( ltest ) then
...
*  #[ normal case:
	if ( mod(isoort(1),5).ne.mod(isoort(9),5) .or. isoort(1).gt.-5
     +								) then
	    if ( lwrite ) print *,'vbfffdcs: normal case'
	    if ( ltest .and. isoort(1) .le. -100 ) then
		print *,'vbfffdcs: error: wrong value for isoort'
	    endif
	    call vbfffcs3(cs3( 1),ipi12(1),cy(1,3),cz(1,3),cdyz(1,1,3),
     +		cd2yzz(3),cpi(1,3),cpiDpj(1,1,3),ii,6,isoort(1),ier)
	    call vbfffcs3(cs3(81),ipi12(9),cy(1,4),cz(1,4),cdyz(1,1,4),
...
*  #[ rotate R's:
	if ( absc(cy(2,3)) .lt. 1/xloss .or. isoort(1) .le. -100 ) then
	    if ( lwrite ) print *,'vbfffdcs: rotated R''s'
*
...
*
		...
		call vbfffclg2(cs3(81),ipi12(9),cy(2,3),cz(1,3),
...
*		    we have the wrong sign
  ...
  120		    continue
     ...
     +								) then
		    if ( lwrite ) print *,'vbfffdcs: skipped other logs ',
  ...
  130		    continue
		    ...
		    print *,'vbfffdcs: error: not yet tested'
		    call vbfffclg2(cs3(91),ipi12(10),cy(2,3),cz(2,3),
...
*			have been dealt with in vbfffdxc0,vbfffdcc0)
*
			call vbffferr(79,ier)
			goto 180
...
*
			if ( lwrite ) print *,'vbfffdcs: skipped next ',
...
*
*		    ieps = 3 means: dear vbfffcrr, do not use eta terms,
...
*
     ...
     +								) then
		    ...
		    call vbfffcrr(cs3(10*l-9),ipi12(l),yy,yy1,zz,zz1,dyyzz,
...
*			no eta terms in the real case
     ...
     +			     DIMAG(cdyz(2,j,m)) .eq. 0 ) then
			    ni(k,m) = 0
			elseif ( i .eq. 1 ) then
			    ni(k,m) = vbfnffeta(-cz(k,m),1/cdyz(2,j,m),ier)
			else
			    ni(k,m) = vbfnffeta(cz(k,m),1/cdyz(2,j,m),ier)
			endif
  170		    continue
 ...
 175			    continue
			    ...
			    if ( i .eq. 1 ) then
				n1a = vbfnffeta(cy(k,3)/cdyz(2,j,3),
     ...
     +					-cdyz(2,j,4)/cy(k,4),ier)
			    ...
			    n = 10*l - 2
			    if ( ltest .and. cs3(n) .ne. 0 ) then
				print *,'vbfffdcs: error: cs3(',n,') not 0'
			    endif
			    if ( i .eq. 1 ) then
...
*  #] rotate R's:
*  #[ other cases (not ready):
	if ( lwrite ) print *,'vbfffdcs: warning: special case not',
     +		' yet implemented, trying normal route'
	call vbfffcs3(cs3( 1),ipi12(1),cy(1,3),cz(1,3),cdyz(1,1,3),
...
*
		...
		ni(1,k) = vbfnffet1(-cz(1,k),-cz(2,k),cmip,ier)
		i = vbfnffet1(cz(3,k),cz(4,k),cmip,ier)
		if ( i .ne. ni(1,k) ) call vbffferr(53,ier)
	    endif
	    ni(2,k) = 0
...
*		follow the i*epsilon prescription as (y-z-)(y-z+) real
		...
		ni(4,k) = -vbfnffet1(cdyz(2,1,k),cdyz(2,2,k),cmip,ier)
	    else
     ...
     +			cdyz(2,2,k)) .lt. 0 ) then
		...
		ni(4,k) = -vbfnffeta(cdyz(2,1,k),cdyz(2,2,k),ier)
	    endif
	elseif ( (mod(isoort(l),10).eq.-1 .or. mod(isoort(l),10).eq.-3)
     +		.and. isoort(l+1).eq.0 ) then
	    ni(1,k) = 0
	    if ( DIMAG(cz(1,k)) .ne. 0 ) then
		ni(2,k) = vbfnffet1(-cpiDpj(ii,ip,k),-cz(1,k),DCMPLX(DBLE(0
     +			),DBLE(-1)),ier)
	    else
		ni(2,k) = vbfnffet1(-cpiDpj(ii,ip,k),DCMPLX(DBLE(0),
     +			DBLE(1)),DCMPLX(DBLE(0),DBLE(-1)),ier)
	    endif
	    ni(3,k) = 0
	    ni(4,k) = -vbfnffeta(-cpiDpj(ii,ip,k),cdyz(2,1,k),ier)
	else
	    if ( mod(isoort(l),5).ne.0 .and. mod(isoort(l),5).ne.-1
     +			.and. mod(isoort(l),5).ne.-3 ) then
		call vbffferr(81,ier)
	    ...
	    print *,'vbfffdcs: error: cpi complex should not occur'
	    stop
	endif
  740	continue
	if ( lwrite ) then
	    print *,'vbfffdcs: eta''s are: '
...
*  #[ add eta's:
  ...
  750	continue
	if ( ntot(3) .ne. 0 .and. ntot(3) .eq. ntot(4) ) then
	    if ( lwrite ) print *,'vbfffdcs: warning: could be smarter...'
	endif
	do 760 k=3,4
	    if ( ntot(k) .ne. 0 ) call vbfffclgy(cs3(20+80*(k-3)),
...
*  #[ debug:
  ...
  806		continue
		...
		print '(a)','   #[ compare: '
		call vbfffcs3(cs3p( 1),ipi12p(1),cy(1,3),cz(1,3),cdyz(1,1,3
     +			),cd2yzz(3),cpi(1,3),cpiDpj(1,1,3),ii,ns,
     +			isoort(1),ier0)
		call vbfffcs3(cs3p(21),ipi12p(3),cy(1,4),cz(1,4),cdyz(1,1,4
...
*###[ vbfffclg2:
	subroutine vbfffclg2(cs3,ipi12,cy3,cz3,cdyz3,cy4,cz4,cdyz4,cdyyzz,
...
*	local variables
*
	integer n1,vbfnffeta,vbfnffet1,ipi3,ipi4
	DOUBLE COMPLEX c,cc,chck,clog3,clog4,clog1,vbfzfflo1,cipi
...
*  #[ calculations:
	...
	if ( absc(cc) .ge. xloss*absc(clog3) ) then
	    ...
	    c = cdyyzz/cdyz4
	    clog1 = vbfzfflo1(c,ier)
*
*	    notice that vbfzfflog return log(a-ieps) (for compatibility
*	    with the dilog)		   ^
*
	    if ( DIMAG(cdyz3) .eq. 0 ) then
		n1 = vbfnffet1(DCMPLX(DBLE(0),DBLE(-ieps)),-1/cdyz4,-c,
     +			ier)
	    elseif ( DIMAG(cdyz3) .eq. 0 ) then
		n1 = vbfnffet1(-cdyz3,DCMPLX(DBLE(0),DBLE(ieps)),-c,ier)
	    else
		n1 = vbfnffeta(-cdyz3,-1/cdyz4,ier)
	    endif
	    if ( n1 .ne. 0 ) then
...
*	ATTENTION: now (23-jul-1989) vbfffdcs assumes that only *3* cs are
*	set. Change vbfffdcs as well if this is no longer true!
*  #] calculations:
*  #[ debug:
	if ( lwrite ) then
	    if ( clog1 .ne. -123 ) then
		print *,'vbfffclg2: originally:'
