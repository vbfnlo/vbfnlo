*###[ vbfffcxyz:
	subroutine vbfffcxyz(cy,cz,cdyz,cd2yzz,ivert,sdelpp,sdelps,etalam,
...
*  #[ test output:
  200	continue
	if ( ltest ) then
     ...
     +		    cy(i)),absc(cy(i+2)),x1)) print *,'vbfffcxyz: error: ',
     ...
     +			print *,'vbfffcxyz: error: cdyz(',i,j,') <> cy(',i,
...
*###[ vbfffcdwz:
	subroutine vbfffcdwz(cdwz,cw,cz,i1,j1,l,calpha,calph1,cpi,cdpipj,
...
*		    first get the difference of csdeli(1) and csdel2:
			...
			if ( lwrite ) print *,'vbfffcdwz: canc in cddel'
			ier = 1
			goto 5
		    endif
    ...
    1		continue
		if ( absc(csum) .lt. xloss*xmax ) then
		    if ( lwrite ) print *,'vbfffcdwz: still cancellations',
     ...
     +				,xmax/absc(calph1*cpi(4)*cpi(5))
		...
		if ( lwrite ) print *,'vbfffcdwz:  warning: cannot handle',
...
*		    vbfffxc0p)
		    if ( lwrite ) then
			call vbfffwarn(78,ier,absc(csum),xmax)
		    else
			ier = ier + 1
		    endif
     ...
     +			' this case cdwz(',i1,j1,l,') yet'
	    ...
	    if ( lwrite ) print *,'vbfffcdwz:  error: l <> 1 or 3 but ',l
	    ier = ier + 100
	endif
*  #] calculations:
*  #[ test output:
	if ( ltest .and. ier .eq. 0 ) then
     ...
     +		print *,'vbfffcdwz:  error: cdwz(',i1,j1,l,') <> cw - cz :'
     ...
     +		absc(cz(j1+2))) ) print *,'vbfffcdwz:  error: cdwz(',i1,j1,
