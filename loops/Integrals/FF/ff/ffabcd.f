*###[ vbfffabcd:
	subroutine vbfffabcd(aijkl,xpi,dpipj,piDpj,del2s,sdel2s,
...
*  #[ check input:
	if ( ltest ) then
	    if ( abs(isji) .ne. 1 ) print *,'vbfff2d22: error: abs(isji)',
     +		' /= 1',isji
	    if ( abs(islk) .ne. 1 ) print *,'vbfff2d22: error: abs(islk)',
     +		' /= 1',islk
	    if ( ns .ne. 10 ) print *,'vbfffabcd: only valid for ns=10!!'
	endif
*  #] check input: 
*  #[ prepare input:
	i = in
...
*	    we have a good memory of things already calculated ...
		...
		call vbfffdl3s(del3(i+k),xpi,piDpj,iii,10,ier)
	    endif
...
*		l .eq. 3 .and. j .eq. 3
   ...
   20		continue
	...
	if ( j .eq. 2 .and. l .eq. 4 ) then
	    call vbfff3dl2(s(1),xpi,dpipj,piDpj, 4, 1,2,5,+1,
     +			k,3,inx(3,k),isgn(3,k), 4, 3,4,7,+1, 10,ier)
	    ...
	    iii(4,2) = isgn(3,k)*inx(3,k)
	    iii(6,2) = isgn(k,4)*inx(k,4)
	    call vbfffdl3s(s(1),xpi,piDpj,iii,10,ier)
...
*	First term:
*
	call vbfff2dl2(del2d2,dum,xpi,dpipj,piDpj, 4,
...
*
	if ( i .eq. 4 ) then
	    ...
	    ii = inx(4,i)
	    isii = isgn(4,i)
	    call vbfffdl2s(del2i,xpi,piDpj,i,4,ii,isii,3,4,7,+1,10,ier)
	endif
	if ( j .eq. 4 ) then
	    del2j = 0
	else
	    ii = inx(4,j)
	    isii = isgn(4,j)
	    call vbfffdl2s(del2j,xpi,piDpj,j,4,ii,isii,3,4,7,+1,10,ier)
	endif
	call vbfff2dl2(d2d2i,dum,xpi,dpipj,piDpj, i, k,l,lk,islk, 4,
     +						3,4,7,+1, 10, ier)
	call vbfff2dl2(d2d2j,dum,xpi,dpipj,piDpj, j, k,l,lk,islk, 4,
     +						3,4,7,+1, 10, ier)
	s(2) = +del2i*d2d2j
	s(3) = -del2j*d2d2i
	somb = s(1) + s(2) + s(3)
...
*	if the first term is wrong ... forget about it
	...
	call vbfffdl2t(del2ji,piDpj, ji,4, 3,4,7,+1,+1, 10,ier)
	call vbfff2dl2(d2d2ji,dum,xpi,dpipj,piDpj, ji, k,l,lk,islk, 4,
     +						3,4,7,+1, 10, ier)
	s(2) = +del2j*d2d2ji
	s(3) = -del2ji*d2d2j
...
*	    ii = inx(4,k)
*	    isii = isgn(4,k)
*	    call vbfffdl2s(del2k,xpi,piDpj,k,4,ii,isii,3,4,7,+1,10,ier)
*	endif
*	if ( l .eq. 4 ) then
*	    del2l = 0
*	else
*	    ii = inx(4,l)
*	    isii = isgn(4,l)
*	    call vbfffdl2s(del2l,xpi,piDpj,l,4,ii,isii,3,4,7,+1,10,ier)
*	endif
*	call vbfff2dl2(d2d2k,dum,xpi,dpipj,piDpj, k, i,j,ji,isji, 4,
*     +						3,4,7,+1, 10, ier)
*	call vbfff2dl2(d2d2l,dum,xpi,dpipj,piDpj, l, i,j,ji,isji, 4,
*     +						3,4,7,+1, 10, ier)
*	s(2) = +del2k*d2d2l
*	s(3) = -del2l*d2d2k
*	somb = s(1) + s(2) + s(3)
...
*	endif
*
*	call vbfffdl2t(del2lk,piDpj, lk,4, 3,4,7,+1,+1, 10,ier)
*	call vbfff2dl2(d2d2lk,dum,xpi,dpipj,piDpj, lk, i,j,ji,isji, 4,
*     +						3,4,7,+1, 10, ier)
*	s(2) = +del2l*d2d2lk
*	s(3) = -del2lk*d2d2l
...
**
*	call vbfff3dl2(d3d2m,xpi,dpipj,piDpj, 3, i,j,ji,isji,
*     +				k,l,lk,islk, 4, 3,4,7,+1, 10,ier)
*	call vbfff3dl2(d3d2n,xpi,dpipj,piDpj, 4, i,j,ji,isji,
*     +				k,l,lk,islk, 4, 3,4,7,+1, 10,ier)
*	s(2) = +d3d2m*piDpj(4,4)
*	s(3) = -d3d2n*piDpj(3,4)
*	somb = s(1) + s(2) + s(3)
...
*	endif
*
*	call vbfff3dl2(d3d2nm,xpi,dpipj,piDpj, 7, i,j,ji,isji,
*     +				k,l,lk,islk, 4, 3,4,7,+1, 10,ier)
*	s(2) = +d3d2n*piDpj(7,4)
...
*
	somb = save
	call vbfffwarn(89,ier,somb,xmax)
	if ( lwrite ) then
	    print *,'vbfffabcd: giving up on somb'
...
*  #] normal case b: 
*  #[ normal case d:
	call vbfff3dl2(s(1),xpi,dpipj,piDpj, 4, i,j,ji,isji, k,l,lk,islk,
     +						4, 3,4,7,+1, 10, ier)
	if ( i .eq. k .and. j .eq. l ) then
	    somd = -2*s(1)
	    if ( lwrite ) s(2) = s(1)
	else
	    call vbfff3dl2(s(2),xpi,dpipj,piDpj, 4, k,l,lk,islk,
     +				i,j,ji,isji, 4, 3,4,7,+1, 10, ier)
	    somd = - s(1) - s(2)
	    if ( abs(somd) .lt. xloss*abs(s(1)) ) then
		call vbfffwarn(90,ier,somd,s(1))
	    endif
	endif
...
*	take into account that we know that we only need x+
	...
	call vbfffwarn(91,ier,min(abs(s(1)),abs(s(2))),xb)
...
*  #[ check output:
  ...
  910	    continue
		...
		print *,'vbfffabcd: error: xb does not agree with ',
     +			'normal case:'
		print *,'  xb:  ',xb
		print *,'  xbp: ',xbp,smaxp
		print *,'  diff:',xb-xbp
  ...
  920	    continue
		...
		print *,'vbfffabcd: error: xd does not agree with ',
     +			'normal case:'
		print *,'  xd:  ',xd
		print *,'  xdp: ',xdp,smaxp
		print *,'  diff:',xd-xdp
...
*  #[ and tne final answer:
  990	continue
	call vbfffroot(dum,aijkl,xa,xb,xc,xd,ier)
*  #] and tne final answer: 
*###] vbfffabcd: 
	end
