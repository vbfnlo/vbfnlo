*###[ vbfffdel5:
	subroutine vbfffdel5(del5,xpi,pDp,ns,ier)
***#[*comment:***********************************************************
*									*
...
*  #[ check input:
	if ( ltest .and. ns .ne. 15 ) then
	    print *,'vbfffdel5: error: ns <> 15!'
	    stop
	endif
	if ( lwrite ) then
	    print *,'vbfffdel5: xpi = ',xpi
	endif
*  #] check input:
*  #[ out of memory:
*
   ...
   20	continue
	if ( abs(del5p) .lt. xloss**2*xmaxp ) then
     ...
     +		j1,j2,j3,j4,j5
		...
		if ( lwarn ) call vbfffwarn(160,ier,del5,xmax)
		goto 800
	    endif
	    goto 10
	endif
...
*  #[ error messages:
	return
  910	print *,'vbfffdel5: error: cannot open file ffperm5.dat with data'
	stop
  920	print *,'vbfffdel5: error: error reading from ffperm5.dat'
	stop
*  #] error messages:
*###] vbfffdel5:
	end
*###[ vbfffdl4p:
	subroutine vbfffdl4p(dl4p,xpi,piDpj,ns,ii,ier)
***#[*comment:***********************************************************
...
*  #[ check input:
	if ( ltest ) then
		...
		if ( ii(i).lt.1 .or. ii(i).gt.ns ) then
		    print *,'vbfffdl4p: error: index out of bounds: ',ii
		    stop
		endif
   10	    continue
	endif
...
*     +			piDpj(jj(3),jj(3))*piDpj(jj(1),jj(4))
  ...
  100	continue
	if ( lwarn ) call vbfffwarn(159,ier,dl4p,xmax)
  110	continue
*  #] calculations:
*  #[ debug output:
	if ( lwrite ) then
	    print *,'vbfffdl4p: input: '
	    print *,'  ii = ',ii
	    print *,'  xpi= ',xpi
	    print *,'vbfffdl4p: output: ',dl4p,xmax
	endif
*  #] debug output:
*###] vbfffdl4p:
	end
*###[ vbfffdl4r:
	subroutine vbfffdl4r(dl4r,xpi,piDpj,ns,miss,ier)
***#[*comment:***********************************************************
*	calculate in a numerically stable way				*
*									*
*	     s1 s2 s3 s4						*
...
*  #[ check input:
	if ( ltest ) then
		...
		print *,'vbfffdl4r: error: miss < 1 or > 5: ',miss
		stop
	    endif
	    do 4 i=1,15
		xnul = 0
    ...
    1		continue
		if ( xloss*xnul .gt. precx*xmax ) print *,'vbfffdl4r: ',
...
*	again, a number relative prime to 125 and a few times smaller
  ...
  110	continue
	if ( lwarn ) call vbfffwarn(169,ier,dl4r,xmax)
  120	continue
*  #] calculations:
*  #[ into memory:
     	continue
