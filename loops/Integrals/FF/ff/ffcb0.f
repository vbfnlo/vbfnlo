*	$Id: vbfffcb0.f,v 1.11 1996/07/18 10:49:04 gj Exp $
*###[ vbfffcb0:
	subroutine vbfffcb0(cb0,d0,xmu,cp,cma,cmb,ier)
***#[*comment:***********************************************************
*									*
...
*
	if ( lwrite ) then
	    print *,'vbfffcb0: input:'
	    print *,'cma,cmb,cp,ier = ',cma,cmb,cp,ier
	endif
	if ( ltest ) then
	    if ( DIMAG(cma) .gt. 0 .or. DIMAG(cmb) .gt. 0 ) then
		print *,'vbfffcb0: error: Im(masses) > 0: ',cma,cmb
		stop
	    endif
	    if ( DBLE(cma) .lt. 0 .or. DBLE(cmb) .lt. 0 ) then
		print *,'vbfffcb0: error: Re(masses) < 0: ',cma,cmb
		stop
	    endif
	    if ( DIMAG(cp) .gt. 0 ) then
		print *,'vbfffcb0: error: Im(p^2) > 0: ',cp
		ier = ier + 100
	    endif
	    if ( DIMAG(cp) .ne. 0 .and. DBLE(cp) .le. 0 ) then
		print *,'vbfffcb0: error: cannot handle Re(p^2)<0, '//
     +			'Im(p^2)<0: ',cp
		ier = ier + 100
	    endif
	endif
...
*
	if ( DIMAG(cma) .eq. 0 .and. DIMAG(cmb) .eq. 0 .and. 
     +		DIMAG(cp).eq.0 ) then
	    lreal = .TRUE.
	    if ( lwrite ) print *,'vbfffcb0: real masses'
	elseif ( nschem.le.4 ) then
	    lreal = .TRUE.
	    if ( lwrite .or. init.eq.0 ) then
		init = 1
		print *,'vbfffcb0: nschem <= 4, ignoring complex masses: ',
     +			nschem
	    endif
	elseif ( nschem.le.6 ) then
	    if ( lwrite .or. init.eq.0 ) then
		init = 1
		print *,'vbfffcb0: nschem = 5,6 complex masses near ',
     +			'threshold: ',nschem
	    endif
	    cqi(1) = cma
	    cqi(2) = cmb
	    ...
	    cqiqj(2,2) = 0
	    cqiqj(3,3) = 0
	    call vbfffthre(ithres,cqi,cqiqj,3,1,2,3)
	    if ( ithres.eq.0 .or. ithres.eq.1 .and. nschem.eq.5 ) then
		lreal = .TRUE.
		if ( lwrite ) print *,'vbfffcb0: no threshold'
	    else
		lreal = .FALSE.
		if ( lwrite ) print *,'vbfffcb0: found threshold'
	    endif
	else
	    lreal = .FALSE.
	endif
	if ( lreal ) then
	    ...
	    sprec = precx
	    precx = precc
	    if ( lwrite ) print *,'vbfffcb0: to real case'
	    call vbfffxb0(cb0,d0,xmu,xp,xma,xmb,ier)
	    precx = sprec
	    if ( ldot ) then
		do 120 j=1,3
		    do 110 i=1,3
...
*
	...
	if ( lwarn ) then
     ...
     +		call vbfffwarn(94,ier0,absc(cmamb),absc(cmb))
	    if ( absc(cmap) .lt. xloss*absc(cp) .and. cma .ne. cp )
     +		call vbfffwarn(95,ier0,absc(cmap),absc(cp))
	    if ( absc(cmbp) .lt. xloss*absc(cp) .and. cmb .ne. cp )
     +		call vbfffwarn(96,ier0,absc(cmbp),absc(cp))
	endif
*
*  #] get differences: 
*  #[ calculations:
...
*
	nschsa = nschem
	nschem = 7
	call vbfffcb0p(cb0p,cp,cma,cmb,cmap,cmbp,cmamb,ier)
	nschem = nschsa
	if ( cma .eq. 0 ) then
	    if ( cmb .eq. 0 ) then
	    	cm = 1
	...
	if ( absc(cm) .gt. xclogm ) then
     ...
     +		call vbfffwarn(149,ier,absc(cb0),smax)
	else
	    call vbffferr(3,ier)
	    cb0 = -cb0p + DBLE(d0)
	endif
...
*  #[ check output:
	if ( ltest ) then
     ...
     +	    		sqrt(abs(cmb)))**2 ) then
	    	print *,'vbfffcb0: warning: sign imaginary part looks '//
     +	    		'suspicious: ',cb0
...
*###[ vbfffcb0p:
	subroutine vbfffcb0p(cb0p,cp,cma,cmb,cmap,cmbp,cmamb,ier)
***#[*comment:***********************************************************
*									*
...
*	local variables
*
	integer i,j,initeq,initn1,n1,n2,vbfnffeta,vbfnffet1,ier0,init,
     +		ithres,is1
	logical lwsave,lreal
	DOUBLE PRECISION xp,ax,ay,vbfffbnd,
	...
	DOUBLE COMPLEX check,cm,cmp,cm1,cm2,cm1m2,
     ...
     +		vbfzfflo1,c,zm,zp,zm1,zp1,vbfzfflog,cb0r,cqi(3),
...
*
	if (ltest) then
     ...
     +			cmamb))/xloss ) then
		print *,'vbfffcb0p: input not OK, cmamb /= cma - cmb',check
	    endif
	    check = cp - cma + cmap
	    if ( absc(check) .gt. precc*max(absc(cp),absc(cma),absc(
     +			cmap))/xloss ) then
		print *,'vbfffcb0p: input not OK, cmap /= cma - cp',check
	    endif
	    check = cp - cmb + cmbp
	    if ( absc(check) .gt. precc*max(absc(cp),absc(cmb),absc(
     +			cmbp))/xloss ) then
		print *,'vbfffcb0p: input not OK, cmbp /= cmb - cp',check
	    endif
	endif
*
*  #] check input: 
...
*
	call vbfffcot2(cpiDpj,cp,cma,cmb,cmap,cmbp,cmamb,ier)
	if ( ldot ) then
	    do 20 i=1,3
		do 10 j=1,3
...
*
	if ( DIMAG(cma) .eq. 0 .and. DIMAG(cmb) .eq. 0 .and. 
     +		DIMAG(cp).eq.0 ) then
	    lreal = .TRUE.
	    if ( lwrite ) print *,'vbfffcb0p: real masses'
	elseif ( nschem.le.4 ) then
	    lreal = .TRUE.
	    if ( lwrite .or. init.eq.0 ) then
		init = 1
		print *,'vbfffcb0p: nschem <= 4, ignoring complex masses:',
     +			nschem
	    endif
	elseif ( nschem.le.6 ) then
	    if ( lwrite .or. init.eq.0 ) then
		init = 1
		print *,'vbfffcb0p: nschem = 4,6 complex masses near ',
     +			'threshold: ',nschem
	    endif
	    cqi(1) = cma
	    cqi(2) = cmb
	    ...
	    if ( ithres.eq.0 .or. ithres.eq.1 .and. nschem.eq.5 ) then
		lreal = .TRUE.
		if ( lwrite ) print *,'vbfffcb0p: no threshold'
	    else
		if ( lwrite ) print *,'vbfffcb0p: found threshold'
		lreal = .FALSE.
	    endif
	else
	    lreal = .FALSE.
	...
	if ( lreal ) then
	    ...
	    precx = precc
	    if ( lwrite ) print *,'vbfffcb0: to real case'
	    call vbfffxb0p(cb0p,xp,xma,xmb,dmap,dmbp,dmamb,ier)
	    precx = sprec
	    if ( ldot ) then
		do 120 j=1,3
		    do 110 i=1,3
...
*	make sure we get the right Riemann sheet!
	if ( absc(cs1) .lt. xloss ) then
	    cs = vbfzfflo1(cs1,ier)
	elseif ( DBLE(cs2).gt.0 ) then
	    cs = vbfzfflog(cs2,0,c0,ier)
	else
	    cs = vbfzfflog(-cs2,0,c0,ier)
	    cs = cs - c2ipi/2
	endif
...
*
	    if ( lwrite ) print*,'vbfffcb0: equal masses, Taylor expansion'
*  #[ data and bounds:
	    if ( initeq .eq. 0 ) then
		initeq = 1
		xpneq(1) = x1/6
...
*
		...
		sprec = precx
		precx = precc
		bdeq01 = vbfffbnd(1,1,xpneq)
		bdeq05 = vbfffbnd(1,5,xpneq)
		bdeq11 = vbfffbnd(1,11,xpneq)
		bdeq17 = vbfffbnd(1,17,xpneq)
		bdeq25 = vbfffbnd(1,25,xpneq)
		precx = sprec
	    endif
*  #] data and bounds: 
	    cx = cp/cm
		...
		call vbfffwarn(2,ier,precc,xpneq(25)*ax**25)
	    endif
...
*
	if ( lwrite ) print*,'vbfffcb0: equal masses, normal case'
	call vbfffclmb(clam,-cp,-cm,-cm,cmp,cmp,c0,ier)
	cslam = sqrt(clam)
	call vbfffcoot(zm,zp,c1,c05,cm/cp,cslam/(2*cp),ier)
...
*	    calculate log(1-cx).
	    ...
	    cs = vbfzfflo1(cs2/(2*cm),ier)
	else
*	    finally the normal case
	    cs = vbfzfflog(cs1,0,c0,ier)
	endif
	cs = cslam*cs/cp
...
*	eta terms
*
	n1 = vbfnffet1(zp,1/zm,cs1,ier)
	if ( ltest .and. n1.ne.0 ) print *,'vbfffcb0: surprise! n1= ',n1
	if ( DIMAG(cp).eq.0 ) then
	    n2 = vbfnffet1(-zp,-1/zm,cs1,ier)
	else
*	    use the onshell expression to get the correct continuation
	    ck = DBLE(cp)
	    call vbfffclmb(clamr,-ck,-cm,-cm,cm-ck,cm-ck,c0,ier)
	    cslamr = sqrt(clamr)
	    call vbfffcoot(zmr,zpr,c1,c05,cm/ck,cslamr/(2*ck),ier)
     ...
     +	    		) then
	    ...
	    if ( DIMAG(zmr).eq.0 .or. DIMAG(zpr).eq.0 ) then
	    	...
	    	n2 = vbfnffeta(-zpr,-1/zmr,ier)
	    endif
	endif
	if ( ltest .and. DBLE(cp).gt.0 .and. n2.eq.0 ) print *,
     +		'vbfffcb0: surprise! n2= ',n2
	if ( lwrite .and. (n1.ne.0 .or. n2.ne.0) ) then
	    print *,'vbfffcb0: eta terms: n1,n2 = ',n1,n2
	endif
	if ( n1+n2 .ne. 0 )
...
*	also superfluous - just to make sure
	if ( lwarn .and. absc(cb0p) .lt. xloss*max(x2,absc(cs)) )
     +		call vbfffwarn(4,ier,absc(cb0p),x2)
	goto 990
* -#] normal case: 
*
*  #] both masses equal: 
...
* -#[	get log(xm2/xm1):
	...
	elseif ( absc(c) .lt. xloss ) then
	    clogmm = vbfzfflo1(cm1m2/cm1,ier)
	else
	    clogmm = log(cx)
	endif
* -#]	get log(xm2/xm1): 
...
*		    determine the boundaries for 1,5,10,15 terms
			...
			sprec = precx
			precx = precc
			bdn101 = vbfffbnd(1,1,xpnn1)
			bdn105 = vbfffbnd(1,5,xpnn1)
			bdn110 = vbfffbnd(1,10,xpnn1)
			bdn115 = vbfffbnd(1,15,xpnn1)
			precx = sprec
		    endif
*--#]		    data and bounds: 
*		    calculate:
		    if ( lwarn .and. ax .gt. bdn115 )
     +			call vbfffwarn(5,ier,precc,abs(xpnn1(15))*ax**15)
...
*
			...
			sprec = precx
			precx = precc
			bdn201 = vbfffbnd(4,1,xinfac)
			bdn205 = vbfffbnd(4,5,xinfac)
			bdn210 = vbfffbnd(4,10,xinfac)
			bdn215 = vbfffbnd(4,15,xinfac)
			precx = sprec
		    endif
*  #]		    bounds: 
*		    calculate:
     ...
     +			call vbfffwarn(6,ier,precc,xinfac(18)*ax**15)
...
*
	if ( lwrite ) print *,'vbfffcb0: general case, cp,cm1,cm2 = ',cp,
     +		cm1,cm2
	call vbfffclmb(clam,cm1,cm2,cp,cm1m2,cm1p,cm2p,ier)
	cslam = sqrt(clam)
	if ( is1.eq.1 ) then
	    cs = +cpiDpj(2,3)
	else
	...
	call vbfffcoot(zm,zp,cp,cs,cm2,cslam/2,ier)
	zm1 = 1-zm
	zp1 = 1-zp
	if ( absc(zm1) .lt. xloss .or. absc(zp1) .lt. xloss ) then
	    ...
	    call vbfffcoot(zp1,zm1,cp,cs,cm1,cslam/2,ier)
...
*	    compute roots for Im(cp).eq.0 for continuation terms.
     ...
     +	    	cslamr = -cslamr
	    ...
	    if ( lwrite ) then
		print *,'vbfffcb0: zmr = ',zmr,zm1r
		print *,'vbfffcb0: zpr = ',zpr,zp1r
	    endif
	else
	    zmr  = zm
	    zm1r = zm1
	...
	call vbfffc1lg(cs1,zm,zm1,zmr,zm1r,-1,ier)
	call vbfffc1lg(cs2,zp,zp1,zpr,zp1r,+1,ier)
	cb0p = -clogmm/2 + cs1 + cs2
	smax = max(absc(clogmm)/2,absc(cs1),absc(cs2))
	if ( absc(cb0p) .lt. xloss*smax ) then
	    call vbfffwarn(7,ier,absc(cb0p),smax)
	endif
	if ( lwrite ) then
...
*  #[ check output:
	if ( .FALSE. .and. ltest ) then
	    ...
	    lwsave = lwrite
	    lwrite = .FALSE.
	    call vbfffxb0p(cb0r,xp,xma,xmb,dmap,dmbp,dmamb,ier0)
	    lwrite = lwsave
	    precx = sprec
     ...
     +			.gt. 2*precc/rloss ) then
		print *,'vbfffcb0p: warning: complex result differs very ',
...
*###[ vbfffc1lg:
	subroutine vbfffc1lg(cs,z,z1,zr,z1r,is,ier)
***#[*comment:***********************************************************
*									*
...
*	    normal case
*
	    if ( lwrite ) print *,'vbfffc1lg: normal case',z,z1
	    cs = -1 - z*vbfzfflog(-z1/z,0,c0,ier)
*
...
*		    check whetehr we chose the correct continuation
     ...
     +				is.eq.+1) .and. DIMAG(-z1/z).lt.0 ) then
			cs = cs - c2ipi*z
			if ( lwrite ) print*,'vbfffc1lg: added 2ipi to log'
		    elseif ( (DIMAG(c).lt.0 .or. DIMAG(c).eq.0 .and.
     +				is.eq.-1) .and. DIMAG(-z1/z).gt.0 ) then
			cs = cs + c2ipi*z
			if ( lwrite ) print*,'vbfffc1lg: subtracted 2ipi'//
     +				' from log'
		    endif
		endif
	    endif
	    if ( absc(cs) .lt. xloss ) call vbfffwarn(8,ier,absc(cs),x1)
	else
*
*	    Taylor expansion
*
	    if ( lwrite ) print *,'vbfffc1lg: Taylor',z,z1
	    call vbfffcayl(cs,1/z,xninv(2),29,ier)
	endif
*  #] work: 
*###] vbfffc1lg: 
	end
*###[ vbfffcot2:
	subroutine vbfffcot2(cpiDpj,cp,cma,cmb,cmap,cmbp,cmamb,ier)
***#[*comment:***********************************************************
*									*
*	Store the 3 dotproducts in the common block vbfffdot.		*
*									*
*	Input:	see vbfffxc0p						*
*									*
***#]*comment:*********************************************************** 
...
*  #[ work:
	...
	if ( lwarn .and. absc(cpiDpj(1,2)) .lt. xloss*xmax ) then
		call vbfffwarn(10,ier1,absc(cpiDpj(1,2)),xmax)
	endif
	...
	if ( lwarn .and. abs(cpiDpj(1,3)) .lt. xloss*xmax ) then
		ier0 = ier
		call vbfffwarn(11,ier0,absc(cpiDpj(1,3)),xmax)
		ier1 = max(ier0,ier1)
	endif
	...
	if ( lwarn .and. absc(cpiDpj(2,3)) .lt. xloss*xmax ) then
		ier0 = ier
		call vbfffwarn(11,ier,absc(cpiDpj(2,3)),xmax)
		ier1 = max(ier0,ier1)
	endif
	ier = ier1
*  #] work: 
