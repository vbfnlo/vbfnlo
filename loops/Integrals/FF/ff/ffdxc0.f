*###[ vbfffdxc0:
	subroutine vbfffdxc0(cs3,ipi12,isoort,clogi,ilogi,xpi,dpipj,piDpj,
...
*
	...
	DOUBLE PRECISION sdel2i(3,3:4),s(5),som,smax,absc,vbfdfflo1,xhck,
...
*  #[ check input:
	if ( ltest ) then
*	    call vbfffxhck(xpi(1,3),dpipj(1,1,3),6,ier)
*	    call vbfffxhck(xpi(1,4),dpipj(1,1,4),6,ier)
...
*	    then handle the special case Si = 0
     ...
     +		     i .eq. 3 .and. alph(1) .eq. 0 ) then
	    ...
	    call vbfffxxyz(y(1,k,i),z(1,k,i),dyz(1,1,k,i),d2yzz(k,i),
...
*	    complex case
	    ...
	    lcompl = .TRUE.
	    call vbfffcxyz(cy(1,k,i),cz(1,k,i),cdyz(1,1,k,i),cd2yzz(k,i),i,
...
*			-6, -106 if they have unequal roots, isoort=+1
	    ...
	    if ( k .ne. 0 ) then
		if ( lwrite ) print *,'vbfffdxc0: converting i,k=',i,k,
     +			' to complex'
		do 30 j=1,4
		    cy(j,k,i) = y(j,k,i)
		    cz(j,k,i) = z(j,k,i)
...
*  #[	    real case: (note that this implies isoort(11)>0)
 ...
 1000		format(a,i1,a,g22.14,g12.4)
     ...
     +			dyzzy(ii,2),xmax
		if ( ldel2s ) then
		    print *,'vbfffdxc0: not ready for del2s=0, real case'
		    goto 130
		endif
		if ( ifirst .le. 0 ) then
...
*		    print *,(s(i)/xqi(6)**2,i=1,5)
  ...
  130		continue
		if ( lwarn .and. abs(dyzzy(ii,2)) .lt. xloss*xmax ) then
		    call vbfffwarn(140,ieri(2*k+j-2),dyzzy(ii,2),xmax)
		endif
  140	    continue
	    ifirst = ifirst - 1
  150	    continue
...
***		    print *,(cs(i)/DBLE(xqi(ip))**2,i=1,5)
		...
		if ( lwarn .and. absc(cdyzzy(ii,2)).lt.xloss*xmax ) then
		  call vbfffwarn(140,ieri(2*k+j-2),absc(cdyzzy(ii,2)),xmax)
		endif
  170	    continue
*
*	    get cdyyzz
*
     ...
     +			 mod(isoort(11),10).ne.0 ) then
		...
		if ( lwarn .and. absc(cdyyzz(j,2)).lt.xloss*xmax ) then
		    call vbfffwarn(147,ieri(7+j),absc(cdyyzz(j,2)),xmax)
		endif
	    endif
*
*	    bookkeeping
...
***		    print *,(s(i)/xqi(8)**2,i=1,5)
  ...
  190		continue
		if ( lwarn .and. abs(dyzzy(ii,3)) .lt. xloss*xmax ) then
		    call vbfffwarn(140,ieri(2*k+j+2),dyzzy(ii,3),xmax)
		endif
  200	    continue
	    ifirst = ifirst - 1
  210	    continue
...
***		    print *,(cs(i)/DBLE(xqi(ip))**2,i=1,5)
		...
		if ( lwarn .and. absc(cdyzzy(ii,3)).lt.xloss*xmax ) then
		  call vbfffwarn(140,ieri(2*k+j+2),absc(cdyzzy(ii,3)),xmax)
		endif
  230	    continue
*
*	    get cdyyzz
*
     ...
     +			 mod(isoort(13),10).ne.0 ) then
		...
		if ( lwarn .and. absc(cdyyzz(j,3)).lt.xloss*xmax ) then
		    call vbfffwarn(147,ieri(9+j),absc(cdyyzz(j,3)),xmax)
		endif
	    endif
*
*	    bookkeeping
...
*  #[ check differences:
	if ( ltest ) then
     ...
     +			'vbfffdxc0: error: ','dyzzy(',j,i,') <> terms, ',
     ...
     +			'vbfffdxc0: error: ','dyzzy(',j+2,i,') <> terms, ',
     ...
     +			'vbfffdxc0: error: ','cdyzzy(',j,i,') <> terms, ',
     ...
     +			'vbfffdxc0: error: ','cdyzzy(',j+2,i,') <> terms,',
...
*  #[		real case:
*
*		the real case (isoort=-5,-6: really real but complex for vbfffdcs)
*
		s(1) = -dyz(2,1,k,i)/dyz(2,2,k,i)
		if ( lwrite ) then
*		    fantasize imag part, but suppress error message
		    clogi(ii) = vbfzxfflg(s(1),1,x1,ier0)
		    print *,'clogi  = ',clogi(ii)
		endif
		if ( abs(s(1)-1) .lt. xloss ) then
		    clogi(ii) = vbfdfflo1(d2yzz(k,i)/dyz(2,2,k,i),ier)
		    ilogi(ii) = 0
		else
		    if ( abs(s(1)+1) .lt. xloss ) then
			clogi(ii) = vbfdfflo1(-2*sdel2i(i,k)/(xpi(i+3,k)*
     +				dyz(2,2,k,i)),ier)
		    else
			clogi(ii) = vbfzxfflg(abs(s(1)),0,x0,ier)
		    endif
...
*
     ...
     +				xalog2)
		    ...
		    clogi(ii)=vbfzfflog(csom,0,c0,ier0)
...
*	    Note that we generate an error if isoort(l)=0
	    if ( lwrite ) then
		print *,'vbfffdxc0: ',ii,': ',clogi(ii),' + ',ilogi(ii),
     +			'*i*pi'
	endif
  410	continue
...
*  #[	    subtract more smartly:
		...
		if ( lwrite ) print *,'vbfffdxc0: extra logs not ready ',
     +			'in the real case'
		goto 420
	    else
		cs(1) = cdyzzy(1,i)
		...
		if ( i .eq. 1 ) then
		    ...
		    if ( lwrite ) print *,'vbfffdxc0: extra logs not ',
     +			'ready for i <>1'
		    goto 420
		endif
		csom = cs(1) - cs(2) + cs(3)
...
*		change this to "no warning and quit" later
		if ( lwarn .and. absc(csom) .lt. xmax ) then
		    goto 420
***		    call vbfffwarn(148,ier,absc(csom),xmax)
		endif
...
*		
		if ( abs(clogi(ii-3)-clogi(ii)-c).gt.0.1 ) then
		    print *,'vbfffdxc0: error in smart logs: ',clogi(ii-3)-
     +		    	clogi(ii),c,' not used'
		    goto 420
		endif
		clogi(ii-3) = c
...
*	accurately has been put in vbfffcc0p, not yet here
	if ( lwrite ) print '(a)','  ##] logarithms for Ai<0:'
	endif
*  #] logarithms for 4point function:
*  #[ real case integrals:
...
*	    normal case
		...
		if ( isoort(j) .eq. 0 ) then
		    if ( lwrite ) then
			print *,'vbfffdxc0: xk=0, ma=mb/Si-0 -> S3 = 0'
			...
			call vbfffcxs3(cs3(20*i+61),ipi12(j+8),y(1,4,i),
     ...
     +			   xpi(1,4),piDpj(1,1,4),i,6,isoort(j+8),ier)
		...
		elseif ( isoort(j+8) .eq. 0 ) then
		    ...
		    call vbfffcxs3(cs3(20*i-19),ipi12(j),y(1,3,i),
     +			   z(1,3,i),dyz(1,1,3,i),d2yzz(3,i),dy2z(1,3,i),
     +			   xpi(1,3),piDpj(1,1,3),i,6,isoort(j),ier)
		else
		    call vbfffdcxs(cs3(20*i-19),ipi12(j),y(1,3,i),z(1,3,i),
  ...
  540	    continue
     ...
     +			   xpi(1,3),piDpj(1,1,3),i,6,isoort(j),ier)
		elseif ( isoort(j) .gt. 0 ) then
			...
			call vbfffdcxs(cs3(20*i-19),ipi12(j),y(1,3,i),
     +			   z(1,3,i),dyz(1,1,3,i),d2yzz(3,i),dy2z(1,3,i),
     +			   dyzzy(1,i),xpi,piDpj,i,6,isoort(j),ier)
		    else
			print *,'vbfffdxc0: error: should not occur!'
			call vbfffcxs3(cs3(20*i-19),ipi12(j),y(1,3,i),
     +			   z(1,3,i),dyz(1,1,3,i),d2yzz(3,i),dy2z(1,3,i),
     +			   xpi(1,3),piDpj(1,1,3),i,6,isoort(j),ier)
			call vbfffcs3(cs3(20*i+61),ipi12(j+8),cy(1,4,i),
     +			    cz(1,4,i),cdyz(1,1,4,i),cd2yzz(4,i),
     +			    cpi(1,4),cpiDpj(1,1,4),i,6,isoort(j+8),ier)
		    endif
		else
		    if ( isoort(j+8) .lt. 0 ) then
			call vbfffdcs(cs3(20*i-19),ipi12(j),cy(1,3,i),
     +			    cz(1,3,i),cdyz(1,1,3,i),cd2yzz(3,i),
     +			    cdyzzy(1,i),cdyyzz(1,i),cpi,cpiDpj,
     +			    i,6,isoort(j),ier)
		    else
			print *,'vbfffdxc0: error: should not occur!'
			call vbfffcs3(cs3(20*i-19),ipi12(j),cy(1,3,i),
     +			    cz(1,3,i),cdyz(1,1,3,i),cd2yzz(3,i),
