*###[ vbfffzli2:
	subroutine vbfffzli2(zdilog,zlog,cx,lreal,ier)
***#[*comment:***********************************************************
*									*
...
*									*
*	Calls:	log,vbfzfflo1,(d/a)imag,real/dble				*
*									*
***#]*comment:*********************************************************** 
*  #[ declarations:
	implicit none
...
*
	DOUBLE PRECISION xprec,bdn02,bdn05,bdn10,bdn15,bdn20,
     +		xi,xr,xdilog,xlog,x,absc,xa,a,vbfffbnd
	DOUBLE COMPLEX cc,cz,cz2,vbfzfflo1
...
*  #[ exceptional cases:
	...
	xr  = DBLE(cx)
	if ( xi .eq. 0) then
	    call vbfffxli2(xdilog,xlog,xr,ier)
	    zdilog = xdilog
	    zlog = xlog
	    return
	endif
...
*  #[ get log,dilog:
	...
	if ( lwarn .and. a .gt. bdn20 ) then
	    call vbfffwarn(61,ier,precc,abs(bf(20))*a**20)
	endif
	if ( a .gt. bdn15 ) then
...
*	    think of something more intelligent later ...
	    x = DBLE(zdilog)
	    if ( lwarn ) call vbfffwarn(151,ier,x,xi**2/4)
	endif
*  #] check for numerical problems: 
*###] vbfffzli2:
	end
*###[ vbfffzzdl:
	subroutine vbfffzzdl(zdilog,ipi12,zlog,cx,ier)
***#[*comment:***************************************************
*								*
*	Computes the dilogarithm (Li2, Sp) for any (complex) cx *
...
*
	integer jsgn
	DOUBLE PRECISION xprec,bdn02,bdn05,bdn10,bdn15,bdn20,
     +		xi,xr,s1,s2,xa,a,absc,vbfffbnd
	DOUBLE COMPLEX cfact,cx1,cy,cz,cz2,vbfzfflo1,c
	save xprec,bdn02,bdn05,bdn10,bdn15,bdn20
*
*	common blocks
*
...
*  #] initialisations: 
*  #[ debug:
*	if ( lwrite ) print *,'vbfffzzdl(',cx,')'
*  #] debug: 
*  #[ exceptional cases:
	xi  = DIMAG(cx)
	xr  = DBLE(cx)
	if ( xi .eq. 0 ) then
	    if ( xr .gt. 1 ) call vbffferr(31,ier)
	    call vbfffzxdl(zdilog,ipi12,zlog,xr,1,ier)
	    return
	endif
...
*  #[ transform to |x|<1, Re(x) < 0.5:
	if ( xr .le. x05) then
		...
		if ( abs(xr-1) + abs(xi) .lt. xclogm ) then
		    if ( lwarn )
     +			call vbfffwarn(65,ier,abs(1-xr)+abs(xi),xclogm)
		    cy = 0
		else
		    zlog = log(1-cx)
		    cy = cz*zlog
...
*  #[ get dilog:
	...
	if ( lwarn .and. a .gt. bdn20 ) then
	    call vbfffwarn(67,ier,precc,abs(bf(20))*a**20)
	endif
	if ( a .gt. bdn15 ) then
	    zdilog = cz2*(DBLE(bf(16)) + cz2*(DBLE(bf(17))
     +		   + cz2*(DBLE(bf(18)) + cz2*(DBLE(bf(19))
...
*###[ vbfzfflog:
	DOUBLE COMPLEX function vbfzfflog(cx,ieps,cy,ier)
***#[*comment:***********************************************************
*									*
...
*  #[ check input:
	if ( lwarn .and. absc(cx-1) .lt. xloss ) then
	    call vbfffwarn(128,ier,absc(cx-1),x1)
	endif
*  #] check input: 
*  #[ calculations:
	xa = absc(cx)
	if ( xa .lt. xalogm ) then
	    if ( cx .ne. 0 ) call vbffferr(23,ier)
	    vbfzfflog = 0
...
*	    checked imaginary parts 19-May-1988
		...
		if ( DIMAG(vbfzfflog) .gt. 0 .neqv. DIMAG(cx) .gt. 0 )
     +		    call vbffferr(56,ier)
	    endif
	elseif ( xa .lt. xclogm .or. 1/xa .lt. xclogm ) then
	    ctroep = cx*DBLE(1/xa)
	    vbfzfflog = log(ctroep) + DBLE(log(xa))
	else
*	    print *,'vbfzfflog: neem log van ',cx
	    vbfzfflog = log(cx)
	endif
*  #] calculations: 
*###] vbfzfflog: 
...
*###[ vbfzfflo1:
	DOUBLE COMPLEX function vbfzfflo1(cx,ier)
***#[*comment:***************************************************
...
*  #[ calculations:
	xa = absc(cx)
	if ( xa .gt. bdn19 ) then
	    if ( lwarn .and. xa .lt. xloss ) call vbfffwarn(63,ier,xa,x1)
	    c = cx-1
	    xa = absc(c)
	    if ( lwarn .and. xa .lt. xloss ) call vbfffwarn(133,ier,xa,x1)
	    vbfzfflo1 = vbfzfflog(1-cx,0,c0,ier)
	    return
	endif
	if ( xa .gt. bdn15 ) then
	    vbfzfflo1 = cx*( DBLE(xninv(16)) + cx*( DBLE(xninv(17))
	...
	if ( xa .gt. bdn10 ) then
	    vbfzfflo1 = cx*( DBLE(xninv(11)) + cx*( DBLE(xninv(12))
     +		   + cx*( DBLE(xninv(13)) + cx*( DBLE(xninv(14))
     +		   + cx*( DBLE(xninv(15)) + vbfzfflo1 )))))
	endif
	if ( xa .gt. bdn05 ) then
	    vbfzfflo1 = cx*( DBLE(xninv(6)) + cx*( DBLE(xninv(7))
     +		   + cx*( DBLE(xninv(8)) + cx*( DBLE(xninv(9))
     +		   + cx*( DBLE(xninv(10)) + vbfzfflo1 )))))
	endif
	if ( xa .gt. bdn01 ) then
	    vbfzfflo1 = cx*( DBLE(xninv(2)) + cx*( DBLE(xninv(3))
     +		   + cx*( DBLE(xninv(4)) + cx*( DBLE(xninv(5))
     +		   + vbfzfflo1 ))))
	endif
	vbfzfflo1 = - cx*( DBLE(xninv(1)) + vbfzfflo1 )
*  #] calculations: 
*###] vbfzfflo1: 
	end
*###[ vbfzfflo2:
	DOUBLE COMPLEX function vbfzfflo2(x,ier)
***#[*comment:***************************************************
...
*  #[ declarations:
	implicit none
	integer ier,ier0
	DOUBLE COMPLEX x,d1,vbfzfflo1,cc
...
*  #[ calculations:
	xa = absc(x)
	if ( xa .gt. bdn18 ) then
	    vbfzfflo2 = vbfzfflo1(x,ier) + x
	    if ( lwarn .and. absc(vbfzfflo2).lt.xloss*abs(x) )
     +		call vbfffwarn(234,ier,absc(vbfzfflo2),absc(x))
	    return
	endif
	if ( xa .gt. bdn15 ) then
	    vbfzfflo2 = x*( DBLE(xninv(17)) + x*( DBLE(xninv(18)) +
     +		x*( DBLE(xninv(19)) + x*( DBLE(xninv(20)) ))))
	else
	    vbfzfflo2 = 0
	endif
	if ( xa .gt. bdn10 ) then
	    vbfzfflo2 = x*( DBLE(xninv(12)) + x*( DBLE(xninv(13)) +
     +		x*( DBLE(xninv(14)) + x*( DBLE(xninv(15)) +
     +		x*( DBLE(xninv(16)) + vbfzfflo2 )))))
	endif
	if ( xa .gt. bdn05 ) then
	    vbfzfflo2 = x*( DBLE(xninv(7)) + x*( DBLE(xninv(8)) +
     +		x*( DBLE(xninv(9)) +x*( DBLE(xninv(10)) +
     +		x*( DBLE(xninv(11)) + vbfzfflo2 )))))
	endif
	if ( xa .gt. bdn01 ) then
	    vbfzfflo2 = x*( DBLE(xninv(3)) + x*( DBLE(xninv(4)) +
     +		x*( DBLE(xninv(5)) + x*( DBLE(xninv(6)) + vbfzfflo2 ))))
	endif
	vbfzfflo2 = - x**2*( DBLE(xninv(2)) + vbfzfflo2 )
*  #] calculations: 
*  #[ check output:
	if ( ltest ) then
	    ier0 = ier
	    d1 = vbfzfflo1(x,ier0) + x
	    xheck = absc(d1-vbfzfflo2)
	    if ( xloss*abs(xheck) .gt. precc ) print *,'vbfzfflo2: error:',
     +		' answer is not OK',d1,vbfzfflo2,xheck
	endif
*  #] check output: 
*###] vbfzfflo2: 
	end
*###[ vbfzfflo3:
	DOUBLE COMPLEX function vbfzfflo3(x,ier)
***#[*comment:***************************************************
...
*  #[ declarations:
	implicit none
	integer ier,ier0
	DOUBLE COMPLEX x,d1,vbfzfflo2,cc
	DOUBLE PRECISION bdn01,bdn05,bdn10,bdn15,xprec,xa,xheck,vbfffbnd,
     +		absc
...
*  #[ calculations:
	xa = absc(x)
	if ( xa .gt. bdn15 ) then
	    vbfzfflo3 = vbfzfflo2(x,ier) + x**2/2
	    if ( lwarn .and. absc(vbfzfflo3).lt.xloss*absc(x**2)/2 )
     +		call vbfffwarn(235,ier,absc(vbfzfflo3),absc(x**2/2))
	    return
	endif
	if ( xa .gt. bdn10 ) then
	    vbfzfflo3 = x*( DBLE(xninv(13)) + x*( DBLE(xninv(14)) +
	...
	if ( xa .gt. bdn05 ) then
	    vbfzfflo3 = x*( DBLE(xninv(8)) + x*( DBLE(xninv(9)) +
     +		x*( DBLE(xninv(10)) + x*( DBLE(xninv(11)) +
     +		x*( DBLE(xninv(12)) + vbfzfflo3 )))))
	endif
	if ( xa .gt. bdn01 ) then
	    vbfzfflo3 = x*( DBLE(xninv(4)) + x*( DBLE(xninv(5)) +
     +		x*( DBLE(xninv(6)) + x*( DBLE(xninv(7)) + vbfzfflo3 ))))
	endif
	vbfzfflo3 = - x**3*( DBLE(xninv(3)) + vbfzfflo3 )
*  #] calculations: 
*  #[ check output:
	if ( ltest ) then
	    ier0 = ier
	    d1 = vbfzfflo2(x,ier0) + x**2/2
	    xheck = absc(d1-vbfzfflo3)
	    if ( xloss*abs(xheck) .gt. precc ) print *,'vbfzfflo3: error:',
     +		' answer is not OK',d1,vbfzfflo3,xheck
	endif
*  #] check output: 
*###] vbfzfflo3: 
	end
*###[ vbfzff0li:
	DOUBLE COMPLEX function vbfzff0li(r2)
***#[*comment:***********************************************************
*									*
...
*
	...
	elseif ( abs(r2).le.1.0d0 ) then
	    ...
	    print *,'vbfzff0li: error: argumnet must <= 1, not ',r2
	    zx = 0
	endif

	vbfzff0li = DCMPLX(zx,sqrt(r2-zx*zx))
*
*  #] work: 
*###] vbfzff0li: 
	end
