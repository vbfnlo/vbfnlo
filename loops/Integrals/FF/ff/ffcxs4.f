*	$Id: vbfffcxs4.f,v 1.3 1995/10/17 06:55:09 gj Exp $
*	$Log: vbfffcxs4.f,v $
c Revision 1.3  1995/10/17  06:55:09  gj
c Fixed ieps error in vbfffdcrr (vbfffcxs4.f), added real case in vbfffcrr, debugging
c info in vbfffxd0, and warned against remaining errors for del2=0 in vbfffrot4
c (vbfffxd0h.f)
c
c Revision 1.2  1995/10/06  09:17:22  gj
c Found stupid typo in vbfffxc0p which caused the result to be off by pi^2/3 in
c some equal-mass cases.  Added checks to vbfffcxs4.f vbfffcrr.f.
c
*###[ vbfffcxs4:
	subroutine vbfffcxs4(cs3,ipi12,w,y,z,dwy,dwz,dyz,d2yww,d2yzz,
...
*  #[ groundwork:
	if ( ltest .and. ns .ne. 6 )
     +	    print *,'vbfffcxs4: error: only for ns=6, not ',ns
	...
	if ( isoort(2) .ne. 0 ) then
	    ...
	    print *,'vbfffcxs4: error: untested algorithm'
...
*  #[ zm and wp:
	if ( isoort(4) .eq. 0 ) then
	    if (lwrite) print *,'vbfffcxs4: to vbfffcxr(zm)'
	    call vbfffcxr(cs3(1),ipi12(1),y(2),y(4),z(1),z(3),dyz(2,1),
     +		ld2yzz,d2yzz,z(2),z(4),.FALSE.,x00,iepz(1),ier)
	else
	    if (lwrite) print *,'vbfffcxs4: to vbfffdcxr(zm,wp)'
	    if ( .not. ( dwz(2,1).eq.0 .and. iepz(1).eq.iepw(2) ) )
     +	    	call vbfffdcxr(cs3( 1),ipi12(1),y(2),y(4),z(1),z(3),
...
*  #[ zp and wm:
	if ( isoort(2) .eq. 0 ) then
	    if (lwrite) print *,'vbfffcxs4: to vbfffcxr(wm)'
	    call vbfffcxr(cs3(1),ipi12(1),y(2),y(4),w(1),w(3),-dwy(1,2),
     +		ld2yww,d2yww,w(2),w(4),.FALSE.,x00,iepw(1),ier)
	else
	    if (lwrite) print *,'vbfffcxs4: to vbfffdcxr(zp,wm)'
	    if ( .not. ( dwz(1,2).eq.0 .and. iepz(2).eq.iepw(1) ) )
     +	    	call vbfffdcxr(cs3(21),ipi12(3),y(2),y(4),z(2),z(4),
...
*###[ vbfffcs4:
	subroutine vbfffcs4(cs3,ipi12,cw,cy,cz,cdwy,cdwz,cdyz,cd2yww,cd2yzz
...
*
	...
	DOUBLE COMPLEX vbfzfflo1,vbfzfflog
	DOUBLE PRECISION absc
*
*	common blocks
*
...
*  #[ get counters:
	if ( ltest ) then
	    if ( ns .ne. 6 ) then
	    	print *,'vbfffcs4: error: only for ns=6, not ',ns
	    	stop
	    endif
	    do i=1,4
	    	if ( ipi12(i).ne.0 ) then
	    	    print *,'vbfffcs4: error: ipi12(',i,') non-zero! ',
     +	    	    	ipi12(i)
	    	endif
	    enddo
	endif
	...
	call vbfffieps(iepz,cz,cpi(ip),cpiDpj(ip,ii),isoort)
	call vbfffieps(iepw,cw,cp2p,cpiDpj(ip,ii),isoort(3))
	if ( isoort(4) .eq. 0 ) then
	    print *,'vbfffcs4: error: case not implemented'
	    ier = ier + 50
	endif
*  #] get counters: 
*  #[ R's:
	if ( isoort(4) .eq. 0 ) then
	    call vbfffcrr(cs3(1),ipi12(1),cy(2),cy(4),cz(1),cz(3),cdyz(2,1)
     +		,ld2yzz,cd2yzz,cz(2),cz(4),isoort(4),iepz(1),ier)
	else
	    if (lwrite) print *,'vbfffcs4: to vbfffdcrr(zm,wp)'
	    if ( .not. ( cdwz(2,1).eq.0 .and. iepz(1).eq.iepw(2) ) )
     +	    call vbfffdcrr(cs3( 1),ipi12(1),cy(2),cy(4),cz(1),cz(3),cz(2),
     +		cz(4),cd2yzz,cw(2),cw(4),cw(1),cw(3),cd2yww,cdyz(2,1),
     +		cdwy(2,2),cdwz(2,1),isoort(4),iepz(1),iepw(2),ier)
	endif
	if ( isoort(2) .eq. 0 ) then
	    call vbfffcrr(cs3(1),ipi12(1),cy(2),cy(4),cw(1),cw(3),-cdwy(1,2
     +	    	),ld2yww,cd2yww,cw(2),cw(4),isoort(2),iepw(1),ier)
	else
	    if (lwrite) print *,'vbfffcs4: to vbfffdcrr(zp,wm)'
	    if ( .not. ( cdwz(1,2).eq.0 .and. iepz(2).eq.iepw(1) ) )
     +	    call vbfffdcrr(cs3(21),ipi12(3),cy(2),cy(4),cz(2),cz(4),cz(1),
...
*  #[ eta's:
	if ( DIMAG(cpi(ip)) .eq. 0 ) then
	    call vbfffgeta(nz,cz,cdyz,cd2yzz,
...
*		    take out the factor 2*pi^2
     ...
     +				absc(cz(2)),absc(cz(3))) ) then
				call vbfffwarn(134,ier,absc(c2y1),
     +					absc(cy(2)))
			    endif
			endif
			if ( lwrite ) then
			...
			call vbffferr(51,ier)
			i2pi = 0
		    endif
		    ipi12(2) = ipi12(2) - ntot*24*i2pi
		endif
	    endif
	    if ( cs3(40) .ne. 0 ) print *,'vbfffcs4: error: cs3(40) != 0'
...
*###[ vbfffdcxr:
	subroutine vbfffdcxr(cs3,ipi12,y,y1,z,z1,zp,zp1,d2yzz,
...
*	    nothing's the matter
	    ...
	    inorm = 1
	    call vbfffcxr(cs3( 1),ipi12(1),y,y1,z,z1,dyz,
     +		.TRUE.,d2yzz,zp,zp1,.FALSE.,x00,iepsz,ier)
	    call vbfffcxr(cs3(11),ipi12(2),y,y1,w,w1,-dwy,
...
*	    there are no cancellations the other way:
     ...
     +		1 ) ) then
		again = .TRUE.
		if ( lwrite ) then
		    print *,'vbfffdcxr: problems with ieps, solvable,'
		    print *,'        but for the moment just call the ',
     +		    	'normal case'
		endif
		again = .TRUE.
		goto 123
*		call vbffferr(21,ier)
	    endif
	    yy = dwy/dwz
	    zz = yy*z/y
	    yy1 = dyz/dwz
	    ...
	    call vbfffcxr(cs3( 1),ipi12(1),yy,yy1,zz,zz1,dyyzz,.FALSE.,x0,
     +			x0,x0,.FALSE.,x00,2*ieps,ier)
	    ...
	    call vbfffcxr(cs3(11),ipi12(2),yy,yy1,zz,zz1,dyyzz,.FALSE.,x0,
     +			x0,x0,.FALSE.,x00,2*ieps,ier)
	    do 20 i=11,20
		cs3(i) = -cs3(i)
   20	    continue
...
*	    do a Hill identity on the y,y-1 direction
     ...
     +			x0,x0,.FALSE.,x00,ieps,ier)
	    ...
	    zz1 = w*z1/dwz
	    dyyzz = w*w1/dwz
	    call vbfffcxr(cs3( 9),ipi12(2),yy,yy1,zz,zz1,dyyzz,.FALSE.,x0,
     +			x0,x0,.FALSE.,x00,ieps,ier)
	    do 30 i=9,16
   30		cs3(i) = -cs3(i)
	    ipi12(2) = -ipi12(2)
...
*		isign = 1
   ...
   50		continue
		if ( lwarn ) call vbfffwarn(46,ier,tot,term)
   51		continue
*		if ( isign .eq. 1 ) then
		    cs3(1) = tot
*		else
...
*		endif
     ...
     +		     .TRUE.,d2yww,wp,wp1,.FALSE.,x00,iepsw,ier)
		do 110 i=11,20
  110		    cs3(i) = -cs3(i)
*		if ( lwrite ) print *,'vbfffdcxr: Taylor expansion, 1-x'
*		print *,'NOT YET READY !!!'
*		ier = ier + 100
*		yy = y1*dwz/(z1*dwy)
*		if ( abs(yy) .lt. xloss ) then
*		    cs3(10) = -vbfdfflo1(1/y,ier)*vbfdfflo1(yy,ier)
*		else
*		    yy1 = -w1*dyz/(z1*dwy)
*		    if ( yy1 .gt. xalogm ) then
*			cs3(10) = -vbfdfflo1(1/y,ier)*log(yy1)
*		    elseif ( yy1 .gt. -xalogm ) then
*			if ( lwarn ) call vbfffwarn(80,ier,yy1,xalogm)
*		    else
*			xlogy = log(-yy1)
*			if ( lwarn .and. iepsz.ne.iepsw )
*     +				call vbfffwarn(81,ier,x1,x1)
*			if ( (w1+dyz)*dwz*y1*iepsz .lt. 0 ) then
*			    cs3(10) = -vbfdfflo1(1/y,ier)*DCMPLX(DBLE(xlogy),DBLE(pi))
*			else
*			    cs3(10) = -vbfdfflo1(1/y,ier)*DCMPLX(DBLE(xlogy),DBLE(-pi))
*			endif
*		    endif
*		endif
*		cs3(11) = -vbfdfflo1(1/z,ier)*vbfdfflo1(dwz/dwy,ier)
*		yy = dwz/(w*z1)
*		if ( abs(yy) .lt. xloss ) then
*		    cs3(12) = -vbfdfflo1(w/dwy,ier)*vbfdfflo1(yy,ier)
*		else
*		    yy1 = z*w1/(w*z1)
*		    if ( yy1 .gt. xalogm ) then
*			cs3(12) = -vbfdfflo1(w/dwy,ier)*log(yy1)
*		    elseif (  yy .gt. -xalogm ) then
*			if ( lwarn ) call vbfffwarn(80,ier,yy,xalogm)
*		    else
*			xlogy = log(-yy1)
*			if ( lwarn .and. iepsz.ne.iepsw )
*     +				call vbfffwarn(81,ier,x1,x1)
*			if ( dwz*(dwz+1)*ieps .gt. 0 ) then
*			    cs3(12) = -vbfdfflo1(w/dwy,ier)*DCMPLX(DBLE(xlogy),DBLE(pi))
*			else
*			    cs3(12) =-vbfdfflo1(w/dwy,ier)*DCMPLX(DBLE(xlogy),DBLE(-pi))
*			endif
*		    endif
...
*		d3 = dwz/dwy
		...
		if ( lwrite ) print *,'vbfffdcxr: Taylor expansion, 1/x'
		call vbffferr(22,ier)
		return
	    endif
	else
	    if ( lwrite ) print *,'Not clear, take normal route'
	    inorm = 1
	    call vbfffcxr(cs3( 1),ipi12(1),y,y1,z,z1,dyz,.FALSE.,x0,x0,x0,
     +				.FALSE.,x00,iepsz,ier)
	    call vbfffcxr(cs3(11),ipi12(2),y,y1,w,w1,-dwy,.FALSE.,x0,x0,x0,
     +				.FALSE.,x00,iepsw,ier)
	    do 40 i=11,20
   40		cs3(i) = -cs3(i)
	    ipi12(2) = -ipi12(2)
...
*  #[ debug output:
  ...
  905		cs3p(i) = 0
	    ipi12p(1) = 0
	    ipi12p(2) = 0
	    call vbfffcxr(cs3p( 1),ipi12p(1),y,y1,z,z1,dyz,.FALSE.,x0,x0,
     +				x0,.FALSE.,x00,iepsz,ier1)
	    call vbfffcxr(cs3p(11),ipi12p(2),y,y1,w,w1,-dwy,.FALSE.,x0,x0,
...
*###[ vbfffdcrr:
	subroutine vbfffdcrr(cs3,ipi12,cy,cy1,cz,cz1,czp,czp1,cd2yzz,cw,cw1
...
*  #[ groundwork:
	if ( cdwz .eq. 0 ) then
     ...
     +		iepsz .eq. iepsw ) return
	    ...
	    call vbffferr(76,ier)
	    return
	endif
	if ( cdyz .eq. 0 ) then
	    call vbffferr(77,ier)
	    return
	endif
	cc1 = cy/cdyz
	cdw = cdwz/cdyz
...
*	    nothing's the matter
	    if ( lwrite ) print *,'vbfffdcrr: Normal case'
	    inorm = 1
*	    special case to avoid bug found 15-oct=1995
	    if ( iepsz.eq.iepsw ) then
	    	if ( DIMAG(cz).eq.0 .and. DIMAG(cz1).eq.0 ) then
	    	    print *,'vbfffdcrr: flipping sign iepsz'
	    	    iepsz = -iepsz
	    	elseif ( DIMAG(cw).eq.0 .and. DIMAG(cw1).eq.0 ) then
	    	    print *,'vbfffdcrr: flipping sign iepsw'
	    	    iepsw = -iepsw
	    	else
	    	    print *,'vbfffdcrr: error: missing eta terms!'
	    	    ier = ier + 100
	    	endif
	    endif
	    call vbfffcrr(cs3(1),ipi12(1),cy,cy1,cz,cz1,cdyz,.TRUE.,
     +		cd2yzz,czp,czp1,isoort,iepsz,ier)
	    call vbfffcrr(cs3(8),ipi12(2),cy,cy1,cw,cw1,-cdwy,.TRUE.,
...
*	    there are no cancellations the other way:
	    if ( lwrite ) print *,'vbfffdcrr: Cancellations one way, ',
     +		'turned Rs'
	    cyy = cdwy/cdwz
	    czz = cz*cyy/cy
	    cyy1 = cdyz/cdwz
...
*		later try more possibilities
	    ...
	    call vbfffcrr(cs3(1),ipi12(1),cyy,cyy1,czz,czz1,cdyyzz,
...
*		later try more possibilities
	    ...
	    call vbfffcrr(cs3(8),ipi12(2),cyy,cyy1,czz,czz1,cdyyzz,
     +		.TRUE.,cd2yyz,czzp,czzp1,isoort,ieps2,ier)
	    do 20 i=8,14
		cs3(i) = -cs3(i)
   20	    continue
	    ipi12(2) = -ipi12(2)
*	    eta terms (are not calculated in vbfffcrr as ieps = 3)
	    cfactz = 1/cdyz
		...
		if ( DIMAG(cy) .eq. 0 ) then
		    ...
		    n1 = vbfnffet1(DCMPLX(DBLE(0),DBLE(iepsz)),cfactz,
     +			-cz*cfactz,ier)
		    n2 = vbfnffet1(DCMPLX(DBLE(0),DBLE(iepsz)),cfactz,
     +			cz1*cfactz,ier)
		endif
	    else
		n1 = vbfnffeta(-cz,cfactz,ier)
		n2 = vbfnffeta(cz1,cfactz,ier)
	    endif
	    cfactw = -1/cdwy
		...
		if ( DIMAG(cy) .eq. 0 ) then
		    ...
		    n4 = vbfnffet1(DCMPLX(DBLE(0),DBLE(iepsw)),cfactw,
     +			-cw*cfactw,ier)
		    n5 = vbfnffet1(DCMPLX(DBLE(0),DBLE(iepsw)),cfactw,
     +			cw1*cfactw,ier)
		endif
	    else
		n4 = vbfnffeta(-cw,cfactw,ier)
		n5 = vbfnffeta(cw1,cfactw,ier)
	    endif
*
...
*		    nothing to do
			...
			cs3(15) = n1*c2ipi*vbfzfflo1(cc1,ier)
		    else
			cc1 = -cdwy/cdyz
			cs3(15) = n1*c2ipi*vbfzfflog(cc1,0,c0,ier)
		    endif
		    cc1 = cy*cfactz
		    cc2 = cy*cfactw
			...
			n3 = vbfnffeta(cc1,1/cc2,ier)
		    endif
		    if ( n3.ne.0 ) then
			print *,'vbfffdcrr: error: untested algorithm'
			ier = ier + 50
		...
		cc1 = cy*cfactz
		cc2 = cy*cfactw
		cs3(15) = (n1*vbfzfflog(cc1,ieps1,c0,ier) + 
     +			   n4*vbfzfflog(cc2,ieps1,c0,ier))*c2ipi
	    endif
	    if ( n2.eq.n5 ) then
		if ( n2.eq.0 ) then
*		    nothing to do
		    ...
		    cc1 = cdwz/cdyz
		    if ( absc(cc1) .lt. xloss ) then
			cs3(16) = n2*c2ipi*vbfzfflo1(cc1,ier)
		    else
			cc1 = -cdwy/cdyz
			cs3(16) = n2*c2ipi*vbfzfflog(cc1,0,c0,ier)
		    endif
		    cc1 = -cy1*cfactz
		    cc2 = -cy1*cfactw
		    if ( DIMAG(cc1).eq.0 .or. DIMAG(cc2).eq.0 ) then
		...
		cc1 = -cy1*cfactz
		cc2 = -cy1*cfactw
		cs3(15) = (n2*vbfzfflog(cc1,ieps2,c0,ier) + 
     +			   n5*vbfzfflog(cc2,ieps2,c0,ier))*c2ipi
	    endif
	    if ( lwrite ) then
...
*	    do a Hill identity on the cy,cy-1 direction
	    if ( lwrite ) print *,'vbfffdcrr: Hill identity to split cz,cw'
	    cyy = -cy*cw1/cdwy
	    cyy1 = cw*cy1/cdwy
	    czz = -cz*cw1/cdwz
	    czz1 = cw*cz1/cdwz
...
*	    the extra logarithms ...
		...
		chulp = vbfzfflo1(1/cw,ier)
	    else
		chulp = vbfzfflog(-cw1/cw,0,c0,ier)
	    endif
	    cs3(15) = -vbfzfflo1(cdwz/cdwy,ier)*chulp
*  #] Hill identity: 
*  #[ Taylor expansion:
	else
*	    Do a Taylor expansion
	    if ( absc(cc1) .lt. xloss ) then
		if ( lwrite ) print *,'vbfffdcrr: Taylor expansion, normal'
		cd3 = cdwz/cdwy
*		isign = 1
		cc1n = cc1
		cd2n = cd2
   ...
   50		continue
		if ( lwarn ) call vbfffwarn(45,ier,absc(ctot),absc(cterm))
   51		continue
*		if ( isign .eq. 1 ) then
		    cs3(1) = ctot
*		else
...
*		endif
  ...
  110			cs3(i) = -cs3(i)
		ipi12(2) = -ipi12(2)
*		if ( lwrite ) print *,'vbfffdcrr: Taylor expansion, 1-x'
*		print *,'NOT YET READY !!'
*		ier = ier + 100
*		cyy = cy1*cdwz/(cz1*cdwy)
*		if ( absc(cyy) .lt. xloss ) then
*		    cs3(10) = -vbfzfflo1(1/cy,ier)*vbfzfflo1(cyy,ier)
*		else
*		    cyy1 = -cw1*cdyz/(cz1*cdwy)
*		    cs3(10) = -vbfzfflo1(1/cy,ier)*vbfzfflog(cyy1,0,cy,ier)
*		endif
*		cs3(11) = -vbfzfflo1(1/cz,ier)*vbfzfflo1(cdwz/cdwy,ier)
*		cyy = cdwz/(cw*cz1)
*		if ( absc(cyy) .lt. xloss ) then
*		    cs3(12) = -vbfzfflo1(cw/cdwy,ier)*vbfzfflo1(cyy,ier)
*		else
*		    cyy1 = cz*cw1/(cw*cz1)
*		    cs3(12) = -vbfzfflo1(cw/cdwy,ier)*vbfzfflog(cyy1,0,c0,ier)
*		endif
*		isign = -1
*		cc1 = -cz/cdyz
*		cd2 = 1/cz
...
*  #[ debug output:
  ...
  905			cs3p(i) = 0
		ipi12p(1) = 0
		ipi12p(2) = 0
		call vbfffcrr(cs3p(1),ipi12p(1),cy,cy1,cz,cz1,cdyz,
     +			.TRUE.,cd2yzz,czp,czp1,isoort,iepsz,ier1)
		call vbfffcrr(cs3p(8),ipi12p(2),cy,cy1,cw,cw1,-cdwy,
