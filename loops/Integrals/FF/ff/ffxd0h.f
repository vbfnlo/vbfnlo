*--#[ log:
*	$Id: vbfffxd0h.f,v 1.6 1996/01/22 13:33:49 gj Exp $
*	$Log: vbfffxd0h.f,v $
c Revision 1.6  1996/01/22  13:33:49  gj
c Added the word 'error' to print statements in vbfffxuvw that u,v,w were wrong
c
...
c JUst added some comments in vbfffrot4
c
...
*--#] log: 
*###[ vbfffrot4:
	subroutine vbfffrot4(irota,del2,xqi,dqiqj,qiDqj,xpi,dpipj,piDpj,ii,
     +		itype,ier)
***#[*comment:***********************************************************
*									*
...
*  #[ check input:
	if ( ltest ) then
		...
		if ( xpi(i) .ne. piDpj(i,i) ) then
		    print *,'vbfffrot4: error: xpi(',i,')!=piDpj(',i,i,
     +			'):',xpi(i),piDpj(i,i),xpi(i)-piDpj(i,i)
		endif
  890	    continue
	    endif
	    call vbfffxhck(xpi,dpipj,10,ier0)
	    call vbfffxuvw(xpi,dpipj,ier0)
	    if ( ier0 .gt. ier ) print *,'vbfffrot4: error: input wrong!'
	endif
*  #] check input:
...
*
	    ier0 = 0
	    call vbfffxlam(del2,xpi,dpipj,10,
...
*
   ...
   40	continue
		...
		if ( init.lt.10 ) then
		    init = init + 1
	print *,'vbfffrota: warning: the algorithms for del2=0 have not '
   ...
   50	continue
	if ( lwrite ) then
	    print *,'vbfffrot4: chose permutation no ',irota
	endif
...
*  #[ test output:
	if ( ltest ) then
	    ier0 = ier
	    call vbfffxhck(xqi,dqiqj,10,ier0)
	    call vbfffxuvw(xqi,dqiqj,ier0)
	    call vbfffxlam(del2p,xqi,dqiqj,10,3,4,7,ier0)
	    if ( del2p .ne. del2 .or. del2 .gt. 0 ) then
		print *,'vbfffrot4: error: rotated wrongly!!'
		print *,'del2  = ',del2
		print *,'del2p = ',del2p
	    endif
	    if ( ii .eq. 5 ) then
		call vbfffdot4(qiDqjp,xqi,dqiqj,10,ier0)
		xlosn = xloss*DBLE(10)**(-mod(ier0,50))
		do 990 i=1,10
		    do 980 j=1,10
			if ( xlosn*abs(qiDqjp(j,i)-qiDqj(j,i)).gt.precx*
     +			     abs(qiDqjp(j,i)) ) print*,'vbfffrot4: error ',
...
*###[ vbfffxlam:
	subroutine vbfffxlam(xlam,xpi,dpipj,ns,i1,i2,i3,ier)
*************************************************************************
*									*
...
*
	...
	if ( lwarn .and. abs(xlam) .lt. xloss*s2 )
     +		call vbfffwarn(71,ier,xlam,s2)
*###] vbfffxlam:
	end
*###[ vbfffdot4:
	subroutine vbfffdot4(piDpj,xpi,dpipj,ns,ier)
***#[*comment:***********************************************************
*									*
...
*  #] declarations:
*  #[ check input:
	if ( ns .ne. 10 ) print *,'vbfffdot4: error: ns <> 10 '
	if ( ltest ) then
	    call vbfffxhck(xpi,dpipj,10,ier)
	    call vbfffxuvw(xpi,dpipj,ier)
	endif
*  #] check input:
...
*
     ...
     +		xloss*min(xpi(is1),xpi(is2)) )call vbfffwarn(105,ier0,
...
*
     ...
     +			xloss*min(xpi(is1),xpi(is3)) ) call vbfffwarn(106,
...
*
     ...
     +		xloss*min(abs(xpi(ip1)),xpi(is1))) call vbfffwarn(107,ier0,
...
*
     ...
     +		xloss*min(abs(xpi(ip1)),xpi(is2))) call vbfffwarn(108,ier0,
...
*
     ...
     +		xloss*min(abs(xpi(ip3)),xpi(is1))) call vbfffwarn(109,ier0,
...
*
     ...
     +		call vbfffwarn(110,ier0,piDpj(ip1,is3),
...
*
     ...
     +		call vbfffwarn(111,ier0,piDpj(ip2,is1),
...
*
     ...
     +		call vbfffwarn(112,ier0,piDpj(ip3,is2),
...
*
     ...
     +		vbfffwarn(113,ier0,piDpj(ip1,ip2),
...
*
     ...
     +		vbfffwarn(114,ier0,piDpj(ip2,ip3),
...
*
     ...
     +		vbfffwarn(115,ier0,piDpj(ip3,ip1),
...
*
     ...
     +		vbfffwarn(116,ier0,piDpj(5,7),xmax)
	    ier1 = max(ier1,ier0)
	endif
	piDpj(7,5) = piDpj(5,7)
*--#] p5.p7:
...
*
     ...
     +		vbfffwarn(117,ier0,piDpj(6,8),xmax)
	    ier1 = max(ier1,ier0)
	endif
	piDpj(8,6) = piDpj(6,8)
*--#] p6.p8:
...
*
     ...
     +		vbfffwarn(118,ier0,piDpj(9,10),xmax)
	    ier1 = max(ier1,ier0)
	endif
...
*  #[ check:
   ...
   20		continue
     ...
     +			'vbfffdot4: error: dotproducts with p(',i,
     ...
     +			'vbfffdot4: error: piDpj(',i,j,') <> piDpj',j,i,')'
   30		continue
   40	    continue
	endif
*  #] check:
...
*###[ vbfffxuvw:
	subroutine vbfffxuvw(xpi,dpipj,ier)
***#[*comment:***********************************************************
*									*
...
*  #[ check!:
	...
	if ( xlosn*abs(xheck) .gt. precx*xmax ) print *,
     +		'vbfffxuvw: error: u wrong! ',xpi(11),+xpi(5)+xpi(6)+xpi(7)
     +		+xpi(8)-xpi(9)-xpi(10),xheck,xmax
	xheck = -xpi(12)-xpi(5)+xpi(6)-xpi(7)+xpi(8)+xpi(9)+xpi(10)
	if ( xlosn*abs(xheck) .gt. precx*xmax ) print *,
     +		'vbfffxuvw: error: v wrong! ',xpi(12),-xpi(5)+xpi(6)-xpi(7)
     +		+xpi(8)+xpi(9)+xpi(10),xheck,xmax
	xheck = -xpi(13)+xpi(5)-xpi(6)+xpi(7)-xpi(8)+xpi(9)+xpi(10)
	if ( xlosn*abs(xheck) .gt. precx*xmax ) print *,
     +		'vbfffxuvw: error: w wrong! ',xpi(13),xpi(5)-xpi(6)+xpi(7)-
     +		xpi(8)+xpi(9)+xpi(10),xheck,xmax
	do 20 i=10,13
	    do 10 j=1,10
     ...
     +		    'vbfffxuvw: error: dpipj(',j,i,') != xpi(',j,')-xpi(',
...
*###[ vbfffgdt4:
	subroutine vbfffgdt4(piDpj,xpip,dpipjp,xpi,dpipj,itype,ier)
***#[*comment:***********************************************************
*									*
*	calculate the dotproducts pi.pj with				*
...
*
	call vbfffdot4(piDpj,xpip,dpipjp,10,ier)
...
*		(we messed around with the xpi if itype=2)
     ...
     +				precx*abs(fpij4(j,i)) ) then
			    print *,
     +			    'vbfffxd0: error: fpij4(',j,i,') not correct!',
...
*		if all spacelike everything is OK!
     ...
     +			.and. xpi(iperm(3,i)).lt.0 ) goto 60
		call vbfffdel2(del2,piDpj,10,iperm(1,i),iperm(2,i),
     +						iperm(3,i), 1,ier0)
		if ( del2 .gt. 0 ) then
...
*		    if ( lwrite )
   ...
   60	    continue
	...
	if ( ldot .or. ltest ) then
		...
		ii(6)= 10
		fidel3 = ier
		call vbfffdl3p(dl3p,piDpj,10,ii,ii,fidel3)
		fdel3 = dl3p
	    else
		dl3p = fdel3
	    endif
