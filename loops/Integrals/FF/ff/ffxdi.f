*###[ vbfffxdi:
	subroutine vbfffxdi(cd4pppp,cd4ppdel,cd4deldel, cd3ppp,cd3pdel,
...
*  #[ check input:
	if ( lwrite ) then
	    print *,'vbfffxdi: input:'
	    print *,'  degree ',degree
	    print *,'  xpi  = ',xpi
	    print *,'  ier  = ',ier
	endif
	if ( degree .gt. 2 ) then
	    print *,'vbfffxdi: degree > 2 not yet supported: ',degree
	    stop
	endif
	if ( del2pi(1).eq.0 .or. del2pi(2).eq.0 .or. del2pi(3).eq.0
     +		.or. del2pi(4).eq.0 ) then
	    call vbffferr(87,ier)
	    return
	endif
	if ( ltest ) then
*
...
*
		...
		do 50 j=i+1,4
		    ...
		    lsave2 = lwrite
		    lwrite = .FALSE.
		    call vbfffxb0(cc,d0,xmu,xpi(inx(i,j)),xpi(i),xpi(j),
     +			ier0)
		    lwrite = lsave2
		    xlosn = xloss*DBLE(10)**(-mod(ier0,50))
		    if ( cb0ij(i,j) .ne. cb0ij(j,i) ) print *,
     +			'vbfffxdi: error: cb0ij(',i,j,') != cb0ij(',j,i,
     +			') : ',cb0ij(i,j),cb0ij(j,i)
		    if ( xlosn*abs(cc-cb0ij(i,j)) .gt. precc*abs(cb0ij(i
     +			,j)) ) print *,'vbfffxdi: error: input B0(',i,j,
...
*
		...
		lsave2 = lwrite
		lwrite = .FALSE.
		call vbfffxa0(cc,d0,xmu,xpi(i),ier0)
		lwrite = lsave2
		xlosn = xloss*DBLE(10)**(-mod(ier0,50))
		if ( xlosn*abs(cc-ca0i(i)) .gt. precc*abs(ca0i(i)) )
     +		    print *,'vbfffxdi: error: input A0(',i,') disagrees ',
...
*	    to check when called from ffzfi, ffzei
	...
	if ( degree .le. 0 ) then
	    if ( ltest ) print *,'vbfffxdi: rather useless call to vbfffxdi'
	    return
	endif
*  #] check input:
*  #[ preliminaries:
...
*
	ier1 = ier
	call vbfffdl2i(dl2pij(i2p(6,7),i2p(7,8)),piDpj,10,
     +		6,7,10,+1,7,8,9,+1,ier1)
	dl2pij(i2p(7,8),i2p(6,7)) = dl2pij(i2p(6,7),i2p(7,8))
*
	ier0 = ier
	call vbfffdl2i(dl2pij(i2p(5,8),i2p(6,7)),piDpj,10,
     +		6,7,10,+1,5,8,10,-1,ier0)
	ier1 = max(ier1,ier0)
	dl2pij(i2p(6,7),i2p(5,8)) = dl2pij(i2p(5,8),i2p(6,7))
*
	ier0 = ier
	call vbfffdl2i(dl2pij(i2p(5,6),i2p(6,7)),piDpj,10,
     +		6,7,10,+1,5,6,9,-1,ier0)
	ier1 = max(ier1,ier0)
	dl2pij(i2p(6,7),i2p(5,6)) = dl2pij(i2p(5,6),i2p(6,7))
*
	ier0 = ier
	call vbfffdl2t(dl2pij(i2p(5,7),i2p(6,7)),piDpj,5,7,
     +		6,7,10,-1,-1, 10,ier0)
	ier1 = max(ier1,ier0)
	dl2pij(i2p(6,7),i2p(5,7)) = dl2pij(i2p(5,7),i2p(6,7))
*
	ier0 = ier
	call vbfffdl2t(dl2pij(i2p(5,7),i2p(7,8)),piDpj,5,7,
     +		7,8,9,-1,-1, 10,ier0)
	ier1 = max(ier1,ier0)
	dl2pij(i2p(7,8),i2p(5,7)) = dl2pij(i2p(5,7),i2p(7,8))
*
	ier0 = ier
	call vbfffdl2t(dl2pij(i2p(5,7),i2p(5,8)),piDpj,5,7,
     +		5,8,10,+1,-1, 10,ier0)
	ier1 = max(ier1,ier0)
	dl2pij(i2p(5,8),i2p(5,7)) = dl2pij(i2p(5,7),i2p(5,8))
*
	ier0 = ier
	call vbfffdl2t(dl2pij(i2p(5,6),i2p(5,7)),piDpj,5,7,
     +		5,6,9,+1,-1, 10,ier0)
	ier1 = max(ier1,ier0)
	dl2pij(i2p(5,7),i2p(5,6)) = dl2pij(i2p(5,6),i2p(5,7))
*
	ier0 = ier
	call vbfffdl2i(dl2pij(i2p(5,6),i2p(7,8)),piDpj,10,
     +		5,6,9,-1,7,8,9,+1,ier0)
	ier1 = max(ier1,ier0)
	dl2pij(i2p(7,8),i2p(5,6)) = dl2pij(i2p(5,6),i2p(7,8))
*
	ier0 = ier
	call vbfffdl2i(dl2pij(i2p(5,6),i2p(5,8)),piDpj,10,
     +		5,6,9,-1,5,8,10,-1,ier0)
	ier1 = max(ier1,ier0)
	dl2pij(i2p(5,8),i2p(5,6)) = dl2pij(i2p(5,6),i2p(5,8))
*
	ier0 = ier
	call vbfffdl3q(dl3qi(i2p(6,7)),piDpj, 1,6,7, 0,10,0, 0,-1,0,
     +		0,+1,0, ier0)
	ier1 = max(ier1,ier0)
*
	ier0 = ier
	call vbfffdl3q(dl3qi(i2p(5,7)),piDpj, 1,5,7, 2,0,0, -1,0,0,
     +		+1,0,0, ier0)
	ier1 = max(ier1,ier0)
*
	ier0 = ier
	call vbfffdl3q(dl3qi(i2p(5,6)),piDpj, 1,2,3, 5,6,9, +1,+1,+1,
     +		-1,-1,-1, ier0)
	ier1 = max(ier1,ier0)
*
	if ( degree.gt.1 ) then
*
	ier0 = ier
	call vbfffdl3q(dl3qi(i2p(5,8)),piDpj, 1,5,8, 2,10,4, -1,-1,+1,
     +		+1,-1,+1, ier0)
	ier1 = max(ier1,ier0)
*
	ier0 = ier
	call vbfffdl3q(dl3qi(i2p(7,8)),piDpj, 3,4,1, 7,8,10, +1,+1,+1,
     +		-1,-1,-1, ier0)
	ier1 = max(ier1,ier0)
*
	ier0 = ier
	call vbfffdl3q(dl3qi(7),piDpj, 2,3,4, 6,7,10, +1,+1,+1,
     +		-1,-1,+1, ier0)
	ier1 = max(ier1,ier0)
*
	endif
...
*
  ...
  110	continue
	if ( lwarn .and. abs(cd1p(1)) .lt. xloss*xmax ) then
	    a = abs(cd1p(1))
	    call vbfffwarn(164,ier1,a,xmax)
...
*
  ...
  120	continue
	if ( lwarn .and. abs(cd1p(2)) .lt. xloss*xmax ) then
	    a = abs(cd1p(2))
	    ier0 = ier
	    call vbfffwarn(164,ier0,a,xmax)
	    ier1 = max(ier1,ier0)
...
*-	#[ D2del:
*
	if ( lwrite ) print *,'vbfffxdi: D2del'
...
*
  ...
  210	continue
	if ( lwarn .and. abs(cd2del) .lt. xloss*xmax ) then
	    a = abs(cd2del)
	    ier0 = ier
	    call vbfffwarn(189,ier0,a,xmax)
	    ier1 = max(ier1,ier0)
...
*
  ...
  220	continue
	if ( lwarn .and. abs(cd2pp(1,1)) .lt. xloss*xmax ) then
	    a = abs(cd2pp(1,1))
	    ier0 = ier
	    call vbfffwarn(190,ier0,a,xmax)
	    ier1 = max(ier1,ier0)
...
*-	#[ print output:
	...
	if ( ltest ) then
		...
		xmax = max(xmax,a)
  910	    continue
	    if ( lwrite ) print *,'vbfffxdi: checking D2.gmumu= ',cnul,xmax
	    if ( xlosn*abs(cnul) .gt. precc*xmax ) print *,'vbfffxdi: ',
     +		'error: D2(mu,mu) not correct ',cnul,xmax,ier1
		...
		xmax = max(xmax,a)
  920	    continue
	    if ( lwrite ) print *,'vbfffxdi: checking D2.p1p3 = ',cnul,xmax
	    if ( xlosn*abs(cnul) .gt. precc*xmax ) print *,'vbfffxdi :',
     +		'error: D2(p1,p3) not correct ',cnul,xmax,ier1
		...
		xmax = max(xmax,a)
  930	    continue
	    if ( lwrite ) print *,'vbfffxdi: checking D2.p2p4 = ',cnul,xmax
	    if ( xlosn*abs(cnul) .gt. precc*xmax ) print *,'vbfffxdi :',
...
*  #] D2:
	print *,'vbfffxdi: error: D3 not ready'
	stop
*###] vbfffxdi:
	end
