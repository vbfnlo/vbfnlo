*###[ vbfffcdb0:
	subroutine vbfffcdb0(cdb0,cdb0p,cp,cma,cmb,ier)
***#[*comment:***********************************************************
*									*
...
*  #[ check input:
	...
	if ( DIMAG(cma).eq.0 .and. DIMAG(cmb).eq.0 ) then
	    ...
	    xmb = DBLE(cmb)
	    xp = DBLE(cp)
	    if ( lwrite ) print *,'vbfffcdb0: calling real case'
	    call vbfffxdb0(cdb0,cdb0p,xp,xma,xmb,ier)
	    return
	endif
*  #] check input: 
*  #[ get differences:
	...
	if ( lwarn ) then
     ...
     +		call vbfffwarn(94,ier0,absc(cmamb),absc(cma))
...
*  #] get differences: 
*  #[ calculations:
	call vbfffcdbp(cdb0,cdb0p,cp,cma,cmb,cmap,cmbp,cmamb,ier)
	if ( lwrite ) then
	    print *,'   B0'' = ',cdb0,ier
...
*###[ vbfffcdbp:
	subroutine vbfffcdbp(cdb0,cdb0p,cp,cma,cmb,cmap,cmbp,cmamb,ier)
***#[*comment:***********************************************************
*									*
...
*	local variables
*
	integer i,initeq,jsign,init,ithres,initir,n1,n2,vbfnffet1
	logical lreal
	DOUBLE PRECISION ax,vbfffbnd,vbfffbndc,
	...
	DOUBLE COMPLEX xcheck,cm,cdmp,cm1,cm2,cm1m2,cdm1p,
     ...
     +		vbfzfflo1,vbfzfflo3,d1,d2,diff,h,a,b,c,d,beta,
...
*  #[ check input:
	if (ltest) then
     ...
     +			cmamb))/xloss ) then
		print *,'vbfffcdbp: input not OK, cmamb <> cma-cmb',xcheck
	    endif
	    xcheck = -cp + cma - cmap
	    if ( absc(xcheck) .gt. precc*max(absc(cp),absc(cma),absc(
     +			cmap))/xloss ) then
		print *,'vbfffcdbp: input not OK, cmap <> cma - cp',xcheck
	    endif
	    xcheck = -cp + cmb - cmbp
	    if ( absc(xcheck) .gt. precc*max(absc(cp),absc(cmb),absc(
     +			cmbp))/xloss ) then
		print *,'vbfffcdbp: input not OK, cmbp <> cmb - cp',xcheck
	    endif
	endif
*  #] check input: 
*  #[ the real cases:
*
	...
	elseif ( nschem.le.2 ) then
	    ...
	    if ( init.eq.0 ) then
		init = 1
		print *,'vbfffcb0: nschem <= 2, ignoring complex masses: ',
     +			nschem
	    endif
	elseif ( nschem.le.4 ) then
	    if ( init.eq.0 ) then
		init = 1
		print *,'vbfffcdbp: nschem = 3,4 complex masses near ',
     +			'singularity: ',nschem
	    endif
	...
	elseif ( nschem.le.6 ) then
	    if ( init.eq.0 ) then
		init = 1
		print *,'vbfffcdbp: nschem = 5,6 complex masses near ',
     +			'threshold: ',nschem
	    endif
	    cqi(1) = cma
	    cqi(2) = cmb
	...
	if ( lreal ) then
	    ...
	    sprec = precx
	    precx = precc
	    if ( lwrite ) print *,'vbfffcdbp: to real case'
	    call vbfffxdbp(cdb0,cdb0p,xp,xma,xmb,dmap,dmbp,dmamb,ier)
	    precx = sprec
	    return
	endif
*
...
* 	#[ Taylor expansion:
		...
		prcsav = precx
		precx = precc
		bdn001 = vbfffbnd(2,1,xninv)
		bdn005 = vbfffbnd(2,5,xninv)
		bdn010 = vbfffbnd(2,10,xninv)
		bdn015 = vbfffbnd(2,15,xninv)
		bdn020 = vbfffbnd(2,20,xninv)
		precx = prcsav
	    endif
	    if ( lwarn .and. ax .gt. bdn020 ) then
		call vbfffwarn(15,ier,precc,xninv(21)*ax**20)
	    endif
     ...
     +		    + cx*(DBLE(xninv(5)) + cx*(DBLE(xninv(6)) + som ))))
	    ...
	    if ( lwrite ) then
		print *,'cdb0p = ',cdb0p
		print *,'verg    ',-1 - cm/cp*vbfzfflo1(cx,ier),1
	    endif
* 	#] Taylor expansion: 
	else
* 	#[ short formula:
		...
		call vbfffwarn(13,ier,absc(cdb0p),x1)
	    endif
* 	#] short formula: 
	endif
	cdb0 = cdb0p/cp
...
*
		xprceq = precx
		bdeq01 = vbfffbndc(1,1,xpneq)
		bdeq05 = vbfffbndc(1,5,xpneq)
		bdeq11 = vbfffbndc(1,11,xpneq)
		bdeq17 = vbfffbndc(1,17,xpneq)
		bdeq25 = vbfffbndc(1,25,xpneq)
	    endif
*--#]	    data and bounds: 
	    cx = -cp/cm
	    ax = absc(cx)
	    if ( lwarn .and. ax .gt. bdeq25 ) then
		call vbfffwarn(13,ier,precc,abs(xpneq(25))*ax**25)
	    endif
     ...
     +		xpneq(5) + som ))))
	    ...
	    if (lwrite) then
		print *,'vbfffcdbp: m1 = m2, Taylor expansion in ',cx
		print *,'cdb0p = ',cdb0p
	    endif
	    if ( cp.ne.0 ) then
...
*	normal case
*
	if ( lwrite ) print*,'vbfffcdb0: equal masses, normal case'
	call vbfffclmb(clam,-cp,-cm,-cm,cdmp,cdmp,c0,ier)
	slam = sqrt(clam)
	call vbfffcoot(zm,zp,c1,c05,cm/cp,slam/(2*cp),ier)
...
*	    calculate log(1-cx).
	    ...
	    s = -2*cm/slam*vbfzfflo1(s2/(2*cm),ier)
	else
...
*	eta terms
*
	n1 = vbfnffet1(zp,1/zm,s1,ier)
	n2 = vbfnffet1(-zp,-1/zm,s1,ier)
	...
	if ( lwarn .and. absc(cdb0p).lt.xloss )
     +		call vbfffwarn(233,ier,absc(cdb0),x1)
	goto 990
* -#]	normal case: 
*
*  #] both masses equal: 
...
* -#[	get log(cm2/cm1):
	...
	elseif ( absc(c) .lt. xloss ) then
	    xlogmm = vbfzfflo1(cm1m2/cm1,ier)
	else
	    xlogmm = log(cx)
	endif
* -#]	get log(cm2/cm1): 
...
*		second try
*
		h = vbfzfflo3(cm1m2/cm1,ier)
		s1 = -cm1*h/cm1m2**2
		s2 = 1/(2*cm1)
		s3 = cm1**2*h/cm1m2**3
		s = s1 + s2 + s3
		if ( absc(s) .lt. xloss*max(absc(s2),absc(s3)) ) then
		    call vbfffwarn(234,ier,absc(s),absc(s2))
		endif
	    endif
	    cdb0 = s
	    cdb0p = 0
...
*
	call vbfffclmb(clam,-cp,-cm2,-cm1,cdm2p,cdm1p,cm1m2,ier)
	...
	if ( absc(diff) .lt. xloss*absc(clam) ) then
		...
		call vbfffwarn(235,ier,absc(diff),min(absc(cm1m2)**2,
     +			absc(clam)))
	    endif
	endif
*--#[	first try:
...
*	    looks fine
	...
	elseif ( absc(s2-1) .lt. xloss ) then
	    ier = ier + 50
	    print *,'vbfffcdb0: untested: s2 better in first try'
	    if ( jsign.eq.1 ) then
		if ( lwrite ) print *,'s2 ',-diff/(2*slam*cp)*2*log(s2)
		s2 = -slam*(s2a+slam)/(2*cm1*cm2)
		s2 = -diff/(2*slam*cp)*vbfzfflo1(s2,ier)
	    else
		if ( lwrite ) print *,'s2 ',+diff/(2*slam*cp)*2*log(s2)
		s2 = +slam*(s2a-slam)/(2*cm1*cm2)
		s2 = +diff/(2*slam*cp)*vbfzfflo1(s2,ier)
	...
	cdb0p = s1+s2-1
	if (lwrite) then
	    print *,'vbfffcdbp: first try, cdb0p = ',cdb0p,s1,s2,-1
	endif
*--#]	first try: 
...
*		    choose a quick way to get the logarithm
		...
		cdb0p = s1 + s2 - 1
		if (lwrite) then
		    print *,'vbfffcdbp: 2nd try, cdb0p  = ',cdb0p,s1,s2,-1
		endif
*--#]		second try: 
...
*		#[ split up 1:
		    ...
		    call vbfffcoot(d1,d2,a,b,c,d,ier)
     ...
     +				call vbfffwarn(236,ier,absc(alph1),xmax)
		    else
			betm2n = beta - 2/xnoe
		    endif
		    if ( lwrite ) then
...
*			determine the boundaries for 1,5,10,15 terms
			if ( xprcn5 .ne. precx ) then
			    ...
			    prcsav = precx
			    precx = precc
			    bdn501 = vbfffbnd(3,1,xinfac)
			    bdn505 = vbfffbnd(3,5,xinfac)
			    bdn510 = vbfffbnd(3,10,xinfac)
			    bdn515 = vbfffbnd(3,15,xinfac)
			    precx = prcsav
			endif
* -#]			bounds: 
			cx = beta*cp
			...
			if ( lwarn .and. ax .gt. bdn515 ) then
			    call vbfffwarn(13,ier,absc(s2p),absc(s2))
			    goto 495
			endif
     ...
     +				call vbfffwarn(237,ier,absc(s2p),absc(s2a))
			s2p = -diff/(cp*slam)*vbfzfflo1(s2p,ier)
			if (lwrite) then
			    print *,'vbfffcdbp: Taylor expansion of s2-a'
			    print *,'	     in cx = ',cx
			    print *,'	     gives s2p = ',s2p
			endif
		    endif
...
*
			...
			if ( lwarn .and. ax .gt. bdn315 ) then
			    call vbfffwarn(238,ier,absc(s1p),absc(s1))
			    goto 496
			endif
...
*
     ...
     +				absc(s1d),absc(s1e))
			if ( lwarn .and. absc(s1p).lt.xloss*xmax ) then
			    call vbfffwarn(239,ier,absc(s1p),xmax)
			endif
			s1p = s*vbfzfflo1(s1p,ier)
			if (lwrite) then
			    print *,'s1a = ',s1a
			    print *,'s1b = ',s1b
			    print *,'s1c = ',s1c
			    ...
			    print *,'s1f = ',s1f
			    print *,'s   = ',s
			    print *,'vbfffcdbp: Taylor exp. of s1-(1-a)'
			    print *,'        in cx = ',cx
			    print *,'        gives s1p = ',s1p
...
*
  496		    cdb0p = s1p + s2p
     ...
     +				then
			call vbfffwarn(240,ier,absc(cdb0p),absc(s1p))
		    endif
*--#]		    third try: 
		endif
	endif
