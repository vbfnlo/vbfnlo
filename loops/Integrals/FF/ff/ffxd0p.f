*(##[ vbfffxd0p:
	subroutine vbfffxd0p(cs4,ipi12,isoort,cfac,xpi,dpipj,piDpj,
...
*									*
*	Calls:	vbfffxc0p,ffpi34,vbfffxhck,vbfffdl3m,vbfffdel2,vbfffdel3,...		*
*									*
***#]*comment:***********************************************************
*  #[ declarations:
	implicit none
...
*  #[ check input:
	if ( ltest ) then
*	    call vbfffxhck(xpi,dpipj,10,ier)
*	    call vbfffxhck(xqi,dqiqj,10,ier)
*	    if ( ier .ne. 0 ) print *,'(input tested by vbfffxd0p)'
	endif
*  #] check input:
*  #[ preparation:
*	Note that the piDpj3(,,3) contain now the threepoint function
*	with s3, (,,4) with s4 (and NOT *without* as before)
	call vbfffpi43(xpi3(1,3),dpipj3(1,1,3),piDpj3(1,1,3),
     +		xqi,dqiqj,qiDqj,7-3,ier)
	call vbfffpi43(xpi3(1,4),dpipj3(1,1,4),piDpj3(1,1,4),
...
*
	...
	if ( .not.ldel2s ) then
	    ier0 = ier
	    call vbfffdel2(del2,qiDqj,10, 5,6,9, 0,ier0)
	    ier1 = max(ier1,ier0)
	else
	    s(1) = xqi(5)*xqi(3)
	    s(2) = qiDqj(5,3)**2
	...
	if ( ier1 .ne. ier ) then
	    ier0 = ier
	    call vbfffdel4(del4,xpi,piDpj,10,ier0)
	    if ( ldel2s ) then
...
*	    call vbffferr(44,ier)
	...
	elseif ( del2 .eq. 0 ) then
	    call vbffferr(45,ier)
	    if ( ltest ) then
		print *,'vbfffxd0p: error: del2 = 0'
		print *,'xqi = ',xqi,ier
		return
	    endif
	else
	...
	ier0 = ier
	call vbfffdl3s(del3(3),xpi,piDpj,ii,10,ier0)
	ier1 = max(ier0,ier1)
	if ( lwrite ) print *,'del3s(untransformed) 3 = ',del3(3)
	ier0 = ier
	call vbfffdl3s(del3(4),xpi,piDpj,jj,10,ier0)
	ier1 = max(ier1,ier0)
	...
	do 108 m=3,4
	    ier0 = ier
	    if ( .not.ldel2s ) then
		call vbfffdl3m(del3mi(1,m),.TRUE.,del3(m),del2,xpi3(1,m)
...
*
		call vbfffdl3m(del3mi(1,m),.FALSE.,x0,x0,xpi3(1,m),
     +		   dpipj3(1,1,m),piDpj3(1,1,m), 6, 4,3,0, 1,2,ier0)
		ier1= max(ier1,ier0)
		ier0 = ier
		call vbfffdl3m(del3mi(5,m),.FALSE.,x0,x0,xpi3(1,m),
     +		   dpipj3(1,1,m),piDpj3(1,1,m), 6, 4,3,0, 5,2,ier0)
		...
		if ( i.eq.1 .and. m.eq.4 ) then
		    ...
		    ier0 = ier
		    call vbfffdel2(del2s(i,m),piDpj,10,inx(ip,jp),ip,
     +				jp,1,ier0)
		...
		ier0 = ier
		if ( .not.ldel2s ) then
		    call vbfffdl2p(delpsi(i,m),xpi3(1,m),dpipj3(1,1,m),
     +			piDpj3(1,1,m),i+3,j+3,k+3,i,j,k,6,ier0)
		else
		    call vbfffdl2t(delpsi(i,m),qiDqj, m,5, ip,jp,inx(ip,jp)
     +			,+1,+1, 10,ier0)
		endif
		ier1 = max(ier1,ier0)
...
*
	if ( .not. lcroot ) then
     ...
     +			sdel2.gt.0) ) then
		    ...
		    lb = .TRUE.
		    call vbfffroot(blph(1),alph(1),xpi3(5,3),
     +			-piDpj3(5,6,3),xpi3(6,3),sdel2,ier)
		    call vbfffroot(alph(3),blph(3),xpi3(5,3),
     +			-piDpj3(5,4,3),xpi3(4,3),sdel2,ier)
		endif
*		We cannot change the sign as it is fixed by the choice
*		of sign in vbffftrans (sqrt(delta(s3,s4))) WRONG
...
*		endif
    ...
    4	    continue
	    if ( .not.ldel2s ) then
		call vbfffcoot(c,calph(1),cpi(5,3),-cpiDpj(5,6,3),
     +			cpi(6,3),csdel2,ier)
		call vbfffcoot(calph(3),c,cpi(5,3),-cpiDpj(5,4,3),
     +			cpi(4,3),csdel2,ier)
	    else
		calph(1) = 1
		calph(3) = 0
...
*
	...
	ier1 = ier
	if ( lcroot ) then
	    call vbfffcc0p(cs4( 1),ipi12(1),isoort(1),clogi(1),ilogi(1),
     +		cpi(1,3),cdpipj(1,1,3),cpiDpj(1,1,3),csdel2,cel2s(1,3),
     +		cetalm(3),cetami(1,3),celpsi(1,3),calph,4,ier0)
	    call vbfffcc0p(cs4(81),ipi12(9),isoort(9),clogi(4),ilogi(4),
     +		cpi(1,4),cdpipj(1,1,4),cpiDpj(1,1,4),csdel2,cel2s(1,4),
     +		cetalm(4),cetami(1,4),celpsi(1,4),calph,4,ier1)
	else
	    if ( lsmug ) call vbfffsm43(xpi3(1,3),7-3)
	    call vbfffxc0p(cs4( 1),ipi12(1),isoort(1),clogi(1),ilogi(1),
     +		xpi3(1,3),dpipj3(1,1,3),piDpj3(1,1,3),sdel2,del2s(1,3),
     +		etalam(3),etami(1,3),delpsi(1,3),alph,4,ier0)
	    if ( lsmug ) call vbfffsm43(xpi3(1,4),7-4)
	    call vbfffxc0p(cs4(81),ipi12(9),isoort(9),clogi(4),ilogi(4),
...
*
	    if ( lwarn ) call vbfffwarn(139,ier,ddel2s(i),xmax)
  510	continue
	if ( .not. lcroot ) then
	    call vbfffdxc0(cs4,ipi12,isoort,clogi,ilogi,xpi3,dpipj3,piDpj3,
  ...
  520		continue
  530	    continue
	    call vbfffdcc0(cs4,ipi12,isoort,clogi,ilogi,cpi,cdpipj,cpiDpj,
...
*	    we need the S term
		...
		if ( lcroot ) then
		   call vbfffcxra(cs4(167),ipi12(23),xqi,qiDqj,sdel2,1,ier)
		else
*		   call ffxtro(cs4(167),ipi12(23),xqi,qiDqj,sdel2,1,ier)
		   call vbfffxtra(cs4(167),ipi12(23),xqi,qiDqj,sdel2,1,ier)
		endif
	    else
		if ( lcroot ) then
		   call vbfffcxra(cs4(167),ipi12(23),xqi,qiDqj,sdel2,2,ier)
		   call vbfffcxra(cs4(169),ipi12(25),xqi,qiDqj,sdel2,3,ier)
		else
		   call vbfffxtra(cs4(167),ipi12(23),xqi,qiDqj,sdel2,2,ier)
		   call vbfffxtra(cs4(169),ipi12(25),xqi,qiDqj,sdel2,3,ier)
...
*
	if ( ai(1) .lt. 0 .neqv. ai(2) .lt. 0 ) then
	    ...
	    ipi12(17) = 12*ilogi(1)
	    if ( ilogi(1) .eq. -999 ) call vbffferr(46,ier)
	    cs4(164) = cipi*clogi(4)
	    ipi12(20) = -12*ilogi(4)
	    if ( ilogi(4) .eq. -999 ) call vbffferr(46,ier)
	endif
	if ( ai(2) .lt. 0 .neqv. ai(3) .lt. 0 ) then
	    cs4(162) = -cipi*clogi(2)
	    ipi12(18) = 12*ilogi(2)
	    if ( ilogi(2) .eq. -999 ) call vbffferr(46,ier)
	endif
	if ( ai(3) .lt. 0 .neqv. ai(1) .lt. 0 ) then
	    cs4(163) = -cipi*clogi(3)
	    ipi12(19) = 12*ilogi(3)
	    if ( ilogi(3) .eq. -999 ) call vbffferr(46,ier)
	endif
	if ( ai(2) .lt. 0 .neqv. ai(4) .lt. 0 ) then
	    cs4(165) = cipi*clogi(5)
	    ipi12(21) = -12*ilogi(5)
	    if ( ilogi(5) .eq. -999 ) call vbffferr(46,ier)
	endif
	if ( ai(4) .lt. 0 .neqv. ai(1) .lt. 0 ) then
	    cs4(166) = cipi*clogi(6)
	    ipi12(22) = -12*ilogi(6)
	    if ( ilogi(6) .eq. -999 ) call vbffferr(46,ier)
	endif
...
*###[ vbfffpi43:
	subroutine vbfffpi43(xpi3,dpipj3,piDpj3,xpi,dpipj,piDpj,imiss,ier)
***#[*comment:***********************************************************
*									*
...
*	endif
   ...
   10	    continue
   20	continue
*	call vbfffxhck(xpi3,dpipj3,6,ier)
*	if ( lscale .and. lwrite ) then
*	    print *,'vbfffpi43: scaled momenta:'
*	    print *,xpi3
*	endif
*  #] calculations:
*###] vbfffpi43:
...
*###[ vbfffxtra:
	subroutine vbfffxtra(cs4,ipi12,xqi,qiDqj,sdel2,ii,ier)
***#[*comment:***********************************************************
*									*
...
*
	integer i,ip(5)
	DOUBLE PRECISION x(2,3),vbfdfflo1,s,s1
*
*	common blocks
*
	include 'ff.h'
...
*  #[ calculations:
	...
	do 10 i=1,3
	    ...
	    call vbfffroot(x(1,i),x(2,i),xqi(ip(i)),-qiDqj(ip(i),
     ...
     +				x(1,i))
		...
		s1 = vbfdfflo1(-2*qiDqj(ip(i),ip(i+1))/(xqi(ip(i))*x(1,i)),
     +				ier)
	    elseif ( s .gt. 0 ) then
		s1 = log(s)
	    else
		if ( abs(s+1) .lt. xloss ) then
		    ...
		    s1 = vbfdfflo1(-2*sdel2/(xqi(ip(i))*x(1,i)),ier)
		else
		    s1 = log(-s)
		endif
...
*		ier = ier + 50
*		print *,'vbfffxtra: imaginary part may well be wrong -> ',
*     +			'n*pi^2 fout'
...
*###[ vbfffcxra:
	subroutine vbfffcxra(cs4,ipi12,xqi,qiDqj,sdel2,ii,ier)
***#[*comment:***********************************************************
*									*
*	calculate the extra terms S_ii^{\infty\prime}, put them in	*
...
*
	integer i,ip(5)
	DOUBLE COMPLEX x(2,3),vbfzfflo1,s,s1,c
...
*  #[ calculations:
	...
	do 10 i=1,3
     ...
     +				/x(1,i)
		...
		s1 = vbfzfflo1(DBLE(-2*qiDqj(ip(i),ip(i+1))/xqi(ip(i)))/
     +				x(1,i),ier)
     ...
     +			x(1,i)
		...
		s1 = vbfzfflo1(DCMPLX(x0,-2*sdel2/xqi(ip(i)))/x(1,i),ier)
...
*###[ vbfffsm43:
	subroutine vbfffsm43(xpi3,imiss)
***#[*comment:***********************************************************
*									*
...
*	cannot be included in vbfffpi43.					*
*									*
***#]*comment:***********************************************************
*  #[ declarations:
	implicit none
