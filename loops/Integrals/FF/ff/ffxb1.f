*###[ vbfffxb1:
	subroutine vbfffxb1(cb1,cb0,ca0i,xp,xm1,xm2,piDpj,ier)
***#[*comment:***********************************************************
*									*
...
*  #[ get differences:
	...
	if ( lwarn ) then
     ...
     +		call vbfffwarn(97,ier0,dm1m2,xm1)
	    if ( abs(dm1p) .lt. xloss*abs(xp) .and. xp .ne. xm1 )
     +		call vbfffwarn(98,ier0,dm1p,xp)
	    if ( abs(dm2p) .lt. xloss*abs(xp) .and. xp .ne. xm2 )
     +		call vbfffwarn(99,ier0,dm2p,xp)
	endif
*  #] get differences:
*  #[ call vbfffxb1a:
	call vbfffxb1a(cb1,cb0,ca0i,xp,xm1,xm2,dm1p,dm2p,dm1m2,piDpj,ier)
*  #] call vbfffxb1a:
*###] vbfffxb1:
	end
*###[ vbfffxb1a:
	subroutine vbfffxb1a(cb1,cb0,ca0i,xp,xm1,xm2,dm1p,dm2p,dm1m2,piDpj,
     +		ier)
***#[*comment:***********************************************************
*									*
*	Calculate   1	/	   d^n Q Q(mu)				*
...
*
	...
	DOUBLE PRECISION xmax,absc,s,s1,h,slam,bnd101,bnd105,bnd110,
     ...
     +		xnul,rloss,xmxp,xlo3,vbfdfflo3
	DOUBLE COMPLEX cs(5),cc,csom
	DOUBLE PRECISION vbfffbnd,vbfdfflo1
...
*  #[ check input:
	if ( ltest ) then
     ...
     +		'vbfffxb1a: error: s1.s2 wrong: ',2*piDpj(1,2),xm1+xm2-xp,
     +		xnul,ier
	    xnul = 2*piDpj(1,3) + xm1 - xm2 + xp
	    if ( rloss*abs(xnul) .gt. precx*xmax ) print *,
     +		'vbfffxb1a: error: s1.p wrong: ',2*piDpj(1,3),-xm1+xm2-xp,
     +		xnul,ier
	    xnul = 2*piDpj(2,3) + xm1 - xm2 - xp
	    if ( rloss*abs(xnul) .gt. precx*xmax ) print *,
     +		'vbfffxb1a: error: s2.p wrong: ',2*piDpj(2,3),-xm1+xm2+xp,
     +		xnul,ier
	endif
*  #] check input:
*  #[ p^2 != 0:
...
* 	#[ almost equal masses:
		...
		cs(1) = -xm2*vbfdfflo1(-dm1m2/xm2,ier)
...
*		#[ real roots:
     ...
     +		    call vbfffwarn(220,ier,precx,xinfac(16)*ax**14)
     ...
     +			then
		    call vbfffwarn(221,ier,h,max(abs(s),abs(s1)))
		endif
		if ( lwrite ) then
		    print *,'arg ',h
     ...
     +			slam)
		...
		if ( abs(h) .lt. .1 ) then
		    cs(2) = dmbma*slam/xp*vbfdfflo1(h,ier)
		else
		    if ( lwrite ) then
		    	print *,
     +			  'vbfffxb1: warning: I thought this was small: ',h
