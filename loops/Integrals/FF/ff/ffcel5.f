*	$Id: vbfffcel5.f,v 1.2 1995/12/08 10:37:10 gj Exp $
*###[ vbfffcel5:
	subroutine vbfffcel5(del5,xpi,pDp,ns,iquad,ier)
***#[*comment:***********************************************************
*									*
...
*
	if ( init .eq. 0 ) then
	    init = 1
	    call vbfffopen(ifile,'ffperm5.dat',ier0)
...
*  #[ check input:
	if ( ltest .and. ns .ne. 15 ) then
	    print *,'vbfffcel5: error: ns <> 15!'
	    stop
	endif
	if ( lwrite ) then
	    print *,'vbfffcel5: xpi = ',xpi
	endif
*  #] check input:
*  #[ out of memory:
*
...
*
	if ( iquad.ne.1 ) then
     ...
     +			then
		    inow = memarr(i,3)
		    if ( lwrite ) print *,'vbfffcel5: found in memory'
		    goto 6
		endif
    5	    continue
    6	    continue
   ...
   20	continue
	if ( iquad.ne.1 .and. absc(del5p) .lt. xloss**2*xmaxp ) then
     ...
     +		j1,j2,j3,j4,j5
		...
		if ( lwarn ) call vbfffwarn(160,ier,absc(del5),xmax)
		goto 800
	    endif
	    goto 10
	endif
...
*  #[ error messages:
	return
  910	print *,'vbfffcel5: error: cannot open file ffperm5.dat with data'
	stop
  920	print *,'vbfffcel5: error: error reading from ffperm5.dat'
	stop
*  #] error messages:
*###] vbfffcel5:
	end
*###[ vbfffcl4p:
	subroutine vbfffcl4p(cl4p,cpi,cpiDpj,ns,ii,ier)
***#[*comment:***********************************************************
...
*  #[ calculations:
   ...
   20	continue
	sprecx = precx
	precx = precc
	call vbfffdl4p(dl4p,xpi,piDpj,10,jj,ier)
	cl4p = dl4p
	precx = sprecx
*  #] calculations:
*  #[ debug output:
	if ( lwrite ) then
	    print *,'vbfffcl4p: input'
	    print *,'ii  = ',ii
	    print *,'cpi = ',cpi
	    print *,'xpi = ',xpi
	    print *,'vbfffdl4s: output ',dl4p
	endif
*  #] debug output:
*###] vbfffcl4p:
	end
*###[ vbfffcl4r:
	subroutine vbfffcl4r(dl4r,xpi,piDpj,ns,miss,ier)
***#[*comment:***********************************************************
*	calculate in a numerically stable way				*
*									*
*	     s1 s2 s3 s4						*
...
*  #[ check input:
	if ( ltest ) then
		...
		print *,'vbfffcl4r: error: miss < 1 or > 5: ',miss
		stop
	    endif
	    do 4 i=1,15
		cnul = 0
    ...
    1		continue
     ...
     +			'vbfffcl4r: error: sum p',i,'.p6-10 do not add ',
     +			'up to 0: ',cnul,xmax
		cnul = 0
		xmax = 0
		do 2 j=11,15
    ...
    2		continue
		if ( xloss*absc(cnul) .gt. precx*xmax ) print *,
     +			'vbfffcl4r: error: sum p',i,'.p11-15 do not add ',
     +			'up to 0: ',cnul,xmax
		do 3 j=6,10
		    k = j+1
		    if ( k.eq.11 ) k=6
		    ...
		    if ( xloss*absc(cnul) .gt. precx*xmax ) print *,
     +			'vbfffcl4r: error: sum p',i,'.p',j,k,j+5,' do ',
...
*
	do 5 i=1,mem
     ...
     +			then
		...
		jnow = memarr(i,4)
		if ( lwrite ) then
		    print *,'vbfffcl4r: found in memory'
...
*	again, a number relative prime to 125 and a few times smaller
  ...
  110	continue
	if ( lwarn ) call vbfffwarn(169,ier,absc(dl4r),xmax)
  120	continue
*  #] calculations:
*  #[ into memory:
     	continue
