*###[ vbfffxdir:
	subroutine vbfffxdir(cs,cfac,idone,xpi,dpipj,ipoin,ndiv,ier)
***#[*comment:***********************************************************
*									*
*	Check if this 4point function is IRdivergent and if so, get it	*
*	using vbfffxdbd and set idone to 1 (or 2 if 2 IR poles)		*
*									*
***#]*comment:***********************************************************
*  #[ declarations:
	implicit none
...
*
			...
			if ( lwrite ) print *,'vbfffxdir: found ',
...
*
     ...
     +				then
			    call vbfffxdbd(cs,cfac,xpi,dpipj,i,j,k,l,ier)
			    goto 98
			endif
			if ( c2sisj(i,j).eq.0 .and. dpipj(i,inx(i,l))
     +				.eq.0 .and. c2sisj(i,l).eq.0 ) then
			    call vbfffxdbd(cs,cfac,xpi,dpipj,i,j,l,k,ier)
			    goto 98
			endif
			if ( c2sisj(i,k).eq.0 .and. dpipj(i,inx(i,l))
     +				.eq.0 .and. c2sisj(i,l).eq.0 ) then
			    call vbfffxdbd(cs,cfac,xpi,dpipj,i,k,l,j,ier)
			    goto 98
			endif
*
*			is it nasty?
...
*
   98	continue
	if ( ldot .and. ipoin.eq.4 ) then
	    ier0 = 0
	    if ( idot.lt.1 ) then
	    	call vbfffdot4(fpij4,xpi,dpipj,10,ier0)
	    endif
	    ii(1)= 5
	    ii(2)= 6
	    ii(3)= 7
		...
		fidel3 = ier0
		call vbfffdl3p(fdel3,fpij4,10,ii,ii,fidel3)
	    endif
	    if ( ltest ) then
	    	if ( lwrite ) print *,'vbfffxdir: checking fdel4s'
		call vbfffdel4(del4s,xpi,fpij4,10,ier0)
		rloss = xloss*DBLE(10)**(-mod(ier0,50))
		if ( rloss*abs(del4s-fdel4s) .gt. precx*abs(del4s) )
     +			print *,'vbfffxdir: error: del4s wrong: ',fdel4s,
...
*
   99	continue
	if ( lwrite ) print *,'vbfffxdir: nasty D0'
	lnasty = .TRUE.
	return
*
*  #] work:
...
*###[ vbfffxdbd:
	subroutine vbfffxdbd(csom,cfac,xpi,dpipj,ilam,i1,i4,ic,ier)
***#[*comment:***********************************************************
*									*
...
*	/vbfffcut/	delta	      real	cutoff to use instead of lam^2	*
*									*
...
*
	...
	DOUBLE COMPLEX vbfzxfflg,vbfzfflog
	save init
*
*	common blocks
*
...
*
	if ( init .eq. 0 ) then
	    init = 1
	    print *,'vbfffxdbd: using IR cutoff delta = lam^2 = ',delta
	endif
	if ( lwrite ) then
	    print *,'vbfffxdbd: input: ilam,i1,i4,ic = ',ilam,i1,i4,ic
	endif
	if ( ltest ) then
	    if ( delta .eq. 0 ) print *,'vbfffxdbd: error: (IR)delta = 0!'
	    if ( xpi(ilam) .ne. 0 ) print *,'vbfffxdbd: error: lam != 0 ',
     +		ilam,xpi(ilam)
	    if ( dpipj(i1,inx(ilam,i1)) .ne. 0 ) print *,
     +		'vbfffxdbd: error: m1^2 != p1^2 ',i1,inx(ilam,i1),xpi(i1),
     +		xpi(inx(ilam,i1)),dpipj(i1,inx(ilam,i1))
	    if ( dpipj(i4,inx(ilam,i4)) .ne. 0 ) print *,
     +		'vbfffxdbd: error: m4^2 != p4^2 ',i4,inx(ilam,i4),xpi(i4),
     +		xpi(inx(ilam,i4)),dpipj(i4,inx(ilam,i4))
	    if ( lsmug ) then
	    if ( c2sisj(i1,ilam).ne.0 ) print *,'vbfffxdbd: error: m(',i1,
     +		') not onshell, c2sisj(',i1,ilam,') = ',c2sisj(i1,ilam)
	    if ( c2sisj(i4,ilam).ne.0 ) print *,'vbfffxdbd: error: m(',i4,
     +		') not onshell, c2sisj(',i4,ilam,') = ',c2sisj(i4,ilam)
	    endif
	endif
	if ( xpi(i1).eq.0 .or. xpi(i4).eq.0 ) then
	    call vbffferr(98,ier)
	    return
	endif
*
*  #] check input:
...
*
     ...
     +		 dpipj(i4,inx(ic,i4)).eq.0 ) then
		if ( lwrite ) print *,'vbfffxdbd: doubly IR case'
		ier0 = 0
		call vbfffxkfn(xxs,iepss,xpi(inx(i1,i4)),xm1,xm4,ier0)
		if ( ier0.ge.100 ) then
		    call vbffferr(44,ier)
		    return
		endif
		ier = ier + ier0
		if ( abs(xxs(2)).gt.xloss ) then
		    zlg = vbfzxfflg(xxs(1),iepss,x0,ier)
		else
		    zlg = DBLE(vbfdfflo1(xxs(2),ier))
		endif
		csom = -2*zlg*vbfzxfflg(-delta/xpi(inx(ilam,ic)),-2,x0,ier)
...
*  #[ special case m0=0, m1=m2, m3!=m4:
     ...
     +		 dpipj(i4,inx(ic,i4)).eq.0 ) then
		...
		if ( lwrite ) print *,'vbfffxdbd: special case m0=0, ',
     +			'm1=m2 but m3!=m4'
*
...
*
		...
		ier1 = ier
		if ( abs(xxs(2)).gt.xloss ) then
		    zlg = vbfzxfflg(xxs(1),iepss,x0,ier0)
		else
		    zlg = DBLE(vbfdfflo1(xxs(2),ier0))
		endif
		cs(1) = zlg**2
		ier1 = max(ier0,ier1)
		ier0 = ier
		if ( xxs(1)**2.lt.xloss ) then
		    cs(2) = -2*DBLE(vbfdfflo1(xxs(1)**2,ier0))*zlg
		else
		    cs(2) = -2*vbfzxfflg(xxs(2)*xxs(3),0,x0,ier0)*zlg
		endif
		ier1 = max(ier0,ier1)
		ier0 = ier
		cs(3) = vbfzxfflg(delta/xpi(i4),0,x0,ier0)*zlg
		ier1 = max(ier0,ier1)
		ier0 = ier
		cs(4) = 2*vbfzxfflg(dpipj(inx(ic,i4),i4)/xpi(inx(ilam,ic)),
     +			-1,dpipj(inx(ic,i4),i4),ier0)*zlg
		ier1 = max(ier0,ier1)
		ier0 = ier
		call vbfffzxdl(cs(5),ip,zlg,xxs(1)**2,iepss,ier0)
		cs(5) = -cs(5)
		ipi12 = -ip + 2
		ier1 = max(ier0,ier1)
		ier = ier1
     ...
     +			call vbfffwarn(194,ier,absc(som),xmax)
*
		if ( lwrite ) then
		    print *,'cs  = '
...
*
	    ...
	    if ( lwrite ) print *,'vbfffxdir: dirty hack, put m0 != 0',xm0
	endif
*  #] special case m0=0, m1!=m2, m3!=m4:
...
*	    all is not what it seems
	...
	ier1 = max(ier0,ier1)
	ier0 = 0
	call vbfffxkfn(xx2,ieps2,xpi(inx(i1,ic)),xm1,xm0,ier0)
	ier1 = max(ier0,ier1)
	ier0 = 0
	call vbfffxkfn(xx3,ieps3,xpi(inx(i4,ic)),xm4,xm0,ier0)
	ier1 = max(ier0,ier1)
	if ( ier1 .ge. 100 ) then
	    call vbfffzdbd(csom,cfac,xpi,dpipj,ilam,i1,i4,ic,ier)
	    return
	endif
	ier = ier + ier1
*
...
*
	...
	if ( abs(d) .lt. xloss ) then
	    cs(1) = 2*zlg*DBLE(vbfdfflo1(d,ier0))
	else
	    cs(1) = 2*zlg*vbfzxfflg(xxs(2)*xxs(3),-iepss,x0,ier0)
	endif
	ier1 = max(ier0,ier1)
	ier0 = 0
	if ( lsmug ) then
	    cs(2) = -2*zlg*vbfzfflog(cxt,iepst,c0,ier0)
	else
	    cs(2) = -2*zlg*vbfzxfflg(xxt(1),iepst,x0,ier0)
	endif
	ier1 = max(ier0,ier1)
*
	ipi12 = 6
*
	ier0 = 0
	call vbfffzxdl(cs(3),ip,zlg,xxs(1)**2,iepss,ier0)
	ipi12 = ipi12 + ip
	ier1 = max(ier0,ier1)
	ier0 = 0
	if ( abs(xx2(2)) .gt. xloss ) then
	    z = vbfzxfflg(xx2(1),ieps2,x0,ier0)
	else
	    z = vbfdfflo1(xx2(2),ier0)
	endif
	cs(4) = z**2
	ier1 = max(ier0,ier1)
	ier0 = 0
	if ( abs(xx3(2)) .gt. xloss ) then
	    z = vbfzxfflg(xx3(1),ieps3,x0,ier0)
	else
	    z = vbfdfflo1(xx3(2),ier0)
	endif
	cs(5) = z**2
	ier1 = max(ier0,ier1)
*
...
*
		ier0 = 0
		call vbfffzxdl(cs(is),ip,zlg,xxs(1)*xx2(1)**i2*xx3(1)**i3,
     +			0,ier0)
		cs(is) = -cs(is)
		ipi12 = ipi12 - ip
		is = is + 1
...
*
		ier0 = 0
		if ( abs(xxs(2)) .gt. xloss ) then
		     cs(is) = -zlg*vbfzxfflg(xxs(1),iepss,x0,ier0)
		else
		     cs(is) = -zlg*DBLE(vbfdfflo1(xxs(2),ier0))
		endif
		is = is + 1
		ier1 = max(ier0,ier1)
*
		ier0 = 0
		if ( abs(xx2(2)) .gt. xloss ) then
		    cs(is) = -zlg*vbfzxfflg(xx2(1)**i2,i2*ieps2,x0,ier0)
		elseif ( i2.eq.1 ) then
		    cs(is) = -zlg*DBLE(vbfdfflo1(xx2(2),ier0))
		else
		    cs(is) = -zlg*DBLE(vbfdfflo1(-xx2(2)/xx2(1),ier0))
		endif
		is = is + 1
		ier1 = max(ier0,ier1)
*
		ier0 = 0
		if ( abs(xx3(2)) .gt. xloss ) then
		    cs(is) = -zlg*vbfzxfflg(xx3(1)**i3,i3*ieps3,x0,ier0)
		elseif ( i3.eq.1 ) then
		    cs(is) = -zlg*DBLE(vbfdfflo1(xx3(2),ier0))
		else
		    cs(is) = -zlg*DBLE(vbfdfflo1(-xx3(2)/xx3(1),ier0))
		endif
		is = is + 1
		ier1 = max(ier0,ier1)
*
...
*###[ vbfffxkfn:
	subroutine vbfffxkfn(x,ieps,xpi,xm,xmp,ier)
***#[*comment:***********************************************************
*									*
...
*
	...
	if ( lwarn .and. abs(xx1) .lt. xloss*max(abs(xpi),xm**2)
     +		) then
	    call vbfffwarn(178,ier,xx1,max(xpi,xm**2))
	    if ( lwrite ) print *,'need extra input'
	endif
	xx2 = 1 - 4*xm*xmp/xx1
	if ( lwarn .and. abs(xx2) .lt. xloss )
     +	    call vbfffwarn(179,ier,xx2,x1)
	if ( xx2 .lt. 0 ) then
	    if ( lwrite ) then
		print *,'vbfffxkfn: cannot handle s < 4*m*mp, to vbfffzdbd'
...
*  #[ print output:
	if ( lwrite ) then
	    print *,'vbfffxkfn: input: xpi,xm,xmp = ',xpi,xm,xmp
...
*###[ vbfffzdbd:
	subroutine vbfffzdbd(csom,cfac,xpi,dpipj,ilam,i1,i4,ic,ier)
***#[*comment:***********************************************************
*									*
*	The IR divergent fourpoint function with real masses		*
*	according to Beenakker & Denner, Nucl.Phys.B338(1990)349.	*
...
*
	...
	DOUBLE COMPLEX vbfzxfflg,vbfzfflog,vbfzfflo1
	save init
*
*	common blocks
*
...
*
	if ( init .eq. 0 ) then
	    init = 1
	    print *,'vbfffzdbd: using IR cutoff delta = lam^2 = ',delta
	endif
	if ( lwrite ) then
	    print *,'vbfffzdbd: input: ilam,i1,i4,ic = ',ilam,i1,i4,ic
	endif
	if ( ltest ) then
	    if ( delta .eq. 0 ) print *,'vbfffzdbd: error: (IR)delta = 0!'
	    if ( xpi(ilam) .ne. 0 ) print *,'vbfffzdbd: error: lam != 0 ',
     +		ilam,xpi(ilam)
	    if ( dpipj(i1,inx(ilam,i1)) .ne. 0 ) print *,
     +		'vbfffzdbd: error: m1^2 != p1^2 ',i1,inx(ilam,i1),xpi(i1),
     +		xpi(inx(ilam,i1)),dpipj(i1,inx(ilam,i1))
	    if ( dpipj(i4,inx(ilam,i4)) .ne. 0 ) print *,
     +		'vbfffzdbd: error: m4^2 != p4^2 ',i4,inx(ilam,i4),xpi(i4),
     +		xpi(inx(ilam,i4)),dpipj(i4,inx(ilam,i4))
	endif
*
*  #] check input:
...
*		    i1 = i
*		endif
*		if ( lwrite ) print *,'vbfffzdbd: special case m0=0, ',
*     +			'm1=m2 but m3!=m4'
**
*		From Wim Beenakker, Priv.Comm.
**
*		call vbfffzkfn(cxs,iepss,xpi(inx(i1,i4)),xm1,xm4,ier)
*		ier0 = ier
*		ier1 = ier
*		if ( absc(cxs(2)).gt.xloss ) then
*		    zlg = vbfzfflog(cxs(1),iepss,c0,ier0)
*		else
*		    zlg = vbfzfflo1(cxs(2),ier0)
*		endif
*		cs(1) = zlg**2
*		ier1 = max(ier0,ier1)
*		ier0 = ier
*		if ( absc(cxs(1))**2.lt.xloss ) then
*		    cs(2) = -2*vbfzfflo1(cxs(1)**2,ier0)*zlg
*		else
*		    cs(2) = -2*vbfzfflog(cxs(2)*cxs(3),0,c0,ier0)*zlg
*		endif
*		ier1 = max(ier0,ier1)
*		ier0 = ier
*		cs(3) = vbfzxfflg(delta/xpi(i4),0,x0,ier0)*zlg
...
*		ier1 = max(ier0,ier1)
*		ier0 = ier
*		call vbfffzzdl(cs(5),ip,zlg,cxs(1)**2,ier0)
*		cs(5) = -cs(5)
*		ipi12 = -ip + 2
*		ier1 = max(ier0,ier1)
*		ier = ier1
...
*     +			)) ) then
*			print *,'vbfffzdbd: error: fac is not real: ',cfac
*		    endif
*		    if ( lwrite ) print *,'del4s = ',fdel4s
*		endif
*		return
...
*	    all is not what it seems
	...
	ier1 = 0
	ier0 = 0
	call vbfffzkfn(cxs,iepss,xpi(inx(i1,i4)),xm1,xm4,ier0)
	ier1 = max(ier0,ier1)
	ier0 = 0
	call vbfffzkfn(cx2,ieps2,xpi(inx(i1,ic)),xm1,xm0,ier0)
	ier1 = max(ier0,ier1)
	ier0 = 0
	call vbfffzkfn(cx3,ieps3,xpi(inx(i4,ic)),xm4,xm0,ier0)
	ier1 = max(ier0,ier1)
	ier = ier + ier1
*
	if ( lwrite ) then
...
*
	ier1 = 0
	ier0 = 0
	zlg = vbfzfflog(cxs(1),iepss,c0,ier)
	c = cxs(1)**2
	if ( absc(c) .lt. xloss ) then
	    cs(1) = 2*zlg*vbfzfflo1(c,ier0)
	else
	    cs(1) = 2*zlg*vbfzfflog(cxs(2)*cxs(3),-iepss,c0,ier0)
	endif
	ier1 = max(ier0,ier1)
	ier0 = 0
	if ( lsmug ) then
...
*
	ier0 = 0
	call vbfffzzdl(cs(3),ip,zlg,cxs(1)**2,ier0)
	ipi12 = ipi12 + ip
	ier1 = max(ier0,ier1)
	ier0 = 0
	z = vbfzfflog(cx2(1),ieps2,c0,ier0)
	cs(4) = z**2
	ier1 = max(ier0,ier1)
	ier0 = 0
	z = vbfzfflog(cx3(1),ieps3,c0,ier0)
	cs(5) = z**2
	ier1 = max(ier0,ier1)
*
	is = 6
...
*
		ier0 = 0
		call vbfffzzdl(cs(is),ip,zlg,cxs(1)*cx2(1)**i2*cx3(1)**i3,
     +			ier0)
		cs(is) = -cs(is)
		ipi12 = ipi12 - ip
		is = is + 1
...
*
		ier0 = 0
		cs(is) = -zlg*vbfzfflog(cxs(1),iepss,c0,ier0)
		is = is + 1
		ier1 = max(ier0,ier1)
*
		ier0 = 0
		cs(is) = -zlg*vbfzfflog(cx2(1)**i2,i2*ieps2,c0,ier0)
		is = is + 1
		ier1 = max(ier0,ier1)
*
		ier0 = 0
		cs(is) = -zlg*vbfzfflog(cx3(1)**i3,i3*ieps3,c0,ier0)
		is = is + 1
		ier1 = max(ier0,ier1)
*
  100	    continue
	...
	if ( lsmug ) then
     ...
     +			cxs(2)*cxs(3)/cxs(1))**2
		...
		if ( xloss*DIMAG(c) .gt. precc*DBLE(c) ) then
		    print *,'vbfffzdbd: error: Del4s is not real ',c
		endif
	    endif
	else
...
*###[ vbfffzkfn:
	subroutine vbfffzkfn(cx,ieps,xpi,xm,xmp,ier)
***#[*comment:***********************************************************
*									*
*	Calculate the K-function in this paper:				*
*									*
...
*
	...
	cx3 = 1/(1+wortel)
	if ( xx1.eq.0 ) then
	    print *,'vbfffzkfn: error: xx1=0, contact author'
	    cx(1) = 1/xclogm
	else
...
*  #[ print output:
	if ( lwrite ) then
	    print *,'vbfffzkfn: input: xpi,xm,xmp = ',xpi,xm,xmp
