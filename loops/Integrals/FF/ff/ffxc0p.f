*	$Id: vbfffxc0p.f,v 1.3 1995/10/06 09:17:26 gj Exp $
*	$Log: vbfffxc0p.f,v $
...
c
*###[ vbfffxc0p:
	subroutine vbfffxc0p(cs3,ipi12,isoort,clogi,ilogi,xpi,dpipj,piDpj,
...
*					answer				*
*									*
*	Calls:	vbfffdel3,vbfffdel3m,vbfffroot,vbfffxxyz,vbfffcxyz,vbfffdwz,vbfffcdwz,	*
*		vbfffcxs3,vbfffcs3,vbfffcxs4,vbfffcs4				*
*									*
***#]*comment:***********************************************************
*  #[ declarations:
	implicit none
	...
	if ( ltest ) then
	    ...
	    do i=1,8
	    	if ( ipi12(i).ne.0 ) then
	    	    print *,'vbfffxc0p: error: ipi12(',i,') != 0: ',
     +	    	    	ipi12(i)
	    	endif
	    enddo
	endif
...
*
	if ( lsmug ) then
		...
		if ( xpi(i) .eq. 0 ) then
			...
			call vbfffrot3(irota,xqi,dqiqj,qiDqj,
     +				xpi,dpipj,piDpj,6,3,4,ier)
			if ( lwrite ) print *,'vbfffxc0p: rotated over ',
     +				irota
			if ( npoin.eq.4 ) call vbfffrt3p(clogip,ilogip,
     +				irota,clogi,ilogi,+1)
			call vbfffxc0j(cs3(1),ipi12(1),sdel2,clogip,ilogip,
     +				xqi,dqiqj,qiDqj,x0,4,ier)
			if ( npoin.eq.4 ) call vbfffrt3p(clogi,ilogi,irota,
     +				clogip,ilogip,-1)
			return
		    endif
		endif
...
*		then handle the special case Si = 0
     ...
     +			 i .eq. 3 .and. alph(1) .eq. 0 ) then
		...
		call vbfffxxyz(y(1,i),z(1,i),dyz(1,1,i),d2yzz(i),dy2z(1,i),
...
*		complex case
		...
		lcompl = .TRUE.
		call vbfffcxyz(cy(1,i),cz(1,i),cdyz(1,1,i),cd2yzz(i),i,
...
*	    no error message; just bail out
	...
	if ( isoort(4) .gt. 0 .and. isoort(2*iw) .ge. 0 ) then
     ...
     +								goto 14
		    l4 = .TRUE.
		    call vbfffdwz(dwz(1,1,iw),w(1,iw),z(1,iw),j,i,iw,
...
*	    undefined variables etc in vbfffdcs, vbfffdcrr.  They should be
...
*		    fantasize imag part, but suppress error message
		    ier0 = 0
		    clogi(i) = vbfzxfflg(s1,1,x1,ier0)
		    print *,'clogi  = ',clogi(i)
		endif
		if ( abs(s1-1) .lt. xloss ) then
		    clogi(i) = vbfdfflo1(d2yzz(i)/dyz(2,2,i),ier)
		    ilogi(i) = 0
		else
		    if ( abs(s1+1) .lt. xloss ) then
			clogi(i) = vbfdfflo1(-2*sdel2i(i)/(xpi(i+3)*
     +				dyz(2,2,i)),ier)
		    else
			clogi(i) = vbfzxfflg(abs(s1),0,x0,ier)
		    endif
...
*		(this may have to be improved to 8*pi/4...)
     ...
     +				i))/xalog2,DIMAG(cdyz(2,2,i))/xalog2)
		    ...
		    clogi(i)=vbfzfflog(cs,0,c0,ier0)
     ...
     +			DCMPLX(x0,pi)*ilogi(i)
	    ...
	    if ( lwrite ) then
		print *,'vbfffxc0p:',i,': ',clogi(i),' + ',ilogi(i),'*i*pi'
	    endif
   95	continue
...
*		normal case
		do 100 i=1,3
			...
			if ( lwrite ) then
			    print *,'vbfffxc0p: xk=0, ma=mb/Si-0 -> S3 = 0'
			    print *,'isoort:',isoort(j),isoort(j+1)
			endif
		    else
			ier0 = ier
			call vbfffcxs3(cs3(20*i-19),ipi12(j),y(1,i),z(1,i),
  ...
  100		continue
		...
		do 110 i=1,3,2
		    ...
		    ier0 = ier
		    call vbfffcxs4(cs3(20*i-19),ipi12(j),w(1,i),y(1,i),
...
*		normal case
		do 200 i=1,3
			...
			call vbfffcxs3(cs3(20*i-19),ipi12(2*i-1),y(1,i),
  ...
  200		continue
		...
		if ( isoort(1) .gt. 0 .and. isoort(3) .gt. 0 ) then
		    call vbfffcxs4(cs3(1),ipi12(1),w(1,1),y(1,1),
     ...
     +			ier0)
		...
		if ( isoort(5) .gt. 0 .and. isoort(7) .gt. 0 ) then
		    call vbfffcxs4(cs3(41),ipi12(5),w(1,3),y(1,3),
     +			z(1,3),dwy(1,1,3),dwz(1,1,3),dyz(1,1,3),
     +			d2yww(3),d2yzz(3),xpi,piDpj,3,6,isoort(5),ier0)
		else
		    call vbfffcs4(cs3(41),ipi12(5),cw(1,3),cy(1,3),
...
*###[ vbfffrt3p:
	subroutine vbfffrt3p(clogip,ilogip,irota,clogi,ilogi,idir)
***#[*comment:***********************************************************
*									*
