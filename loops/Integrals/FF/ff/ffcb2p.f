*###[ vbfffcb2p:
	subroutine vbfffcb2p(cb2i,cb1,cb0,ca0i,cp,xm1,xm2,piDpj,ier)
***#[*comment:***********************************************************
*									*
...
*  #[ real case:
   ...
   20	    continue
	    sprec = precx
	    precx = precc
	    call vbfffxb2p(cb2i,cb1,cb0,ca0i,rp,rm1,rm2,rpiDpj,ier)
	    precx = sprec
	    return
	endif
*  #] real case:
...
*
	...
	dm2p = xm2 - cp
	dm1m2= xm1 - xm2
	call vbfffcb2q(cb2i,cb1,cb0,ca0i,cp,xm1,xm2,dm1p,dm2p,dm1m2,
     +		piDpj,ier)
*
*  #] work:
*###] vbfffcb2p:
...
*###[ vbfffcb2q:
	subroutine vbfffcb2q(cb2i,cb1,cb0,ca0i,cp,xm1,xm2,dm1p,dm2p,dm1m2,
     +		piDpj,ier)
***#[*comment:***********************************************************
*									*
*	Compute the PV B2, the coefficients of p(mu)p(nu) and g(mu,nu)	*
...
*
	...
	DOUBLE COMPLEX cs(14),cc,slam,xlo3,csom,clam,xlogmm,vbfzfflo1,alp,
     +		bet,xnoe,xnoe2,vbfzfflo3
...
*  #[ real cases:
	if ( DIMAG(xm1).eq.0 .and. DIMAG(xm2).eq.0 ) then
	    lreal = .TRUE.
	    if ( lwrite ) print *,'vbfffcb2q: real masses'
	elseif ( nschem.le.4 ) then
	    lreal = .TRUE.
	    if ( lwrite .or. init.eq.0 ) then
		init = 1
		print *,'vbfffcb2q: nschem <= 4, ignoring complex masses:',
     +			nschem
	    endif
	elseif ( nschem.le.6 ) then
	    if ( lwrite .or. init.eq.0 ) then
		init = 1
		print *,'vbfffcb2q: nschem = 5,6 complex masses near ',
     +			'threshold: ',nschem
	    endif
	    cqi(1) = xm1
	    cqi(2) = xm2
		...
		lreal = .TRUE.
		if ( lwrite ) print *,'vbfffcb2q: no threshold'
	    else
		if ( lwrite ) print *,'vbfffcb2q: found threshold'
		lreal = .FALSE.
	    endif
	else
	    lreal = .FALSE.
   ...
   20	    continue
	    sprec = precx
	    precx = precc
	    call vbfffxb2q(cb2i,cb1,cb0,ca0i,rp,rm1,rm2,rm1p,rm2p,
     +		rm1m2,rpiDpj,ier)
	    precx = sprec
	    return
	endif
...
*  #[ test input:
	if ( ltest ) then
	    ier0 = ier
	    call vbfffcot2(qiDqj,cp,xm1,xm2,dm1p,dm2p,dm1m2,ier0)
	    rloss = xloss*DBLE(10)**(-mod(ier0,50))
	    do 40 j=1,3
		do 30 i=1,3
		    if ( rloss*absc(piDpj(i,j)-qiDqj(i,j)).gt.precc*
     +			absc(qiDqj(i,j)) ) print *,'vbfffcb2q: ',
...
*
		...
		xlo3 = vbfzfflo3((cp-slam)/(2*xm1),ier)
...
*
		...
		xlogmm = vbfzfflo1(dm1m2/xm1,ier)
	    else
		xlogmm = log(xm2/xm1)
	    endif
...
*  #[   wrap up:
	    if ( lwarn ) then
		call vbfffwarn(225,ier0,absc(cb2i(1)),xmax)
		if ( lwrite ) then
...
*
		if ( lwarn ) then
		    call vbfffwarn(225,ier,absc(cb2i(1)),xmax)
		    if ( lwrite ) then
			print *,'cp,xm1,xm2 = ',cp,xm1,xm2
		    endif
		endif
...
*
		if ( lwarn ) then
		    call vbfffwarn(226,ier,absc(cb2i(2)),xmax)
		    if ( lwrite ) then
			print *,'cp,xm1,xm2 = ',cp,xm1,xm2
		    endif
		endif
