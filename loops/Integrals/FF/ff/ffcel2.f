*###[ vbfffcel2:
	subroutine vbfffcel2(del2,piDpj,ns,i1,i2,i3,lerr,ier)
*************************************************************************
...
*		we know we have another chance
		...
		if ( lwarn ) call vbfffwarn(71,ier,absc(del2),absc(s2))
	    endif
	endif
*###] vbfffcel2:
	end
*###[ vbfffcl2p:
	subroutine vbfffcl2p(delps1,xpi,dpipj,piDpj,
...
*	NO possibility
	delps1 = som
	if ( lwarn ) call vbfffwarn(92,ier,absc(delps1),xmax)
  100	continue
*  #] stupid tree:
*###] vbfffcl2p:
	end
*###[ vbfffcl2s:
	subroutine vbfffcl2s(delps1,xpi,piDpj,in,jn,jin,isji,
...
*  #[ check input:
	if ( ltest ) then
	    if ( abs(isji) .ne. 1 ) print *,'vbfffcl2s: error: abs(isji) ',
     +		' <> 1 but ',isji
	    if ( abs(islk) .ne. 1 ) print *,'vbfffcl2s: error: abs(islk) ',
     +		' <> 1 but ',islk
	endif
*  #] check input:
*  #[ stupid tree:
...
*	    and the ii's
   ...
   20	continue
	delps1 = som
	if ( lwarn ) call vbfffwarn(83,ier,absc(delps1),smax)
   30	continue
...
*###[ vbfffcl2t:
	subroutine vbfffcl2t(delps,piDpj,in,jn,kn,ln,lkn,islk,iss,ns,ier)
***#[*comment:***********************************************************
*									*
*	calculate in a numerically stable way				*
*									*
...
*  #[ check input:
	if ( ltest .and. abs(islk) .ne. 1 )
     +		print *,'vbfffcl2t: error: abs(islk) <> 1'
*  #] check input:
*  #[ calculations:
	if ( in .eq. jn ) then
	    delps = 0.
	...
	if ( lwrite ) print *,'  delps++= ',delps,islk,-s1,s2
	if ( absc(delps) .ge. xloss*absc(s1) ) goto 10
	if ( lwarn ) call vbfffwarn(93,ier,absc(delps),absc(s1))
   10	continue
*  #] calculations:
*###] vbfffcl2t:
	end
*###[ vbfffcl3m:
	subroutine vbfffcl3m(del3mi,ldel,del3,del2,xpi,dpipj,piDpj,ns,ip1n,
...
*  #] initialisations:
*  #[ superfluous code:
*	if ( ns .ne. 6 ) print *,'vbfffcl3m: called with ns <> 6 !!'
*	if ( ip1n .lt. 4 ) then
*	    lsign = .TRUE.
*	else
*	    lsign = .FALSE.
...
*	    goto 3
*    2	    continue
*	    print *,'vbfffcl3m: unexpected combination of indices',ip1,ip2,
*     +					ip3
*    3	    continue
*	endif
*	this went at he end:
...
*nor does this
*		if ( j .eq. 1 )
*     +			call vbfffcel2(del2s,piDpj,6,ip1,ip2,ip3,1,ier)
*		call vbfffcl2t(delps,piDpj,isi,ip2,ip1,ip2,ip3,+1,+1,6,ier)
...
*
	    som = xsom
	    smax = xmax
	    if ( lwarn ) call vbfffwarn(75,ier,absc(som),smax)
	    if ( lwrite ) then
		print *,'vbfffcl3m: giving up:'
