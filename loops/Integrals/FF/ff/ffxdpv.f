*###[ vbfffxdpv:
	subroutine vbfffxdpv(cd0,cd1,cd2,cd3,cd4,xpi,degree,ier)
***#[*comment:***********************************************************
*									*
...
*		distribute dotproducts
   ...
   30	    continue
	    ...
	    idotsa = idot
	    idot = max(idot,3)
	    call vbfffxc0(cc0i(i),xpi3,ier0)
	    idot = idotsa
	    isgnal = isgnsa
	    ier1 = max(ier1,ier0)
	    del2pi(i) = fdel2
...
*
	...
	do 60 i=1,3
	    do 50 j=i+1,4
		ier0 = ier
		call vbfffxb0(cb0ij(i,j),x0,x0,xpi(inx(i,j)),xpi(i),
     +			xpi(j),ier0)
		cb0ij(j,i) = cb0ij(i,j)
		ier1 = max(ier1,ier0)
   50	    continue
...
*
	...
	do 70 i=1,4
	    ier0 = ier
	    call vbfffxa0(ca0i(i),x0,x0,xpi(i),ier0)
	    ier1 = max(ier1,ier0)
   70	continue
   80	continue
	ier = ier1
...
*###[ vbfffxdpd:
	subroutine vbfffxdpd(cd0,cd1,cd2,cd3,cd4,xpi,piDpj,del3p,del4s,
     +		info,degree,ier)
***#[*comment:***********************************************************
*									*
*	Compute the scalar and tensor functions D0-D(degree) in the	*
...
*  #[ call vbfffxdpv:
*
	call vbfffxdpv(cd0,cd1,cd2,cd3,cd4,xpi,degree,ier)
	idot = 0
*
*  #] call vbfffxdpv:
*###] vbfffxdpd:
