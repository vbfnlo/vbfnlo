*	$Id: vbfffdl2i.f,v 1.4 1996/01/10 15:36:43 gj Exp $
*###[ vbfffdl2i:
	subroutine vbfffdl2i(dl2i,piDpj,ns,i1,i2,i3,isn,j1,j2,j3,jsn,ier)
***#[*comment:***********************************************************
*									*
...
*  #[ check input:
	if ( lwrite ) then
	    print *,'vbfffdl2i: arbitrary 2x2 p-like determinant, ier ',ier
	...
	if ( .TRUE. .or. ltest ) then
     ...
     +		    isn*piDpj(i3,i),xnul,ier
		...
		if ( xlosn*abs(xnul) .gt. precx*xmax ) print *,
     +		    'vbfffdl2i: error: dotproducts ',j1,j2,j3,' with ',i,
...
*
	if ( lwarn ) call vbfffwarn(165,ier,dl2i,xmax)
*
  100	continue
*  #] stupid tree:
*###] vbfffdl2i: 
...
*###[ vbfffdl3q:
	subroutine vbfffdl3q(dl3q,piDpj,i1,i2,i3,j1,j2,j3,
...
*  #[ debug input:
	if ( lwrite ) then
	    print *,'vbfffdl3q: determinant delta(',i1,i2,i3,';5,6,7)'
...
*
	...
	if ( isn1 .eq. -1 ) then
	    ...
	    call vbfffdl2i(del2i(1),piDpj,10, i1,i2,j1,jsn1,6,7,10,+1,ier1)
	    if ( lwrite ) print *,'vbfffdl2t #2'
	    ier0 = ier
	    call vbfffdl2t(del2i(2),piDpj,7,5, i1,i2,j1,-jsn1,-1, 10,ier0)
	    ier1 = max(ier1,ier0)
	    if ( lwrite ) print *,'vbfffdl2i #3'
	    ier0 = ier
	    call vbfffdl2i(del2i(3),piDpj,10, i1,i2,j1,jsn1,5,6,9,-1,ier0)
	    ier1 = max(ier1,ier0)
	...
	if ( isn2 .eq. -1 ) then
	    ier1 = ier
	    if ( lwrite ) print *,'vbfffdl2i #1'
	    call vbfffdl2i(del2i(1),piDpj,10, i2,i3,j2,jsn2,6,7,10,+1,ier1)
	    if ( lwrite ) print *,'vbfffdl2t #2'
	    ier0 = ier
	    call vbfffdl2t(del2i(2),piDpj,7,5, i2,i3,j2,-jsn2,-1, 10,ier0)
	    ier1 = max(ier1,ier0)
	    if ( lwrite ) print *,'vbfffdl2i #3'
	    ier0 = ier
	    call vbfffdl2i(del2i(3),piDpj,10, i2,i3,j2,jsn2,5,6,9,-1,ier0)
	    ier1 = max(ier1,ier0)
	...
	if ( isn3 .eq. -1 ) then
	    if ( lwrite ) print *,'vbfffdl2i #1'
	    ier1 = ier
	    call vbfffdl2i(del2i(1),piDpj,10, i3,i1,j3,jsn3,6,7,10,+1,ier1)
	    if ( lwrite ) print *,'vbfffdl2t #2'
	    ier0 = ier
	    call vbfffdl2t(del2i(2),piDpj,7,5, i3,i1,j3,-jsn3,-1, 10,ier0)
	    ier1 = max(ier1,ier0)
	    if ( lwrite ) print *,'vbfffdl2i #3'
	    ier0 = ier
	    call vbfffdl2i(del2i(3),piDpj,10, i3,i1,j3,jsn3,5,6,9,-1,ier0)
	    ier1 = max(ier1,ier0)
...
*  #] last try:
*  #[ final:
	if ( lwarn ) call vbfffwarn(166,ier,dl3q,xmax)
  900	continue
*  #] final:
*  #[ check output:
	if ( ltest ) then
     ...
     +		abs(s(5)),abs(s(6)))
		...
		print *,'vbfffdl3q: error: answer does not agree with ',
