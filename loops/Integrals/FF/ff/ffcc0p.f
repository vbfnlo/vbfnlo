*###[ vbfffcc0p:
	subroutine vbfffcc0p(cs3,ipi12,isoort,clogi,ilogi,cpi,cpipj,
...
*									*
*	Calls:	vbfffcel2,vbfffcoot,vbfffccyz,vbfffcdwz,vbfffcs3,vbfffcs4			*
*									*
***#]*comment:***********************************************************
*  #[ declarations:
	implicit none
...
*
	...
	logical l4,l4pos
	DOUBLE COMPLEX c,cs,vbfzfflog,cs1,cs2,cs4,ci
...
*  #[ check input:
	if ( ltest ) then
	    ier0 = 0
	    call vbfffchck(cpi,cpipj,6,ier0)
	    if ( ier0 .ne. 0 ) print *,'vbfffcc0p: error: ',
...
*  #[   get z-roots:
	...
	if ( lwrite ) print '(a)','  ##[ get roots: (vbfffcc0p)'
	do 10 i=1,3
*
*	    get roots (y,z)
*
...
*	    first get the roots
     ...
     +		     i .eq. 3 .and. alpha(1) .eq. 0 ) then
	    ...
	    call vbfffccyz(cy(1,i),cz(1,i),cdyz(1,1,i),cd2yzz(i),i,
...
*
	...
	do 20 j=1,2
     ...
     +								goto 14
		l4 = .TRUE.
		call vbfffcdwz(cdwz(1,1,iw),cw(1,iw),cz(1,iw),j,i,iw,
...
*  #[   which case:
   90	if ( l4 ) then
		...
		if ( lwrite ) print *,'vbfffcc0p: cannot handle unphysical'
...
*		    fantasize imag part, but suppress error message
		    print *,'c = ',c
		    clogi(i) = vbfzfflog(c,1,c1,ier0)
		    print *,'clogi  = ',clogi(i)
		endif
		if ( absc(c-1) .lt. xloss ) then
		    cs = cd2yzz(i)/cdyz(2,2,i)
		    clogi(i) = vbfzfflo1(cs,ier)
		    ilogi(i) = 0
		    ismall(i) = 1
		    if ( lwrite ) then
			print *,'c = ',c
     ...
     +			abs(DIMAG(cs)).lt.precc*absc(cs) ) then
			print *,'vbfffcc0p: error: imaginary part doubtful'
			ier = ier + 50
		    endif
		endif
		if ( lwrite ) then
...
*		assume that we got here because of complex sqrt(-delta)
     ...
     +				absc(cs4)) ) then
			print *,'vbfffcc0p: cancellations in delj-deli'
			goto 96
		    endif
		    cs1=ci*DIMAG(cy(2,j))*cs/(csdl2i(i)+csdl2i(j))
		    call vbfffcl2t(cs2,cpiDpj,k+3,j,4,5,6,+1,-1,6,ier)
		    cs2 = -cs2*csdl2i(j)/sdel2/DBLE(cpi(j+3))
		    cs = cs1 + cs2
		    if ( lwrite ) print *,'Tweede poging:',cs,cs1,cs2
		    if ( absc(cs) .lt. xloss*absc(cs1) ) then
			print *,'vbfffcc0p: cancellations in extra terms'
			goto 96
		    endif
...
*	    normal case
		...
		if ( isoort(2*i-1) .eq. 0 ) then
		    if ( lwrite ) then
			print *,'vbfffcc0p: xk=0, ma=mb/Si-0 -> S3 = 0'
			print *,'isoort:',isoort(j),isoort(j+1)
		    endif
		else
		    call vbfffcs3(cs3(20*i-19),ipi12(2*i-1),cy(1,i),
  ...
  200	    continue
	    ...
	    isoort(3) = jsoort(1)
	    isoort(4) = jsoort(2)
	    call vbfffcs4(cs3(1),ipi12(1),cw(1,1),cy(1,1),
     ...
     +			cpi(5)*alpha(3)**2,etami,1,6,isoort(1),ier)
	    ...
	    isoort(7) = jsoort(5)
	    isoort(8) = jsoort(6)
	    call vbfffcs4(cs3(41),ipi12(1),cw(1,3),cy(1,3),
...
*###[ vbfffccyz:
	subroutine vbfffccyz(cy,cz,cdyz,cd2yzz,ivert,csdelp,csdels,etalam,
...
*  #] declarations:
*  #[ set up pointers:
	if ( lwrite ) print *,'vbfffccyz: ivert = ',ivert
	if ( ltest .and. ns .ne. 6 ) then
	    print *,'vbfffccyz: error: ns != 6 !!',ns
	    stop
	endif
	is1 = ivert
	is2 = ivert+1
...
*  #[ check input:
	if ( ltest ) then
	    ier0 = ier
	    dpipj(1,1) = 1
	    call vbfffcl2p(cverg,xpi,dpipj,piDpj,ip1,ip2,ip3,is1,is2,is3,6,
     +		ier0)
	    rloss = xloss*DBLE(10)**(-mod(ier0,50))
	    if ( rloss*absc(cverg-delps).gt.precc*absc(cverg) ) print *,
     +		'vbfffccyz: error: delps <> cverg',delps,cverg,delps-cverg
	    ier0 = ier
	    call vbfffcel2(cverg,piDpj,6,ip1,ip2,ip3,1,ier0)
	    rloss = xloss*DBLE(10)**(-mod(ier0,50))
	    if ( rloss*absc(cverg+csdelp**2) .gt. precc*absc(cverg) )
     +		print *,'vbfffccyz: error: csdelp**2 incorrect ',
     +		csdelp**2,-cverg,csdelp**2+cverg
	    ier0 = ier
	    call vbfffcel3(cverg,xpi,piDpj,6,ier0)
     ...
     +		'vbfffccyz: error: etami(',is2,') incorrect ',
...
*  #[ get cy(1,2),cz(1,2):
	...
	call vbfffcoot(cz(1),cz(2),xpi(ip1),piDpj(ip1,is2),xpi(is2),
     +							csdels,ier)
	cdisc = delps/csdelp
	call vbfffcoot(cy(1),cy(2),xpi(ip1),piDpj(ip1,is2),etami(is2),
     +							cdisc,ier)
*  #] get cy(1,2),cz(1,2):
*  #[ get cy(3,4),cz(3,4):
	cz(4) = 1-cz(2)
	...
	if ( absc(cz(3)) .lt. xloss .or. absc(cz(4)) .lt. xloss ) then
	    call vbfffcoot(cz(4),cz(3),xpi(ip1),-piDpj(ip1,is1),
     +						xpi(is1),csdels,ier)
	endif
...
*	some precautions:
	...
	if ( DIMAG(cz(1)) .gt. 0 .neqv. DIMAG(cz(3)) .lt. 0 ) then
		...
		cz(1) = DCMPLX(DBLE(cz(1)),-DIMAG(cz(3)))
		if ( lwrite ) print *,'vbfffccyz: comment: imaginary ',
	...
	if ( absc(cy(3)) .lt. xloss .or. absc(cy(4)) .lt. xloss ) then
	    call vbfffcoot(cy(4),cy(3),xpi(ip1),-piDpj(ip1,is1),
     +						etami(is1),cdisc,ier)
	endif
	if ( cy(3) .eq. 0 ) cy(1) = 1
	if ( cy(4) .eq. 0 ) cy(2) = 1
...
*  #[ test output:
   ...
   99	    continue
     ...
     +		print *,'vbfffccyz: error: cd2yzz <> 2*cy(2)+cz(1)+cz(2):',
