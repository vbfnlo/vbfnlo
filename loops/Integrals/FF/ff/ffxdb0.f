*###[ vbfffxdb0:
	subroutine vbfffxdb0(cdb0,cdb0p,xp,xma,xmb,ier)
***#[*comment:***********************************************************
*									*
...
*###[ vbfffxdbp:
	subroutine vbfffxdbp(cdb0,cdb0p,xp,xma,xmb,dmap,dmbp,dmamb,ier)
***#[*comment:***********************************************************
*									*
...
*  #[ check input:
	if (ltest) then
     ...
     +			dmamb))/xloss ) then
		print *,'vbfffxdbp: input not OK, dmamb <> xma-xmb',xcheck
	    endif
	    xcheck = -xp + xma - dmap
	    if ( abs(xcheck) .gt. precx*max(abs(xp),abs(xma),abs(
     +			dmap))/xloss ) then
		print *,'vbfffxdbp: input not OK, dmap <> xma - xp',xcheck
	    endif
	    xcheck = -xp + xmb - dmbp
	    if ( abs(xcheck) .gt. precx*max(abs(xp),abs(xmb),abs(
     +			dmbp))/xloss ) then
		print *,'vbfffxdbp: input not OK, dmbp <> xmb - xp',xcheck
	    endif
	endif
*  #] check input:
*  #[ which case:
...
*
	elseif ( dmp.eq.0 ) then
		...
		if ( initir.eq.0 ) then
		    initir = 1
		    print *,'vbfffxdb0: IR divergent B0'', using cutoff ',
     +		    	delta 
		endif
		if ( delta.eq.0 ) then
...
* 	#[ Taylor expansion:
	    ...
	    if ( lwarn .and. ax .gt. bdn020 ) then
	    	call vbfffwarn(15,ier,precx,xninv(21)*ax**20)
	    endif
     ...
     +		      x*(xninv(6) + som ))))
	    ...
	    if ( lwrite ) then
		print *,'cdb0p = ',cdb0p
		print *,'verg   ',-1 - xm/xp*vbfdfflo1(x,ier),1
	    endif
* 	#] Taylor expansion:
	else
* 	#[ short formula:
...
*--#]	    data and bounds:
     ...
     +		xpneq(5) + som ))))
	    ...
	    cdb0p = -x*(xpneq(1)+som)
	    if (lwrite) then
		print *,'vbfffxdbp: m1 = m2, Taylor expansion in ',x
		print *,'cdb0p = ',cdb0p
	    endif
	    if ( xp.ne.0 ) then
		cdb0 = cdb0p*(1/DBLE(xp))
...
*		the following line is superfluous.
     ...
     +			call vbfffwarn(17,ier,s2,slam)
		s = 2*xm/slam*vbfdfflo1(s2/(2*xm),ier)
	    else
...
*	    the root is complex (k^2 between 0 and (2*m1)^2)
	...
	if (lwrite) print *,'s =   ',s
	xx = s - 1
	if ( lwarn .and. abs(xx).lt.xloss ) call vbfffwarn(18,ier,xx,x1)
...
*		second try
*
		h = vbfdfflo3(dm1m2/xm1,ier)
		s1 = -xm1*h/dm1m2**2
		s2 = 1/(2*xm1)
		s3 = xm1**2*h/dm1m2**3
		s = s1 + s2 + s3
		if ( abs(s) .lt. xloss*max(abs(s2),abs(s3)) ) then
		    call vbfffwarn(228,ier,s,s2)
		endif
	    endif
	    cdb0 = s
	    cdb0p = 0
...
*
	...
	if ( abs(diff) .lt. xloss*xlam ) then
		...
		if ( lwarn ) then
		    call vbfffwarn(221,ier,diff,min(dm1m2**2,abs(xlam)))
		endif
	    endif
	endif
	if ( xlam .eq. 0 ) then
...
*		looks fine
		...
		if ( jsign.eq.1 ) then
		    ...
		    s2 = -diff/(2*slam*xp)*vbfdfflo1(s2,ier)
		else
		    ier = ier + 50
		    print *,'vbfffxdb0: untested: s2 better in first try'
		    if (lwrite) print *,'s2 ',+diff/(2*slam*xp)*log(s2)
		    s2 = +slam*(s2a-slam)/(2*xm1*xm2)
		    s2 = +diff/(2*slam*xp)*vbfdfflo1(s2,ier)
		endif
	    ...
	    xx = s1+s2-1
	    if (lwrite) then
		print *,'vbfffxdbp: lam>0, first try, xx  = ',xx,s1,s2,-1
	    endif
*--#]	    first try:
...
*		    choose a quick way to get the logarithm
		    s2 = vbfdfflo1(s2,ier)
		elseif ( s2.eq.1 ) then
		    print *,'vbfffxdbp: error: arg log would be 0!'
		    print *,'        xp,xma,xmb = ',xp,xma,xmb
		    goto 600
		else
		    h = abs(1-s2)
		    s2 = vbfzxfflg(h,0,c0,ier)
		endif
		s2 = -diff/(slam*xp)*s2
		xx = s1 + s2 - 1
		if (lwrite) then
		    print *,'vbfffxdbp: lam>0, 2nd try, xx  = ',xx,s1,s2,-1
		endif
*--#]		second try:
		if ( abs(xx) .lt. xloss**2*max(abs(s1),abs(s2)) ) then
*--#[		    third try:
...
*		#[ split up 1:
		    ...
		    call vbfffroot(d1,d2,a,b,c,d,ier)
		    if ( xp.gt.0 ) then
			beta = d2
		    else
			beta = d1
     ...
     +				betm2n
			...
			if ( lwarn .and. abs(alph1).lt.xloss*xmax ) then
			    call vbfffwarn(222,ier,alph1,xmax)
...
*			    do not do the Taylor expansion
     ...
     +				call vbfffwarn(24,ier,s2p,s2a)
			s2p = -diff/(xp*slam)*vbfdfflo1(s2p,ier)
			if (lwrite) then
			    print *,'vbfffxdbp: Taylor expansion of s2-a'
			    print *,'	     in x = ',x
			    print *,'	     gives s2p = ',s2p
			endif
		    endif
...
*
     ...
     +				abs(s1e))
			if ( lwarn .and. abs(s1p).lt.xloss*xmax ) then
			    call vbfffwarn(223,ier,s1p,xmax)
			    if ( lwrite )
     +				print *,'s1p,s1e,s1d,s1c,s1b,s1a,s1f = '
     +				,s1p,s1e,s1d,s1c,s1b,s1a,s1f
			endif
			s1p = s*vbfdfflo1(s1p,ier)
			if (lwrite) then
			    print *,'s1a = ',s1a
			    print *,'s1b = ',s1b
			    print *,'s1c = ',s1c
			    ...
			    print *,'s1f = ',s1f
			    print *,'s   = ',s
			    print *,'vbfffxdbp: Taylor exp. of s1-(1-a)'
			    print *,'        in x = ',x
			    print *,'        gives s1p = ',s1p
...
*--#]	    first try:
     ...
     +			then
		call vbfffwarn(224,ier,xx,max(abs(s1),abs(s2)))
	    endif
	    y = 0
	endif
     	continue
