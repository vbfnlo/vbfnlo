*	$Id: vbfffxe1.f,v 1.6 1997/04/07 19:10:57 gj Exp $
*###[ vbfffxe1:
	subroutine vbfffxe1(ce1i,ce0,del3ij,del4i,cd0i,xpi,piDpj,del4,ier)
***#[*comment:***********************************************************
*									*
*	calculate the							*
...
*  #[ check input:
    ...
    1	    continue
	    ier0 = ier
	    call vbfffdl4p(del4p,xpi,piDpj,15,jj,ier0)
	    xheck = del4 - del4p
	    if ( xloss*abs(xheck) .gt. precx*abs(del4) ) print *,
     +		'vbfffxe1: error: del4 wrong ',del4,del4p,xheck
	endif
*  #] check input:
*  #[ work:
*
...
*		we do not need (3,3), but compute it anyway for export
		ier0 = ier
		idsub = idsub + 1
		call vbfffdl3p(del3ij(i,j),piDpj,15,ii(1,i),ii(1,j),ier0)
   ...
   30	continue
	do 40 i=1,4
	    ier0 = ier
	    idsub = idsub + 1
	    call vbfffdl4s(del4i(i),xpi,piDpj,15,1,i+5,10,ier0)
	    ier1 = max(ier1,ier0)
   40	continue
	ier = ier1
*
...
*
  ...
  120	    continue
		...
		ier0 = ier
		call vbfffwarn(171,ier0,absc(ce1i(i)),xmax)
		ier1 = max(ier1,ier0)
	    endif
	    if ( lwrite ) then
		do 125 j=1,11
...
*	    test a few identities: 2pi.Q = N(i+1)-Ni+2s1.pi
		...
		enddo
	    	if ( lwrite ) print *,'vbfffxe1: checking E1.p(',i,'): ',
     +	    		cnul,xmax,ier
		if ( xlosn*absc(cnul).gt.precc*xmax ) then
		    print *,'vbfffxe1: error: E1 fails consistency check',i
...
*###[ vbfffdl4s:
	subroutine vbfffdl4s(del4,xpi,piDpj,ns,is,miss1,miss2,ier)
***#[*comment:***********************************************************
*									*
...
*  #[ check input:
	if ( lwrite ) then
	    print *,'vbfffdl4s: is,miss1,miss2 = ',is,miss1,miss2
	endif
	if ( ns.ne.15 ) then
	    print *,'vbfffdl4s: only for ns=15, not ',ns
	    stop
	endif
*  #] check input:
*  #[ special case:
...
*
	...
	if ( i.eq.1 .or. i.eq.4 ) then
	    ...
	    if ( .not.( is .eq. j ) ) then
		if ( lwrite ) print *,'vbfffdl4s: using vbfffdl4r'
		call vbfffdl4r(del4,xpi,piDpj,ns,j,ier)
		return
	    endif
	endif
*  #] special case:
...
*
	do 5 i=1,mem
		...
		inow = memarr(i,3)
		jnow = memarr(i,4)
		if ( lwrite ) print *,'vbfffdel5: found in memory'
		goto 6
	    endif
    5	continue
    6	continue
