# This routine can be used to check the differences between old and new regress runs for different levels of statistics.
...
def check_results(args):
    ...
    #The version below does not check subdirs (does not take the individual parallel runs into account)
    ...
    for checkdir in alldirs:
        ...
        if args.stat == "low":
            outfiles = glob.glob(checkdir + '/' + args.jobprefix + 'result_10_10*.o*')
        elif args.stat == "mid":
            outfiles = glob.glob(checkdir + '/' + args.jobprefix + 'result_20_20*.o*')
        elif args.stat == "high":
            outfiles = glob.glob(checkdir + '/' + args.jobprefix + 'result_high*.o*')
        else:
            outfiles = glob.glob(checkdir + '/' + args.jobprefix + 'result_*.o*')
