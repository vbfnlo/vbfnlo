        ...
        try:
            ...
            if exce.returncode == 2:
                ...
                if os.getenv('RUNALLPROCESSES', None) == 'yes':
                    ...
                    pytest.skip("Process not compiled. Compile with e.g. --enable-processes=all or --enable-kk etc to run the complete testsuite.")
...
def compare_output(out, referenceout):
    ...
    for refout in referenceout:
        ...
        for line in refclean:
            if line not in outclean:
                foundmatch = False
    if not foundmatch:
        assert '\n'.join(outclean) == '\n'.join(refclean)
    if foundmatch:
