...
AC_ARG_ENABLE(kk,
        AS_HELP_STRING([--enable-kk],[--enable-kk to enable simulation of Kaluza-Klein resonances]),
        [],
        [enable_kk=no]
        )
...
AC_ARG_ENABLE(NLO,
        AS_HELP_STRING([--enable-NLO],[--disable-NLO to disable next-to-leading order QCD]),
        [],
        [enable_NLO=yes]
        )
...
AC_ARG_ENABLE(MPI,
        AS_HELP_STRING([--enable-MPI],[--enable-MPI to use MPI parallelization]),
        [],
        [enable_MPI=no]
        )
...
AC_ARG_ENABLE(quad,
        AS_HELP_STRING([--enable-quad],[--enable-quad to enable quad precision for difficult phase space points]),
        [],
...
dnl check whether we know how to compile in quad precision
    ...
    elif (test "x$VBFFC" = "xmpif90"); then
      AM_FCFLAGS="$AM_FCFLAGS -DQREAL=REALPART -DQIMAG=IMAGPART -DQCMPLX=COMPLEX"
      QUADFCFLAGS="-fdefault-real-8 -DQUAD=1 -DKIND=2"
...
AC_DEFUN([VBFNLO_APPEND_BLHA],
[
AC_MSG_CHECKING([whether _VBFNLO should be appended to the BLHA interface functions])
AC_ARG_ENABLE(custom_blha,
        AS_HELP_STRING([--enable-custom-blha],[append _VBFNLO to BLHA interface functions]),
        [],
        [enable_custom_blha=no]
        )
AC_MSG_RESULT([$enable_custom_blha])
AM_CONDITIONAL(WITH_CUSTOM_BLHA,[test "x$enable_custom_blha" = "xyes"])
])
...
AC_ARG_ENABLE(processes,
        AS_HELP_STRING([--enable-processes=LIST],[Comma-separated list of processes to enable. Options are: vbf, ggf, diboson, dibosonjet, triboson, tribosonjet, hjjj, qcdvvjj, qcdvjj, all_except_hexagons, all. Default is all_except_hexagons.]),
        [],
...
AM_CONDITIONAL(WITH_FERMIONLOOPS,[test "$diboson" -o "$dibosonjet" -o "$qcdvjj" -o "$qcdvvjj" -o "$ggf" -o "$all" -o "$all_except_hexagons"])
...
AM_COND_IF([WITH_FERMIONLOOPS],
  [supports_quad=yes],
  [
if [test "$all" -o "$qcdvjj" -o "$qcdvvjj" -o "$ggf" ]; then
  supports_quad=yes
else
  supports_quad=no
fi
...
AC_ARG_WITH(root,
        AS_HELP_STRING([--with-root=DIR],[location of ROOT installation]),
        [],
        [with_root=no])
...
AC_ARG_WITH(LHAPDF,
			AS_HELP_STRING([--with-LHAPDF=DIR],[location of LHAPDF installation]),
...
AC_ARG_WITH(hepmc,
        AS_HELP_STRING([--with-hepmc=DIR],[location of HepMC installation]),
        [],
        [with_hepmc=no])
...
[
  ...
  check_gfortran=`echo $FC | grep gfortran || echo $FC | grep mpifort|| echo $FC | grep mpif90`
...
AC_ARG_ENABLE(madgraph,
        AS_HELP_STRING([--enable-madgraph],[--enable-madgraph to include code for MadGraph comparisons]),
        [],
        [enable_madgraph=no]
        )
...
AC_ARG_ENABLE(debug,
        AS_HELP_STRING([--enable-debug],[--enable-debug to enable more compile warnings]),
        [],
        [enable_debug=no]
        )
...
echo ""
echo "  FORTRAN compiler  $FCSTRING"
echo "  C++ compiler      $CXXSTRING"
